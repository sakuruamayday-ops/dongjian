 ...
 * first. With nothing to list, activating the composer surface either raises
 * the dialog directly or opens the searchable picker whose add action raises
 * it. The directory is staged here
...
export async function connectFreshWorkspace(page: Page, root: string, name = 'workspace'): Promise<void> {
  ...
  const dialog = await openWorkspaceDirectoryDialog(
    page, 'Choose workspace', 'Select Workspace Directory', 'New enterprise space')
...
async function openWorkspaceDirectoryDialog(
  page: Page,
  triggerName: string,
  dialogName: string,
  addName: string,
): Promise<Locator> {
  await page.getByRole('textbox', { name: triggerName }).click()
  const dialog = page.getByRole('dialog', { name: dialogName })
  const add = page.getByRole('menuitem', { name: addName, exact: true })
  const route = await Promise.race([
    dialog.waitFor({ timeout: 10_000 }).then(() => 'dialog' as const),
    add.waitFor({ timeout: 10_000 }).then(() => 'menu' as const),
  ])
  if (route === 'menu') {
    await add.click()
