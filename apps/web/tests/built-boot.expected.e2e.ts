...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  // The compact layout dropped group session counts and Alpha now keeps the
  // workspace collapsed until the user opens it. Expand it explicitly before
  // addressing the resident fixture session; otherwise this smoke would depend
  // on a stale default-expanded layout detail instead of the boot graph.
  const fixtureGroup = (await within(tree).findAllByText('fixture'))
    .map(el => el.closest<HTMLElement>('[role="treeitem"]'))
    .find((el): el is HTMLElement => el !== null && el.getAttribute('aria-expanded') !== null)
  if (fixtureGroup === undefined) throw new Error('fixture Workspace group missing')
  if (fixtureGroup.getAttribute('aria-expanded') === 'false') {
    fireEvent.click(within(fixtureGroup).getByText('fixture'))
    await waitFor(() => {
      expect(fixtureGroup.getAttribute('aria-expanded')).toBe('true')
    })
  }
