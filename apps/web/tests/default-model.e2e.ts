...
const MODEL_SEARCH_EXPECTED = join(
  fileURLToPath(new URL('./snapshots/default-model', import.meta.url)),
  'model-search.expected.md',
)
...
describe('web e2e: the composer model switch is the default for later sessions', () => {
  ...
  it('writes the switched model as the default and leaves a logged session alone', async () => {
    ...
    await expect.poll(async () => search.evaluate(element => element === document.activeElement)).toBe(true)
    await search.fill('Acme')
    await page.getByRole('menuitemradio', { name: 'Acme Large' }).waitFor()
    expect(await page.getByRole('menuitemradio', { name: 'Origin Large' }).count()).toBe(0)
    await compareOrRefreshGolden(
      MODEL_SEARCH_EXPECTED,
      await captureStableAria(page, '[role="menu"]', scaffold.workspaceCwd),
      MODE,
    )

    await search.fill('')
    await page.getByRole('menuitem', { name: '收起 Acme Gateway' }).click()
    expect(await page.getByRole('menuitemradio', { name: 'Acme Large' }).count()).toBe(0)
    await search.fill('acme-large')
    await page.getByRole('menuitemradio', { name: 'Acme Large' }).waitFor()
    await search.fill('')
    expect(await page.getByRole('menuitemradio', { name: 'Acme Large' }).count()).toBe(0)
    await page.getByRole('menuitem', { name: '展开 Acme Gateway' }).click()
    ...
    expect(settingsDocument).toContain(`provider: ${ROUTE}`)
    expect(settingsDocument).toContain(`model: ${MODEL}`)
