...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      // The notice resolves its durable settings after the shell frame, so an
      // immediate count can race its first render on a brand-new DSH_HOME.
      await welcome.waitFor({ timeout: 15_000 })
      await welcome.getByRole('button', { name: 'Continue', exact: true }).click()
      await welcome.waitFor({ state: 'detached', timeout: 15_000 })
    } catch (error: unknown) {
      if (await welcome.count() > 0) throw error
    }
  }, 120_000)
  ...
  it('cold start: loading page settles into the three-column frame', async () => {
    ...
    const template = await page.locator('[data-app-frame]').evaluate(el => getComputedStyle(el).gridTemplateColumns)
