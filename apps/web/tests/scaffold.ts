...
// A `createdAt: 0` seed intentionally follows wall time. Remember its exact
// rendered clocks per temp workspace so crossing midnight does not change a
// golden, while dated historical fixtures keep asserting their calendar day.
const volatileSeedClockLabels = new Map<string, Set<string>>()

/**
 * Register the exact wall-clock label emitted by one volatile test seed.
 * @param workspaceCwd - Isolated test workspace owning the seed.
 * @param time - Seed timestamp materialized from the current wall clock.
 */
export function registerVolatileSeedClock(workspaceCwd: string, time: number): void {
  ...
  const labels = volatileSeedClockLabels.get(workspaceCwd) ?? new Set<string>()
  labels.add(`${month}/${day} ${clock}`)
  labels.add(`${year}-${month}-${day} ${clock}`)
  labels.add(`${month}月${day}日 ${clock}`)
  labels.add(`${year}年${month}月${day}日 ${clock}`)
  volatileSeedClockLabels.set(workspaceCwd, labels)
}

function normalizeVolatileSeedClocks(snapshot: string, workspaceCwd: string): string {
  const labels = volatileSeedClockLabels.get(workspaceCwd)
  if (labels === undefined) return snapshot
  return [...labels]
    .sort((left, right) => right.length - left.length)
    .reduce((result, label) => result.split(label).join('{{clock}}'), snapshot)
}
...
export async function seedSession(
  ...
  if (fixtureCreatedAt === 0) {
    registerVolatileSeedClock(scaffold.workspaceCwd, meta.createdAt)
    for (const event of materializedEvents) registerVolatileSeedClock(scaffold.workspaceCwd, event.time)
  }
...
/**
 * Normalize nondeterministic Web snapshot values without erasing fixture calendar dates.
 * @param snapshot - Raw Playwright ARIA snapshot.
 * @param workspaceCwd - Isolated workspace path and volatile-seed identity.
 * @param age - Whether relative age labels are nondeterministic for this capture.
 * @returns stable snapshot text retaining fixed historical calendar values.
 */
export function normalizeAria(snapshot: string, workspaceCwd: string, age: boolean): string {
  // The session heading renders the workspace's basename, not the full
  // path, so both spellings must collapse to the token.
  const base = workspaceCwd.split('/').pop()!
  const normalizedClocks = normalizeVolatileSeedClocks(snapshot, workspaceCwd)
  return (age ? normalizedClocks.replace(ARIA_AGE, '{{age}}') : normalizedClocks)
    ...
    // Absolute machine timestamps remain transport noise. Exact dynamic seed
    // dates were narrowed above; fixed fixture calendar dates remain evidence,
    // while their rendered time-of-day is normalized across host time zones.
