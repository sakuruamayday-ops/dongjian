...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it.skipIf(MODE === 'record')('quotes a native double-click selection from the reasoning disclosure', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-round-trip-reasoning-selection'))
    const summary = page.getByText(
      'The user wants me to run a simple bash command and reply with "DONE".',
      { exact: true },
    ).first()
    await summary.dblclick()
    const action = page.getByRole('button', { name: 'Add to conversation' })
    ...
    await expect.poll(() => composer.textContent(), { timeout: 5_000 })
      .toContain(`> ${selected}`)
    await expect.poll(() => composer.evaluate(element => document.activeElement === element), { timeout: 5_000 })
      .toBe(true)
  })
