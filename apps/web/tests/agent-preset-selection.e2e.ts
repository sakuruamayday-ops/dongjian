...
describe('web e2e: agent-preset selection', () => {
  ...
  /** The new-session preset chip, excluding similarly named menu/settings rows. */
  function presetTrigger(name: string | RegExp): ReturnType<Page['getByRole']> {
    return page.locator('[class*="heroWorkspaceRow"]').getByRole('button', { name })
  }
  ...
  it('names every preset and what it is for', async () => {
    ...
    await presetTrigger('Standard mode').click()
  ...
  it('re-reads the slash catalog through the composition the switch installed', async () => {
    ...
    await presetTrigger('Minimal mode').click()
