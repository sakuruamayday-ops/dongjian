...
export function mountAssembledApp(search = '?fixture', options: AssembledBootOptions = {}): void {
  ...
  const target: ClientModuleLoaderTarget = {
    ...
    create: (createOptions) => {
      ...
      if (registration === undefined) throw new Error('assembled boot: modules bootstrap registration missing')
      pendingQueue.splice(index, 1)
      const exports = registration.factory((specifier) => {
        throw new Error(`assembled boot: modules bootstrap requested ${specifier}`)
      }) as {
        createClientModuleSystem: (
          loader: ClientModuleLoaderTarget,
          bootstrap: { id: string; exports: Record<string, unknown> },
          options: Parameters<ClientModuleLoaderTarget['create']>[0],
        ) => ReturnType<ClientModuleLoaderTarget['create']>
      }
      return exports.createClientModuleSystem(target, { id: registration.id, exports }, createOptions)
    },
  }
