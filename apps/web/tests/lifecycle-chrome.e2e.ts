...
describe('web e2e: lifecycle & chrome (workspace flow / reload / dark mode)', () => {
  ...
  it.skipIf(MODE === 'record')('shows active Plan as the warn-state status action', async () => {
    ...
    try {
      ...
      await activePage.waitForSelector('[data-app-frame]', { timeout: 30_000 })
      ...
      const planSnapshot = await captureStableAria(activePage, '[data-app-frame]', activeScaffold.workspaceCwd)
  ...
  it('sends the first prompt from the empty-state hero (all modes)', async () => {
    ...
    if (MODE !== 'record') {
      ...
      const snapshot = await captureStableAria(page, '[data-app-frame]', scaffold.workspaceCwd)
  ...
  it.skipIf(MODE === 'record')('shows automatic and user-requested connection recovery beside Settings', async () => {
    ...
    try {
      ...
      await recoveryPage.waitForSelector('[data-app-frame]', { timeout: 30_000 })
