...
async function openPerformancePage(world: PerformanceWorld): Promise<Locator> {
  ...
  await world.page.waitForSelector('[data-app-frame]', { timeout: 30_000 })
  const group = world.page.getByRole('treeitem').first()
  // Current workspace headers omit counts. Expanded rows and exact search
  // results below verify cardinality without warming the measured expansion.
  await expect.poll(() => group.textContent(), { timeout: 30_000 }).toContain('Ungrouped')
  return group
}
  ...
  it('reports workspace, history, and trajectory cardinality costs', async () => {
    ...
    try {
      ...
      const coldTrajectory = await measure(cdp, async () => {
        ...
        const mounted = await stableCount(trajectoryRows, count => count > 0)
        const loaded = Number(await trajectoryTable.getAttribute('aria-rowcount'))
        expect(loaded).toBeGreaterThanOrEqual(mounted)
        expect(loaded).toBeLessThan(EXPECTED_TRAJECTORY_ROWS)
      ...
      const warmTrajectory = await measure(cdp, async () => {
        ...
        const expand = page.getByRole('button', { name: 'Expand turns', exact: true })
        if (await expand.count() > 0) await expand.click()
        // The ledger now pages and virtualizes independently of Chat. Verify
        // its logical count without demanding thousands of mounted DOM rows.
        for (let pages = 0; pages < 100; pages += 1) {
          const earlier = trajectoryTable.getByRole('button', { name: 'Load earlier history', exact: true })
          if (await earlier.count() === 0) break
          const before = Number(await trajectoryTable.getAttribute('aria-rowcount'))
          await earlier.click()
          await expect.poll(async () => {
            const current = Number(await trajectoryTable.getAttribute('aria-rowcount'))
            // Finishing removes the one synthetic "load earlier" row.
            return current > before || (current === EXPECTED_TRAJECTORY_ROWS && await earlier.count() === 0)
          }).toBe(true)
        }
        await expect.poll(async () => Number(await trajectoryTable.getAttribute('aria-rowcount'))).toBe(EXPECTED_TRAJECTORY_ROWS)
        const mounted = await stableCount(trajectoryRows, count => count > 0 && count < EXPECTED_TRAJECTORY_ROWS)
        return { mounted, loaded: Number(await trajectoryTable.getAttribute('aria-rowcount')) }
      })
      expect(warmTrajectory.value.loaded).toBe(EXPECTED_TRAJECTORY_ROWS)
