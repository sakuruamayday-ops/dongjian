...
describe('web e2e: queue row actions', () => {
  ...
  it.skipIf(MODE === 'record')('edits and removes exact occurrences and preserves Queue across stop', async () => {
    ...
    await expect.poll(() => queueHeader.getAttribute('aria-expanded'), { timeout: 10_000 })
      .toBe('false')
    // Queue projection and editability can arrive before the hanging model's
    // partial output and whole-queue steering gesture are both live.
  ...
  it.skipIf(MODE === 'record')('orders Todo before Goal and Queue on one responsive card column', async () => {
    ...
    await expect.poll(() => queueHeader.getAttribute('aria-expanded'), { timeout: 10_000 })
      .toBe('false')
    // Keep the layout golden tied to the hanging partial and the actionable
    // queue state; the queue header itself may render earlier than either.
    await page.getByText('partial', { exact: true }).waitFor({ timeout: 10_000 })
    await page.getByRole('textbox', { name: 'Cmd/Ctrl+Enter steers all queued messages' })
      .waitFor({ timeout: 10_000 })
