...
/** Product preset root beside this app's config in both source and built layouts. */
...
async function composeProfile(
  ...
  trustedPatchFiles: readonly string[] = [],
  trustedPatches: readonly PatchOptions[] = [],
  userLayers = true,
  healModuleFallback = true,
): Promise<ComposedProfile> {
  const profile = prepareProfile(name, userLayers)
  if (healModuleFallback) await healProfilesModuleFallback({ installAnchor: INSTALL_ANCHOR, profile })
  const homePatches = userLayers ? loadOptionalPatches(NAME, homePatchPath()) ?? [] : []
  const overlays = patchFiles.flatMap(file => loadOverlayPatches(NAME, resolve(file)))
  const trustedOverlays = [
    ...trustedPatchFiles.flatMap(file => loadOverlayPatches(NAME, resolve(file))),
    ...structuredClone(trustedPatches),
  ]
  const bundlePatches = profile.layers.flatMap(layer => layer.patches)
  const rows = new Map<string, EntryOptions>()
  for (const row of composeEntries([bundlePatches, profile.patches, homePatches, overlays, trustedOverlays])) {
    if (typeof row.id === 'string') rows.set(row.id, row)
  }
  // Host-trusted overlays remain above profile, home, and command-line layers.
  // A packaged product can therefore pin security-critical rows without
  // weakening the alpha profile's normal user-layer behavior for CLI callers.
  const composedOverlays = [...overlays, ...trustedOverlays]
...
export interface RunProfileOptions {
  ...
  /** Host-owned patch files applied after every user-controlled layer. */
  trustedPatchFiles?: readonly string[]
  /** Host-constructed patches applied after every user-controlled layer. */
  trustedPatches?: readonly PatchOptions[]
  /** Whether profile and home patch files may execute. Defaults true. */
  userLayers?: boolean
  /** Validate the exact initial and every live-recomposed entry list before Loader sees it. */
  validateComposition?: (entries: readonly EntryOptions[]) => void
  /** Provide host-only services before any Loader entry mounts. */
  setupHost?: (ctx: Context) => void
  /** Resolve bare plugins from the healed DSH profile module fallback. */
  hostOwnedModuleResolution?: boolean
  /**
   * Package manifest or file URL anchoring a packaged host's module graph.
   * This takes precedence over {@link hostOwnedModuleResolution} and skips
   * filesystem fallback creation, which cannot target an Electron ASAR.
   */
  hostOwnedModuleBaseUrl?: string
  /**
   * Whether this launcher owns SIGTERM and SIGINT. Defaults true. Embedded
   * native hosts must disable this and route signals through their lifecycle
   * API so two exit controllers cannot race the same runtime disposal.
   */
  processSignalHandlers?: boolean
}
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  // Install before the first plugin mounts so every profile request honors the
  // launcher's frozen proxy environment, including values loaded from .env.
  const disposeProxy = await installProxyFromEnvironment(
    options.environment,
    (message) => { process.stderr.write(`${NAME}: ${message}\n`) },
  )
  const userLayers = options.userLayers ?? true
  const explicitModuleBase = options.hostOwnedModuleBaseUrl
  const composed = await composeProfile(
    options.profile,
    options.patchFiles,
    options.trustedPatchFiles ?? [],
    options.trustedPatches ?? [],
    userLayers,
    explicitModuleBase === undefined,
  )
  ...
  if (options.processSignalHandlers !== false) {
  ...
  const composeLive = (): PatchOptions[] => {
    const patches = structuredClone([
      ...composed.bundlePatches,
      ...(userLayers ? loadOptionalPatches(NAME, composed.profile.patchPath) ?? [] : []),
      ...(userLayers ? loadOptionalPatches(NAME, homePatchPath()) ?? [] : []),
      ...composed.overlays,
    ])
    options.validateComposition?.(composeEntries([patches]))
    return patches
  }
  const initialPatches = structuredClone(allPatches(composed))
  options.validateComposition?.(composeEntries([initialPatches]))
  // Cloned for the same insert-aliasing reason as composeLive: the boot
  // application must not mutate the objects later reloads recompose from.
  const ctx = await boot(NAME, rootConfig, initialPatches, (hostCtx) => {
    app.current = hostCtx
    options.setupHost?.(hostCtx)
  ...
  }, explicitModuleBase ?? (options.hostOwnedModuleResolution === true
    ? join(resolveDshHome(), PROFILES_DIR, '.host-module-resolution.cjs')
    : undefined))
  app.current = ctx
