...
class Hmr extends Service {
  ...
  async registerConfig(filename: string, refresh: () => Promise<void> | void): Promise<() => Promise<void>> {
    ...
    let fingerprint: string | undefined = depth > 0 ? 'missing' : undefined
    ...
    const observe = () => {
      observation = observation.then(async () => {
        ...
        try {
          ...
          next = `${String(info.dev)}:${String(info.ino)}:${String(info.size)}:${String(info.mtimeNs)}:${String(info.ctimeNs)}`
        } catch (error) {
          if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error
          next = 'missing'
        }
    ...
    // Native watchers can coalesce or drop an exact-path add while the process
    // is busy, even when the parent already existed. Poll this one target as a
    // low-cost correctness backstop; the shared fingerprint suppresses native
    // event duplicates and avoids rescanning the surrounding directory.
    const stopPolling = this.ctx.interval(observe, 250)
