...
每个 pi-ai compat 类型一张漂移门禁——以 `Record<keyof OpenAICompletionsCompat | …, CompatDisposition>` 为键——把每一个上游字段分类为 `offer` 或 `withhold`。去重后三十六个字段，开放二十四个。分界线在于私有 URL 能推出什么：凡是无法从未识别端点推断的，部署方必须能够说出口；而 pi-ai 已安装 catalog 为具名厂商设定的字段保持扣留，因为伸手去够 `openRouterRouting`、`deferredToolsMode` 或 Anthropic 的 `allowedFallbackModels` 的路由，本就是一条应当以该厂商命名、并继承其值的 catalog 路由。

`thinkingTokenBudgetField` 向 OpenAI 兼容端点开放，因为仅凭 URL 无法判断其顶层推理上限字段究竟叫 `thinking_token_budget`、`thinking_budget` 还是 `thinking_budget_tokens`；其值并集派生自 pi-ai，并由独立漂移门禁钉住。`supportsThinkingTokenBudget` 保留为 `thinking_token_budget` 的布尔别名，`thinking.budget` 则加入可接受的聊天模板占位符，使配置的逐档预算能够抵达模板驱动端点。Anthropic 的 `allowedFallbackModels` 保持扣留，因为它描述的是 catalog 自有的跨提供方回退路由，而不是端点请求字段拼写。
