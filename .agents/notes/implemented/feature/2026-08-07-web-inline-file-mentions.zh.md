...
> 范围：引导最终回复以行内代码点名主要输出文件，把这些 token 链接到结构化轮次输出，并允许 Host 打开器通过一个受限的 Office、PDF 或 HTML 文件名兜底。不在范围内：识别普通正文中的路径、绕过 Host 工作区或路径策略，以及流式或轮次中途消息里的提及。
...
**结构化输出仍是权威记录，同时提供一个受控的文档兜底。**[产物行的决定](2026-07-31-web-workspace-file-links.zh.md)否决过让可见交付行依赖模型正文；这一点不变。`producedFileMentions` 先按精确产出路径解析行内代码 token，或当 token 恰好是且仅是一条产出路径的 basename 时解析。当较旧或不完善的模型输出遗漏结构化发布事件时，一个以 DOC、DOCX、XLS、XLSX、PPT、PPTX、PDF、HTML、HTM 或 RTF 结尾的完整行内代码 token 仍可请求 Host 打开该工作区相对路径。这个兜底不会伪造交付行或成功状态：既有 Host 路径规则会解析或拒绝它，ChatView 则会显示带重试的受控错误。URL、带首尾空白的 token、控制字符、Markdown 或源码文件名，以及任意的路径形正文仍保持死文本。
...
- **对全部正文跑路径形状的正则**——会把随口提到的 `package.json` 和从未写过的示例都链接上。采用的兜底仅限一个完整的行内代码文档 token，且仍由 Host 判定文件是否存在、是否位于工作区内并执行打开。
...
提及与产物行是同一事实的两个交互面（两者都以完整路径作 `title`）；提及本身采用 markdown 样式表的锚点语言——静止为链接蓝、悬停出下划线——因为静止下划线在 code 胶囊里会压住等宽字的下伸部。提示词段落在本包加载期间保持不变，因此可跨 Turn 复用缓存。无密钥的正式 Web 组合快照钉住模型可见段落的精确文本，`apps/web/tests/produced-file-mentions.e2e.ts` 则用构造的写入轮次 seed 钉住组装后的渲染行为。终端命令间接创建或结构化输出遗漏的文件，只有在模型于已定稿的收尾消息中点名一个受控文档文件名且 Host 接受时才能打开；文件缺失会出现普通的受控重试对话框，而不是无声的静态胶囊。轮次中途叙述里的提及即使命名了本轮后来产出的文件也保持死文本。窗口前插的边界仍然安全，因为结构化解析优先，而兜底绝不绕过 Host 策略。
