...
**完成的一轮以它产出的文件收尾。** 该行是独立插件 `@deepseek-ai/dsh-client-ui-deliverables`，注册进 chat 视图在收尾消息正文与其 IconActions 之间渲染的 `conversation.chat.turnTail` 空位——ui-conversation 拥有空位与 owner 通货（节点、收尾 seq、`openFile`），插件拥有策略。`producedForClosing` 只读取成功工具的结构化事实：受支持的文件系统修改贡献经过校验的参数路径，`gongchuang_publish_files` 则只在成功后贡献结构化路径列表。原生调用配对 `tool/call` 与 `tool/result`；PTC 发布使用带位置的成功分发。工具结果正文和收尾正文绝不创建文件行。显式发布会成为该轮最终交付约定并隐藏中间构建修改；失败、格式错误、读取、删除和不受支持的调用均不贡献条目。路径按首见顺序排列且不会跨越 turn 边界。CSS 容器宽度档位选择前六条路径中的一个前缀及其匹配的 `+ N 个文件` 标签，flexbox 收缩并省略可见 basename，组件不执行 JavaScript 布局观察（[决策](../simplification/2026-09-01-css-produced-file-layout.zh.md)）。每个 chip 通过现有 Host 安全 `openFile` 路径打开精确完整路径；cordis.yml 中的一行即可把该交互面组合进来或去掉，未注册的空位什么也不渲染。

**路径链接读得出是链接。** 静止状态下就带下划线，而不只在悬停时。这是本次改动中更小的那一半，却是修复中更大的那一半。

**只通过核验探测交付的文件也使用同一行。** 成功的 `gongchuang_artifact_probe` 与发布器的 `paths` 一样贡献结构化 `artifactPath`。失败探测和正文中的文件名都不会生成文件按钮，原生结果与代码模式结果使用同一生产者规则。

**代码模式位置归会话与持久化所有者管理。** 新分发通过公开 `Session.snapshotEvents()` API 捕获所属轮次和步骤；旧 `.events` 属性不是受支持的 Session API。对同时缺少两个坐标的历史记录，持久化归一化器只能从当前步骤中唯一的前置根 `tool/call` 恢复坐标。显式坐标保持不变，缺失或有歧义的根编号仍不定位。检查、恢复、跳转和采用共用该读取视图，不重写原始日志。客户端事件匹配不会重新扫描邻近历史，也不会根据助手正文猜测。
...
现有的每一处文件交互都同时改变了：write、edit、read 与通用单文件卡片都汇到 `openFile`，因此链接修复与浏览器优先策略无需逐行改动。正式报告工具可以发布已经验证的 DOCX 或 PDF，而无需假装该调用属于文件系统编辑。组装层 Web 测试覆盖显式发布、单行 CSS 溢出和单次点击的 Host 交接，且不会启动原生应用。产出的 `file://` 文档无法 `fetch` 同级文件（但 `<script src>`、`<img>` 和 CSS `@import` 可用），这是 HTTP 提供曾有、而此处没有的能力。远程客户端保留 chip，但省略文件夹操作；每个 chip 的 `title` 仍保留完整路径。Markdown 仍由平台的 `.md` 应用打开；产品内渲染属于另一项工作。

冷启动浏览器回放还覆盖旧代码模式发布、仅探测交付、两种完整路径点击和刷新。另一项对所报告本地记录的只读回放确认最后一次成功发布取得真实轮次归属，且引用文件仍存在。两项测试都不宣称已替换安装中的桌面二进制。
