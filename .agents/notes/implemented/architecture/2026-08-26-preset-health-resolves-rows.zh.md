...
打包后的桌面客户端暴露了相反方向的故障。可变的 profile 与 preset 位于应用归档之外，因此 `ctx.baseUrl` 指向用户数据目录，而随客户端交付的插件包位于不可变的宿主模块树中。发现与挂载都把 profile 当成包解析基准，产品 preset 的每一行都被误判为无法解析，最终让启动、恢复会话、切换模型和切换推理强度一起失败，尽管这些包实际都在 `app.asar` 内。
...
**宿主模块基址才是包解析锚点。** `discoverPresets(roots, harnessBase)` 与 `scanRoot(root, harnessBase)` 仍要求显式基址。`presetModuleBase(ctx)` 优先读取宿主发布的 `ctx.get('hostModuleBaseUrl')`，不存在时才为源码与嵌入式消费者回退到 `ctx.baseUrl`。发现注册表与挂载覆写都调用这一处 helper。打包应用不得从可变 profile 目录推断插件位置，健康检查也绝不能按一处基址解析、再按另一处基址挂载。
...
**让 base 可选，没有它就跳过检查。** 否决。它的失败模式正是要修的这个 bug，而且不带任何信号：无法组装的 preset 顶着健康卡片。受支持的宿主会在 preset 服务启动前发布 `hostModuleBaseUrl` 或 `baseUrl`，因此这里抛出是在断言宿主契约已损坏，而不是建立一个没有包解析锚点的运行时兜底。
