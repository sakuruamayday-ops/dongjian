...
cookie 是签名且绑定 authority 的 bearer。在凭据持久化模式中，确定性名称与签名 payload 都包含规范化 hostname 和 port，因此同一 Harness home 可以在不同 Web port 运行而不发生 cookie 冲突。payload 在绝对有效期内携带安全整数形式的签发与过期时间；`cookieMaxAgeDays` 默认为 30。cookie 是 host-only、`Path=/`、`HttpOnly`、`SameSite=Strict`。随附服务器使用 loopback HTTP，因此不设置 `Secure`。这里没有 logout 操作或反向代理专用处理。
...
凭据支持的密钥仍是 Web 与 CLI 组合的默认行为。桌面组合可以改用[进程级浏览器会话决策](2026-08-29-desktop-process-local-browser-session.zh.md)；该模式使用固定名称 `dsh-auth-process-local`，而签名 payload 仍绑定精确 authority。保留 Chromium 持久分区的桌面 Host 会在首次 loopback 导航前仅删除 Connection 持有的 `dsh-auth-*` cookie，过时的按端口名称不会累积成过大的请求 header，账号 cookie 保持不变。该例外只部分取代本说明中的全局持久化与按 authority 派生名称选择。启动令牌交换、签名 cookie 格式、请求信任检查与完整 Host API 认证仍由本说明约束。
