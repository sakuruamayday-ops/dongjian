...
The credential-backed secret remains the default for Web and CLI compositions. A desktop composition may instead select the [process-local browser session decision](2026-08-29-desktop-process-local-browser-session.md); that mode uses the fixed `dsh-auth-process-local` name while its signed payload still binds the exact authority. A desktop host that retains a persistent Chromium partition removes only Connection-owned `dsh-auth-*` cookies before its first loopback navigation, so obsolete per-port names cannot accumulate into an oversized request header and account cookies remain intact. The exception partially supersedes only the universal persistence and authority-derived-name choices in this note. The launch-token exchange, signed-cookie format, request trust checks, and complete Host API authentication remain governed here.
