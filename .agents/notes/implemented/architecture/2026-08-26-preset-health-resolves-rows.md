...
The packaged desktop exposed the inverse failure. Its mutable profile and presets live outside the application archive, so `ctx.baseUrl` points at user data while the shipped plugin packages live under the immutable host module tree. Discovery and mount both treated the profile as the package anchor, marked every row in the product preset unresolved, and made startup, resume, model switching, and reasoning switching fail together even though the packages were present in `app.asar`.
...
**The host module base is the package anchor.** `discoverPresets(roots, harnessBase)` and `scanRoot(root, harnessBase)` still require an explicit base. `presetModuleBase(ctx)` selects the host-published `ctx.get('hostModuleBaseUrl')` when present and otherwise falls back to `ctx.baseUrl` for source and embedded consumers. Both the discovery registry and the mount override call this one helper. A packaged application must not infer package location from the mutable profile directory, and health must never resolve from a different base than mount imports from.
...
**Keep the base optional and skip the check without one.** Rejected. Its failure mode is precisely the bug being fixed, delivered with no signal: healthy cards for presets that cannot compose. A supported host publishes either `hostModuleBaseUrl` or `baseUrl` before the preset service starts, so the throw asserts a broken host contract rather than creating a runtime fallback with no package anchor.
