...
```ts cordis-catalog
/**
 * Host-only lifecycle boundary used before moving a Session persistence artifact.
 * The browser Remote does not expose this capability.
 * @param sessionId - ordinary Session identity to quiesce.
 */
disposeSession(sessionId: SessionId): Promise<void>

/**
 * Release a Host deletion barrier after its recoverable transaction rolls back.
...
/**
 * Resolve one Session-authorized durable file for a same-host desktop integration.
 * The browser Remote cannot call this method.
 * @param sessionId - Session whose durable log owns the reference.
 * @param attachment - Complete file reference shown by the client.
 * @returns provider-owned absolute path.
 */
fileAttachmentPath(sessionId: SessionId, attachment: FileAttachmentRef): Promise<string>
...
Types: [FileAttachmentRef](attachment.zh.md) · [SessionId](core.zh.md) · [SessionInspection](persistence.zh.md) · [SessionSearchRequest](session-query.zh.md)
...
<a id="api-sessionfork-preparing--parallel"></a>

#### `api-session/fork-preparing` — parallel

Prepare referenced workspace files before a cross-workspace fork creates its child.

```ts cordis-catalog
/**
 * Prepare referenced workspace files before a cross-workspace fork creates its child.
 * @mode parallel
 * @param input - Immutable inherited events and source/target working directories.
 */
'api-session/fork-preparing'(input: { readonly events: readonly SessionEvent[] readonly sourceCwd: string readonly targetCwd: string }): Promise<void>
```
