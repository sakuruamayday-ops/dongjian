...
[共创产品](../../packages/client/ui-gongchuang-product/README.zh.md)通过可选的 Chat 选区和 Conversation prompt 准备回调，拥有带编号的选区草稿及其消息投影。Chat 渲染共享的只读摘要，而产品的 dock 入口编辑其 Session 范围草稿。队列摘要会先使用完整显示文本再截断；Session model 会把另行准备的显示行标记为不能由纯文本队列编辑器修改。
