...
<a id="ctxgongchuangaccount--gongchuangaccountservice"></a>

### `ctx.gongchuangAccount` — `GongchuangAccountService`

Host-owned single-device account session backed by the operating-system credential store. Renderer snapshots never contain the password, device id, or access token.

```ts cordis-catalog
/**
 * Read account presentation state without network activity.
 * @returns The current redacted account state.
 */
@Remote('snapshot') snapshot(): GongchuangAccountSnapshot

/**
 * Read the user-authored part of the product-owned global AGENTS.md.
 * @returns Personalization text without its fixed professional-boundary wrapper.
 */
@Remote('personalization') async personalization(): Promise<GongchuangPersonalizationSnapshot>

/**
 * Atomically replace the model-visible local preferences for future conversations.
 * @param request - Complete user-authored preference text.
 * @returns The persisted redacted snapshot.
 */
@Remote('savePersonalization') async savePersonalization( request: GongchuangPersonalizationSaveRequest, ): Promise<GongchuangPersonalizationSnapshot>

/**
 * Authenticate the supplied account and make this machine the active device.
 * @param request - Username, password source, and saved-password preference.
 * @returns Redacted connected state after credentials are committed.
 */
@Remote('login') login(request: GongchuangAccountLoginRequest): Promise<GongchuangAccountSnapshot>

/**
 * Verify the saved session or perform permitted saved-password auto-login.
 * @returns Current connected, offline, superseded, or disconnected state.
 */
@Remote('refresh') refresh(): Promise<GongchuangAccountSnapshot>

/**
 * Remove the active token and optionally forget all saved login material.
 * @param request - Whether username and password are also removed from the credential store.
 * @returns Redacted disconnected state after local credential updates finish.
 */
@Remote('disconnect') disconnect(request: GongchuangAccountDisconnectRequest): Promise<GongchuangAccountSnapshot>
```

Types: [GongchuangAccountDisconnectRequest](../../packages/product/gongchuang-account/README.zh.md) · [GongchuangAccountLoginRequest](../../packages/product/gongchuang-account/README.zh.md) · [GongchuangAccountSnapshot](../../packages/product/gongchuang-account/README.zh.md) · [GongchuangPersonalizationSaveRequest](../../packages/product/gongchuang-account/README.zh.md) · [GongchuangPersonalizationSnapshot](../../packages/product/gongchuang-account/README.zh.md)

Source: [`packages/product/gongchuang-account/src/index.ts`](../../packages/product/gongchuang-account/src/index.ts)
