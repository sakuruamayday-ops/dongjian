...
The [Gongchuang product](../../packages/client/ui-gongchuang-product/README.md) owns numbered selection drafts and their message projection through optional Chat selection and Conversation prompt-preparation callbacks. Chat renders the shared read-only summary, while the product's dock entry edits its session-scoped draft. Queue summaries consume complete display text before truncation; the Session model marks separately prepared display rows non-editable by the plain-text queue editor.
