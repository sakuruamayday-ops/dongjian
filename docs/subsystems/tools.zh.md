...
<a id="ctxgongchuangpolicy--gongchuangpolicyservice"></a>

### `ctx.gongchuangPolicy` — `GongchuangPolicyService`

Capability-limited receipt ledger. Trusted producer plugins receive this service through declared Cordis injection and claim a receipt against the exact live tool execution. The post-execute gate commits the claim only when the tool succeeded and no later listener blocked its result.
 ...
 */
claim( exec: ToolExecution, receiptId: string, subjectText?: string, artifactBinding?: ArtifactBinding, ledger?: ProfessionalLedger, ): void
 ...
 */
attestEvidence( exec: ToolExecution, evidence: readonly ProfessionalEvidence[], sourceText?: string, ): readonly ProfessionalEvidence[]
 ...
 */
recordEvidence( exec: ToolExecution, result: ToolExecutionResult, ): Readonly<EvidenceReceipt> | undefined
...
Types: [ArtifactBinding](../../packages/guard/gongchuang-policy-gate/README.zh.md) · [EvidenceReceipt](../../packages/guard/gongchuang-policy-gate/README.zh.md) · [ProfessionalEvidence](../../packages/guard/gongchuang-policy-gate/README.zh.md)

Source: [`packages/guard/gongchuang-policy-gate/src/index.ts`](../../packages/guard/gongchuang-policy-gate/src/index.ts)
...
<a id="gongchuang-connectors-events"></a>

### `gongchuang-connectors/*` events

<a id="gongchuang-connectorschanged--emit"></a>

#### `gongchuang-connectors/changed` — emit

The redacted connector snapshot changed. Consumers must re-read the snapshot rather than retaining a stale ready state.
...
Source: [`packages/mcp/gongchuang-connectors/src/types.ts`](../../packages/mcp/gongchuang-connectors/src/types.ts)

<a id="mcp-events"></a>

### `mcp/*` events

<a id="mcpconnection-state--emit"></a>

#### `mcp/connection-state` — emit

Reports a credential-free lifecycle snapshot for one supervised MCP connection.

```ts cordis-catalog
/**
 * Reports a credential-free lifecycle snapshot for one supervised MCP connection.
 * @param state Current transport generation, retry count, and user-safe message.
 * @mode emit
 */
'mcp/connection-state'(state: McpConnectionState): void
```
