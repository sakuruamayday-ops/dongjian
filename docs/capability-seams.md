...
| `ctx.deepseekFilesControl` | `core` | [`llm-deepseek`](../packages/llm/llm-deepseek) | - | `gongchuang-model-connections` | - | Owns authenticated listing and release of files uploaded through the official DeepSeek adapter; the product connection service exposes the bounded user controls. |
| `ctx.gongchuangAccount` | `core` | `gongchuang-account` | - | `gongchuang-connectors` | - | Owns server authentication, credential-backed session recovery, version compatibility, and redacted personalization snapshots. |
| `ctx.gongchuangGraphMemory` | `core` | `gongchuang-graph-memory` | - | - | - | Owns the local SQLite graph and selects exactly one personal or enterprise partition for each Session. |
| `ctx.gongchuangLocalAutomation` | `core` | `gongchuang-local-automation` | - | - | - | Owns persistent schedules, atomic run claims, receipts, and the stable result Session bound to each task. |
| `ctx.gongchuangModelConnections` | `core` | `gongchuang-model-connections` | - | - | - | Owns credential-backed endpoint verification and atomically publishes the model directory shared by adapters and client selectors. |
| `ctx.gongchuangPolicy` | `core` | `gongchuang-policy-gate` | - | `gongchuang-signed-skill-runtime` | - | Verifies the signed policy envelope and mediates product tool calls, professional activation, receipts, and deterministic delivery checks. |
| `ctx.gongchuangSignedSkillRuntime` | `core` | `gongchuang-signed-skill-runtime` | - | - | - | Runs only manifest-declared operations from the verified skill bundle through the bundled interpreter and bounded workspace paths. |
| `ctx.gongchuangSkillMarketplace` | `core` | `gongchuang-skill-marketplace` | - | - | - | Owns external marketplace discovery, isolated community-skill installation, and the client-facing installed catalog. |
| `ctx.gongchuangSkillRuntimeBinding` | `seam` | `gongchuang-signed-skill-runtime` | - | `gongchuang-signed-skill-runtime`, `gongchuang-connectors` | - | The desktop Host supplies the exact verified skill version, manifest, and bundled Python runtime consumed by signed operations and connector launchers. |
