...
flowchart LR
  ...
  svc_deepseekFilesControl["ctx.deepseekFilesControl<br/>DeepSeek remote-file lifecycle control"]
  pkg_gongchuang_model_connections["gongchuang-model-connections"]
  pkg_gongchuang_account["gongchuang-account"]
  svc_gongchuangAccount["ctx.gongchuangAccount<br/>Gongchuang account and personalization service"]
  pkg_gongchuang_connectors["gongchuang-connectors"]
  pkg_gongchuang_graph_memory["gongchuang-graph-memory"]
  svc_gongchuangGraphMemory["ctx.gongchuangGraphMemory<br/>Gongchuang partitioned local graph memory"]
  pkg_gongchuang_local_automation["gongchuang-local-automation"]
  svc_gongchuangLocalAutomation["ctx.gongchuangLocalAutomation<br/>Gongchuang local automation registry"]
  svc_gongchuangModelConnections["ctx.gongchuangModelConnections<br/>Gongchuang verified model connections"]
  pkg_gongchuang_policy_gate["gongchuang-policy-gate"]
  svc_gongchuangPolicy["ctx.gongchuangPolicy<br/>Gongchuang signed professional policy"]
  pkg_gongchuang_signed_skill_runtime["gongchuang-signed-skill-runtime"]
  svc_gongchuangSignedSkillRuntime["ctx.gongchuangSignedSkillRuntime<br/>Gongchuang signed skill operation runtime"]
  pkg_gongchuang_skill_marketplace["gongchuang-skill-marketplace"]
  svc_gongchuangSkillMarketplace["ctx.gongchuangSkillMarketplace<br/>Gongchuang community skill marketplace"]
  svc_gongchuangSkillRuntimeBinding["ctx.gongchuangSkillRuntimeBinding<br/>Verified Gongchuang skill bundle binding"]
  ...
  pkg_goal --> svc_goals
  pkg_gongchuang_account --> svc_gongchuangAccount
  pkg_gongchuang_graph_memory --> svc_gongchuangGraphMemory
  pkg_gongchuang_local_automation --> svc_gongchuangLocalAutomation
  pkg_gongchuang_model_connections --> svc_gongchuangModelConnections
  pkg_gongchuang_policy_gate --> svc_gongchuangPolicy
  pkg_gongchuang_signed_skill_runtime --> svc_gongchuangSignedSkillRuntime
  pkg_gongchuang_signed_skill_runtime --> svc_gongchuangSkillRuntimeBinding
  pkg_gongchuang_skill_marketplace --> svc_gongchuangSkillMarketplace
  ...
  pkg_llm --> svc_llm
  pkg_llm_deepseek --> svc_deepseekFilesControl
  ...
  svc_deepseekFilesControl --> pkg_gongchuang_model_connections
  ...
  svc_fs --> pkg_tool_fs
  svc_gongchuangAccount --> pkg_gongchuang_connectors
  svc_gongchuangPolicy --> pkg_gongchuang_signed_skill_runtime
  svc_gongchuangSkillRuntimeBinding --> pkg_gongchuang_connectors
  svc_gongchuangSkillRuntimeBinding --> pkg_gongchuang_signed_skill_runtime
...
| `ctx.deepseekFilesControl` | `core` | [`llm-deepseek`](../packages/llm/llm-deepseek) | - | `gongchuang-model-connections` | - | 负责对通过官方 DeepSeek 适配器上传的文件执行已认证的列举与释放；产品连接服务提供范围受限的用户控制。 |
| `ctx.gongchuangAccount` | `core` | `gongchuang-account` | - | `gongchuang-connectors` | - | 负责服务器认证、基于凭据的会话恢复、版本兼容性与脱敏个性化快照。 |
| `ctx.gongchuangGraphMemory` | `core` | `gongchuang-graph-memory` | - | - | - | 负责本地 SQLite 图，并为每个 Session 精确选择一个个人或企业分区。 |
| `ctx.gongchuangLocalAutomation` | `core` | `gongchuang-local-automation` | - | - | - | 负责持久计划、原子运行认领、回执，以及每项任务绑定的稳定结果 Session。 |
| `ctx.gongchuangModelConnections` | `core` | `gongchuang-model-connections` | - | - | - | 负责基于凭据的端点验证，并原子发布由适配器和客户端选择器共用的模型目录。 |
| `ctx.gongchuangPolicy` | `core` | `gongchuang-policy-gate` | - | `gongchuang-signed-skill-runtime` | - | 验证签名策略封套，并协调产品工具调用、专业激活、回执和确定性交付检查。 |
| `ctx.gongchuangSignedSkillRuntime` | `core` | `gongchuang-signed-skill-runtime` | - | - | - | 只通过随包解释器与受限工作区路径，执行已验签技能包清单声明的操作。 |
| `ctx.gongchuangSkillMarketplace` | `core` | `gongchuang-skill-marketplace` | - | - | - | 负责外部市场发现、隔离安装社区技能，以及面向客户端的已安装目录。 |
| `ctx.gongchuangSkillRuntimeBinding` | `seam` | `gongchuang-signed-skill-runtime` | - | `gongchuang-signed-skill-runtime`, `gongchuang-connectors` | - | 桌面 Host 提供准确的已验签技能版本、清单和随包 Python 运行时，供签名操作与连接器启动器消费。 |
