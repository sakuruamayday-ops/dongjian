...
export interface Config {
  ...
  default: string
  /**
   * Optional deployment-owned roster allowlist. When present, discovery and
   * every resolving/authoring path expose only these ids, even when a scanned
   * root contains additional presets. It must be non-empty and include
   * {@link default}.
   */
  allowedIds?: string[]
...
来源：[`packages/llm/llm-deepseek/src/index.ts:135`](../packages/llm/llm-deepseek/src/index.ts)
...
export interface PiAiProviderProfile {
  /** Installed provider whose per-model protocols and endpoints supply defaults for this named route. */
   ...
   * Null clears a composition-layer override and restores the catalog default.
   */
  api?: string | null
  /** Endpoint for this route's models; null clears an inherited override. */
  baseURL?: string | null
  /**
...
export interface PiAiCompatProfile {
  ...
  /** Exact top-level field used to cap reasoning tokens; `openai-completions`. */
...
/** The top-level field spellings pi-ai accepts for a reasoning token budget. */
...
export interface StdioConfig {
  ...
  env: Record<string, string>
  /** Environment variable to credential-reference bindings, resolved for each connection generation. */
  ...
  /** Optional raw MCP tool call that must succeed before this generation is considered ready. */
...
export interface StreamableHttpConfig {
  ...
  headers: Record<string, string>
  /** Header to credential-reference bindings, resolved for each connection generation. */
  headerCredentials?: Record<string, HeaderCredentialBinding>
  /** Per-tool-call timeout in milliseconds. */
  toolCallTimeoutMs: number
  /** Fail plugin activation when the initial connection or tool synchronization fails. */
  failOnStartupError: boolean
...
/** Product-owned, credential-free readiness check executed before tools are registered. */
export interface ReadinessProbeConfig {
  /** Raw server tool name, before the public `mcp__<server>__` prefix is added. */
  toolName: string
  /** Fixed non-secret arguments supplied by trusted product configuration. */
  arguments: Record<string, unknown>
  /** Probe timeout in milliseconds. */
  timeoutMs: number
}

/** One credential-backed HTTP header. The secret itself never enters plugin config. */
export interface HeaderCredentialBinding {
  /** Credential reference resolved through `ctx.credentials`. */
  ref: string
  /** Optional non-secret prefix, for example `Bearer `. */
  prefix?: string
}
```

来源：[`packages/mcp/mcp-client/src/index.ts:130`](../packages/mcp/mcp-client/src/index.ts)
...
export interface Config {
  ...
  exclude?: string[]
  /** Deny this many consecutive identical attempts before dispatch; unset keeps advisory-only behavior. */
  blockAt?: number
  /**
...
export interface Config {
  ...
  /** Optional shared registry whose `disabledSkillNames` hides bundled skills without mutating their files. */
  disabledSkillRegistryPath?: string
}
```
...
```ts config-catalog
/** Account-server identity and timeout bound by the desktop composition. */
export interface Config {
  /** Root HTTPS URL of the 共创 account portal. */
  portalUrl: string
  /** Exact canonical semantic version sent to the account server. */
  clientVersion: string
  /** Desktop platform identifier included in device admission requests. */
  platform: 'macos' | 'windows'
  /** Abort timeout applied to each account-server request. */
  requestTimeoutMs: number
}
```

来源：[`packages/product/gongchuang-account/src/index.ts:81`](../packages/product/gongchuang-account/src/index.ts)

<a id="gongchuangclient-policy-gate"></a>
...
来源：[`packages/guard/gongchuang-policy-gate/src/index.ts:80`](../packages/guard/gongchuang-policy-gate/src/index.ts)

<a id="gongchuangconnectors"></a>

## `@gongchuang/connectors`

需要：`settings` · `credentials` · `tools` · `web` · `gongchuangAccount`
...
来源：[`packages/mcp/gongchuang-connectors/src/index.ts:69`](../packages/mcp/gongchuang-connectors/src/index.ts)

<a id="gongchuangcredentials-keychain"></a>
...
来源：[`packages/credentials/gongchuang-keychain/src/index.ts:23`](../packages/credentials/gongchuang-keychain/src/index.ts)

<a id="gongchuanggraph-memory"></a>

## `@gongchuang/graph-memory`

需要：`tools` · `llm` · `systemPrompt` · `workspaceRegistry`
...
来源：[`packages/product/gongchuang-graph-memory/src/index.ts:38`](../packages/product/gongchuang-graph-memory/src/index.ts)
...
- `@gongchuang/client-ui`（[`packages/client/ui-gongchuang-product/src/index.ts`](../packages/client/ui-gongchuang-product/src/index.ts)）
- `@gongchuang/local-automation` — 需要 `tools`（[`packages/product/gongchuang-local-automation/src/index.ts`](../packages/product/gongchuang-local-automation/src/index.ts)）
- `@gongchuang/model-connections` — 需要 `credentials` · `settings` · `llm`（[`packages/product/gongchuang-model-connections/src/index.ts`](../packages/product/gongchuang-model-connections/src/index.ts)）
- `@gongchuang/signed-skill-runtime` — 需要 `tools` · `subprocess` · `sandbox` · `gongchuangPolicy` · `gongchuangSkillRuntimeBinding`（[`packages/product/gongchuang-signed-skill-runtime/src/index.ts`](../packages/product/gongchuang-signed-skill-runtime/src/index.ts)）
- `@gongchuang/skill-marketplace` — 需要 `skills`（[`packages/skill/gongchuang-skill-marketplace/src/index.ts`](../packages/skill/gongchuang-skill-marketplace/src/index.ts)）
...
- `@gongchuang/user-errors`（[`packages/product/gongchuang-user-errors/src/index.ts`](../packages/product/gongchuang-user-errors/src/index.ts)）
