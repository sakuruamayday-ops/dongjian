...
用户从客户端输入框添加 Word、Excel、演示文稿、WPS 兼容文件、ODF、RTF、CSV／TSV、PDF 或 TXT 后，资料必须先复制到当前具体企业空间的 `导入资料` 目录，再读取副本；不得把源文件绝对路径、文件字节或账户凭据送入对话。激活本技能后，优先调用已验签操作 `project-application-assistant.extract-workspace-document`，参数为 `{"document":"导入资料/文件名"}`，只使用当前企业空间内的相对路径，无需查找安装目录、运行清单或解释器。提取器先按文件真实内容识别 OOXML、OLE、ODF、RTF、PDF 或文本结构，再选择只读解析器；扩展名只用于记录和受控入口白名单，不得据此猜测内容。
...
用户要求具体项目的前期评估报告或可行性分析报告时，统一由 `project-feasibility` 读取双报告合同，再调用一个项目领域技能切换核心评估对象。前期评估用于首次沟通，优先结论、现行核心条件、企业已有数据、差距和补强；可行性分析用于合作后的实施决策，逐项拆解现行条件或已公布评分表、证据、动作、完成标准和年度节点。两类报告均建议形成国内产品技术水平评价咨询报告和国际产品技术水平评价咨询报告；每份报告默认只交付可编辑 Word，用户明确要求 PDF 时才从同一正文生成 PDF，不设置文档编辑限制。企业分析报告 A、B、C 模式以及尖兵领雁、市级重大和科技计划类既有分析逻辑不因此改变。
