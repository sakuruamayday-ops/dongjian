...
def _set_public_document_metadata(document: Document, fixture: dict[str, Any], report_type: str) -> None:
    ...
    properties.author = str(fixture.get("advisor") or "")
    properties.last_modified_by = ""
    properties.keywords = "政府项目申报,前期评估"
...
def complete_report(
    *,
    ...
    import sys
    scripts = Path(__file__).resolve().parents[2] / "_runtime/gongchuang-branding/scripts"
    sys.path.insert(0, str(scripts))
    from neutral_document import neutralize_office
    neutralize_office(output_path)
