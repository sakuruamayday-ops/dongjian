...
客户端生成正式 DOCX 时，主技能声明的专用模板或生成器始终优先。主技能
...
客户端生成正式 DOCX、XLSX 或 XLSM 时，正文与版式完成后、首次
artifact 校验前，调用已验签操作 `evidence-ledger.finalize-document`，
参数为 `{"artifact":"工作区内最终文件路径"}`。该操作清理默认模板的出版者
标识并检查文件，保留正文和用户图片，不添加客户端页眉或水印；执行后不得
再修改文件。PPTX 使用对应生成技能，不把本 Office 操作套用于演示文稿。
