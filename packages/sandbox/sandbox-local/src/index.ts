...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private seatbeltExec(): string {
    // The product deliberately narrows PATH for signed skill subprocesses.
    // Resolve the OS-owned Seatbelt launcher without relying on caller PATH so
    // a confined operation cannot fail merely because only the embedded
    // Python directory is visible (or select a same-named executable there).
    return this.internals.seatbeltExec ?? '/usr/bin/sandbox-exec'
  }
