 ...
 */
type ManagementField = 'archivedWorkspaceIds' | 'deletedWorkspaceIds' | 'archivedSessionIds' | 'deletedSessionIds'
type StoredDomainState = Omit<WorkspaceDomainState, ManagementField>
  & Partial<Pick<WorkspaceDomainState, ManagementField>>
type StoredWorkspaceRecord = Omit<WorkspaceRecord, 'pinnedSessionIds'>
  & Partial<Pick<WorkspaceRecord, 'pinnedSessionIds'>>

const emptyManagementState = {
...
function storedPool(
  entries: Array<[string, StoredWorkspaceRecord]>,
...
describe('WorkspaceRegistry lifecycle and bootstrap', () => {
  it('stays pending without sessionPersistence and never opens or marks the domain', async () => {
    ...
    expect(storedState(pool)).toEqual({ initialized: true, workspaceIds: [], ...emptyManagementState })
  })

  it('loads records written before pinned sessions existed and persists the field on mutation', async () => {
    const dir = await makeDir('legacy-pins')
    const id = WorkspaceId('legacy-pins')
    const legacy = record(dir, ['s1'])
    const { pinnedSessionIds: _pinnedSessionIds, ...stored } = legacy
    const pool = storedPool([[id, stored]], {
      initialized: true,
      workspaceIds: [id],
      archivedWorkspaceIds: [],
      deletedWorkspaceIds: [],
    ...
    const result = await harness({ pool, sessions: [header('s1', dir, 1)] })
    const workspace = result.registry.get(id)
    expect(workspace?.pinnedSessionIds).toEqual([])

    await workspace?.setSessionPinned(SessionId('s1'), true)
    expect(storedRecord(pool, id).pinnedSessionIds).toEqual(['s1'])
  })

  it('round-trips the sanitized V0.3.3 single-file shape without losing management state', async () => {
    const root = await makeDir('sanitized-v033-json')
    const activePath = await makeDir('placeholder-active')
    const archivedPath = await makeDir('placeholder-archived')
    const deletedPath = await makeDir('placeholder-deleted')
    const activeId = WorkspaceId('00000000-0000-4000-8000-000000000001')
    const archivedId = WorkspaceId('00000000-0000-4000-8000-000000000002')
    const deletedId = WorkspaceId('00000000-0000-4000-8000-000000000003')
    const activeSession = SessionId('placeholder-active-session')
    const archivedSession = SessionId('placeholder-archived-session')
    const deletedSession = SessionId('placeholder-deleted-session')
    const fixture = {
      unit: { name: 'workspace', version: DOMAIN_VERSION },
      global: {
        initialized: true,
        workspaceIds: [activeId, archivedId, deletedId],
        archivedWorkspaceIds: [archivedId],
        deletedWorkspaceIds: [deletedId],
        archivedSessionIds: [archivedSession],
        deletedSessionIds: [deletedSession],
      },
      tables: {
        workspaces: {
          [activeId]: record(activePath, [activeSession]),
          [archivedId]: record(archivedPath, [archivedSession]),
          [deletedId]: record(deletedPath, [deletedSession]),
        },
      },
    }
    await writeFile(join(root, 'workspace.json'), `${JSON.stringify(fixture, null, 2)}\n`, 'utf8')

    const result = await harness({
      backend: new JsonStorageBackend(root),
      sessions: [
        header(activeSession, activePath, 1),
        header(archivedSession, archivedPath, 2),
        header(deletedSession, deletedPath, 3),
      ],
    })
    expect(result.registry.archivedWorkspaceIds).toEqual([archivedId])
    expect(result.registry.deletedWorkspaceIds).toEqual([deletedId])
    expect(result.registry.archivedSessionIds).toEqual([archivedSession])
    expect(result.registry.deletedSessionIds).toEqual([deletedSession])

    await result.registry.get(activeId)!.setTitle('placeholder-renamed')
    const written = JSON.parse(await readFile(join(root, 'workspace.json'), 'utf8')) as typeof fixture
    expect(written.global).toEqual(fixture.global)
    expect(Object.keys(written.tables.workspaces)).toEqual([activeId, archivedId, deletedId])
    expect(written.tables.workspaces[activeId]!.title).toBe('placeholder-renamed')
  })
...
describe('WorkspaceRegistry create and lookup', () => {
  ...
  it('deletes only the registration and leaves its directory and session headers untouched', async () => {
    ...
    await workspace.setSessionPinned(SessionId('kept-session'), true)
    ...
    expect(result.registry.list()).toEqual([])
    expect(storedState(result.pool)).toEqual({ initialized: true, workspaceIds: [], ...emptyManagementState })
...
describe('Workspace session ordering', () => {
  ...
  it('pins at the front, unpins in place, and restores the committed state after restart', async () => {
    const dir = await makeDir('pin-session')
    const result = await harness()
    result.setSessions([header('s1', dir, 1), header('s2', dir, 2), header('s3', dir, 3)])
    const workspace = await result.registry.create(dir)
    await workspace.attachSession(SessionId('s1'))
    ...
    await workspace.setSessionPinned(SessionId('s1'), true)
    expect(workspace.sessionIds).toEqual(['s1', 's3', 's2'])
    expect(workspace.pinnedSessionIds).toEqual(['s1'])
    await workspace.setSessionPinned(SessionId('s2'), true)
    expect(workspace.sessionIds).toEqual(['s2', 's1', 's3'])
    expect(workspace.pinnedSessionIds).toEqual(['s2', 's1'])
    const written = result.changes.length
    await workspace.setSessionPinned(SessionId('s2'), true)
    expect(result.changes).toHaveLength(written)

    await workspace.setSessionPinned(SessionId('s2'), false)
    expect(workspace.sessionIds).toEqual(['s2', 's1', 's3'])
    expect(workspace.pinnedSessionIds).toEqual(['s1'])
    expect(storedRecord(result.pool, workspace.id).pinnedSessionIds).toEqual(['s1'])

    const restarted = await harness({
      pool: result.pool,
      sessions: [header('s1', dir, 1), header('s2', dir, 2), header('s3', dir, 3)],
    })
    const restored = restarted.registry.get(workspace.id)
    expect(restored?.sessionIds).toEqual(['s2', 's1', 's3'])
    expect(restored?.pinnedSessionIds).toEqual(['s1'])
  })

  it('rejects unknown pin targets and keeps the published snapshot unchanged after a write failure', async () => {
    const dir = await makeDir('pin-failure')
    const result = await harness()
    result.setSessions([header('s1', dir, 1)])
    const workspace = await result.registry.create(dir)
    await workspace.attachSession(SessionId('s1'))

    await expect(workspace.setSessionPinned(SessionId('ghost'), true))
      .rejects.toBeInstanceOf(WorkspaceMoveInvalidError)
    result.pool.failNextWrites = 1
    await expect(workspace.setSessionPinned(SessionId('s1'), true)).rejects.toThrow('injected write failure')
    expect(workspace.sessionIds).toEqual(['s1'])
    expect(workspace.pinnedSessionIds).toEqual([])
    expect(storedRecord(result.pool, workspace.id).pinnedSessionIds).toEqual([])
  })

  it('removes detached sessions from the pinned set', async () => {
    const dir = await makeDir('pin-detach')
    const result = await harness()
    result.setSessions([header('s1', dir, 1)])
    const workspace = await result.registry.create(dir)
    await workspace.attachSession(SessionId('s1'))
    await workspace.setSessionPinned(SessionId('s1'), true)
    await workspace.detachSession(SessionId('s1'))
    expect(workspace.sessionIds).toEqual([])
    expect(workspace.pinnedSessionIds).toEqual([])
    expect(storedRecord(result.pool, workspace.id)).toMatchObject({ sessionIds: [], pinnedSessionIds: [] })
  })
...
describe('header-validated membership projection', () => {
  ...
  it('recovers only an explicitly marked interrupted create or delete', async () => {
    ...
    expect(storedState(interruptedCreate)).toEqual({ initialized: true, workspaceIds: [], ...emptyManagementState })
    ...
    expect(storedState(interruptedDelete)).toEqual({ initialized: true, workspaceIds: [], ...emptyManagementState })
...
describe('registry-global session archive', () => {
  ...
  it('accepts unaccounted and live sessions but rejects unknown ids without writing', async () => {
    ...
    await expect(result.registry.archiveSession(SessionId('ghost')))
      .rejects.toThrow(/cannot manage session 'ghost'/)
...
describe('recoverable workspace and session management', () => {
  it('archives and restores a workspace registration without changing its entity or directory', async () => {
    const dir = await makeDir('managed-workspace')
    const result = await harness()
    const workspace = await result.registry.create(dir)
  ...
  it('soft-deletes and restores a workspace registration without changing its entity or directory', async () => {
    const dir = await makeDir('deleted-workspace')
    const result = await harness()
    const workspace = await result.registry.create(dir)

    await result.registry.archiveWorkspace(workspace.id)
    await result.registry.deleteWorkspace(workspace.id)
    expect(result.registry.archivedWorkspaceIds).toEqual([])
    expect(result.registry.deletedWorkspaceIds).toEqual([workspace.id])
    expect(result.registry.list()).toEqual([workspace])
    await expect(realpath(dir)).resolves.toBe(dir)

    await result.registry.restoreDeletedWorkspace(workspace.id)
    expect(result.registry.deletedWorkspaceIds).toEqual([])
    expect(result.registry.get(workspace.id)).toBe(workspace)
  })

  it('soft-deletes and restores a session without changing workspace accounting', async () => {
    const dir = await makeDir('managed-session')
    const result = await harness({ sessions: [header('managed', dir, 100)] })
    const workspace = result.registry.list()[0]!
    await workspace.setSessionPinned(SessionId('managed'), true)

    await result.registry.archiveSession(SessionId('managed'))
    expect(workspace.pinnedSessionIds).toEqual(['managed'])
    await result.registry.unarchiveSession(SessionId('managed'))
    expect(result.registry.archivedSessionIds).toEqual([])
    expect(workspace.pinnedSessionIds).toEqual(['managed'])

    await result.registry.deleteSession(SessionId('managed'))
    expect(result.registry.deletedSessionIds).toEqual(['managed'])
    expect(result.registry.archivedSessionIds).toEqual([])
    expect(workspace.sessionIds).toContain('managed')
    expect(workspace.pinnedSessionIds).toEqual(['managed'])

    await result.registry.restoreSession(SessionId('managed'))
    expect(result.registry.deletedSessionIds).toEqual([])
    expect(workspace.sessionIds).toContain('managed')
    expect(workspace.pinnedSessionIds).toEqual(['managed'])
  })

  it('moves an archived session into the deleted set atomically and rejects archive until restored', async () => {
    const dir = await makeDir('archive-delete-session')
    const result = await harness({ sessions: [header('managed', dir, 100)] })
    await result.registry.archiveSession(SessionId('managed'))
    await result.registry.deleteSession(SessionId('managed'))
    ...
    expect(result.registry.archivedSessionIds).toEqual([])
    expect(result.registry.deletedSessionIds).toEqual(['managed'])
    await expect(result.registry.archiveSession(SessionId('managed')))
      .rejects.toBeInstanceOf(Error)

    await result.registry.restoreSession(SessionId('managed'))
    await expect(result.registry.archiveSession(SessionId('managed'))).resolves.toBeUndefined()
  })
})
