...
export class WorkspaceEntity implements Workspace {
  ...
  get pinnedSessionIds(): readonly SessionId[] {
    return this.record.pinnedSessionIds.filter(id => this.host.sessionPath(id) === this.record.path)
  }
  ...
  async setSessionPinned(sessionId: SessionId, pinned: boolean): Promise<void> {
    await this.mutate((record) => {
      if (!record.sessionIds.includes(sessionId)) {
        throw new WorkspaceMoveInvalidError(
          `cannot ${pinned ? 'pin' : 'unpin'} session '${sessionId}' in workspace '${record.path}': `
          + 'the session is not accounted',
        )
      }
      if (!pinned) {
        return record.pinnedSessionIds.includes(sessionId)
          ? { ...record, pinnedSessionIds: record.pinnedSessionIds.filter(id => id !== sessionId) }
          : record
      }
      const sessionIds = [sessionId, ...record.sessionIds.filter(id => id !== sessionId)]
      const pinnedSessionIds = [sessionId, ...record.pinnedSessionIds.filter(id => id !== sessionId)]
      return sameSessionIds(record.sessionIds, sessionIds)
        && sameSessionIds(record.pinnedSessionIds, pinnedSessionIds)
        ? record
        : { ...record, sessionIds, pinnedSessionIds }
    })
  }
  ...
  async detachSession(sessionId: SessionId): Promise<void> {
    await this.mutate(record => record.sessionIds.includes(sessionId)
      ? {
        ...
        pinnedSessionIds: record.pinnedSessionIds.filter(id => id !== sessionId),
      }
      : record)
  }
  ...
  private async mutate(fn: (record: WorkspaceRecord) => WorkspaceRecord): Promise<void> {
    ...
    try {
      next = await this.host.table().update(this.id, (current) => {
        ...
        const accounted = new Set(sessionIds)
        const pinnedSessionIds = changed.pinnedSessionIds.filter(id => accounted.has(id))
        if (changed === current
          && sameSessionIds(sessionIds, current.sessionIds)
          && sameSessionIds(pinnedSessionIds, current.pinnedSessionIds)) {
          throw unchangedSentinel
        }
        return { ...changed, sessionIds, pinnedSessionIds, updatedAt: new Date().toISOString() }
      })
    } catch (error) {
