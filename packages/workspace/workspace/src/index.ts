...
export class WorkspaceUnknownSessionError extends Error {
  ...
  constructor(readonly sessionId: SessionId) {
    super(`cannot manage session '${sessionId}': live sessions and session persistence hold no such session`)
...
export class WorkspaceRegistry extends Service {
  ...
  /** Workspace registrations hidden from active lists without removing their records. */
  get archivedWorkspaceIds(): readonly WorkspaceId[] {
    return this.requireState().archivedWorkspaceIds
  }

  /** Workspace registrations hidden behind recoverable client tombstones. */
  get deletedWorkspaceIds(): readonly WorkspaceId[] {
    return this.requireState().deletedWorkspaceIds
  }

  /** Session records hidden through recoverable client-side tombstones. */
  get deletedSessionIds(): readonly SessionId[] {
    return this.requireState().deletedSessionIds
  }

  /**
   * Archive a Workspace registration while retaining its directory, record, and session account.
   * @param workspaceId - The Workspace registration to archive.
  ...
  archiveWorkspace(workspaceId: WorkspaceId): Promise<void> {
    return this.enqueueOperation(async () => {
      ...
      const state = this.requireState()
      if (state.deletedWorkspaceIds.includes(workspaceId)) throw new WorkspaceOrderInvalidError(workspaceId)
      if (state.archivedWorkspaceIds.includes(workspaceId)) return
      await this.setState({
        ...state,
        archivedWorkspaceIds: [...state.archivedWorkspaceIds, workspaceId],
      })
    })
  }

  /**
   * Restore an archived Workspace registration to the active list.
   * @param workspaceId - The archived Workspace registration to restore.
   * @returns resolution after the active state is durable.
   */
  unarchiveWorkspace(workspaceId: WorkspaceId): Promise<void> {
    return this.enqueueOperation(async () => {
      ...
      await this.setState({
        ...state,
        archivedWorkspaceIds: state.archivedWorkspaceIds.filter(id => id !== workspaceId),
      })
    })
  }

  /**
   * Soft-delete a Workspace registration while retaining its record, directory, files, and sessions.
   * @param workspaceId - The Workspace registration to hide behind a recoverable tombstone.
   * @returns resolution after the tombstone is durable.
   */
  deleteWorkspace(workspaceId: WorkspaceId): Promise<void> {
    return this.enqueueOperation(async () => {
      if (!this.entities.has(workspaceId)) throw new WorkspaceOrderInvalidError(workspaceId)
      const state = this.requireState()
      if (state.deletedWorkspaceIds.includes(workspaceId)) return
      await this.setState({
        ...state,
        archivedWorkspaceIds: state.archivedWorkspaceIds.filter(id => id !== workspaceId),
        deletedWorkspaceIds: [...state.deletedWorkspaceIds, workspaceId],
      })
    })
  }

  /**
   * Restore a soft-deleted Workspace registration to active projections.
   * @param workspaceId - The soft-deleted Workspace registration to restore.
   * @returns resolution after the tombstone is removed durably.
   */
  restoreDeletedWorkspace(workspaceId: WorkspaceId): Promise<void> {
    return this.enqueueOperation(async () => {
      if (!this.entities.has(workspaceId)) throw new WorkspaceOrderInvalidError(workspaceId)
      const state = this.requireState()
      if (!state.deletedWorkspaceIds.includes(workspaceId)) return
      await this.setState({
        ...state,
        deletedWorkspaceIds: state.deletedWorkspaceIds.filter(id => id !== workspaceId),
      })
    })
  }
  ...
  archiveSession(sessionId: SessionId): Promise<void> {
    return this.enqueueOperation(async () => {
      ...
      const state = this.requireState()
      if (state.deletedSessionIds.includes(sessionId)) throw new WorkspaceUnknownSessionError(sessionId)
  ...
  unarchiveSession(sessionId: SessionId): Promise<void> {
    return this.enqueueOperation(async () => {
      ...
      if (!state.archivedSessionIds.includes(sessionId)) {
        if (!(await this.sessionKnown(sessionId)) || state.deletedSessionIds.includes(sessionId)) {
          throw new WorkspaceUnknownSessionError(sessionId)
        }
        return
      }
  ...
  /**
   * Soft-delete one Session from client history. The persistence artifact and
   * enterprise directory remain untouched; only a durable tombstone is added.
   * @param sessionId - The Session to hide behind a recoverable tombstone.
   * @returns resolution after the tombstone is durable.
   */
  deleteSession(sessionId: SessionId): Promise<void> {
    return this.enqueueOperation(async () => {
      const state = this.requireState()
      if (state.deletedSessionIds.includes(sessionId)) return
  ...
  /**
   * Restore a soft-deleted Session to client history without touching its log or files.
   * @param sessionId - The soft-deleted Session to restore.
   * @returns resolution after the tombstone is removed durably.
   */
  restoreSession(sessionId: SessionId): Promise<void> {
    return this.enqueueOperation(async () => {
      ...
      await this.setState({
        ...state,
        deletedSessionIds: state.deletedSessionIds.filter(id => id !== sessionId),
      })
    })
  }
  ...
  private async createCanonical(canonical: string, title?: string): Promise<WorkspaceEntity> {
    ...
    try {
      const { pendingMutation: _pendingMutation, ...committedState } = state
      await this.setState({
        ...committedState,
        initialized: true,
  ...
  private async deleteKnown(id: WorkspaceId): Promise<boolean> {
    ...
    const nextState: WorkspaceDomainState = {
      ...
      deletedWorkspaceIds: state.deletedWorkspaceIds.filter(workspaceId => workspaceId !== id),
    }
    await this.setState({
      ...nextState,
  ...
  private async recoverPendingMutation(): Promise<void> {
    ...
    await this.setState({
      ...committedState,
      archivedWorkspaceIds: state.archivedWorkspaceIds.filter(id => id !== pending.workspaceId),
      deletedWorkspaceIds: state.deletedWorkspaceIds.filter(id => id !== pending.workspaceId),
    })
  }
  ...
  private async bootstrap(headers: readonly SessionHeader[]): Promise<void> {
    ...
    if (!sameIds(state.workspaceIds, workspaceIds)) {
      const { pendingMutation: _pendingMutation, ...committedState } = state
      await this.setState({ ...committedState, initialized: false, workspaceIds })
    }
    const { pendingMutation: _pendingMutation, ...committedState } = state
    await this.setState({ ...committedState, initialized: true, workspaceIds })
  }
  ...
  private validateStoredState(state: WorkspaceDomainState): void {
    ...
    for (const id of state.archivedWorkspaceIds) {
      if (!order.has(id)) {
        throw new Error(`workspace domain is inconsistent: archived workspace '${id}' is absent from registry order`)
      }
    }
    if (new Set(state.archivedWorkspaceIds).size !== state.archivedWorkspaceIds.length) {
      throw new Error('workspace domain is inconsistent: archived workspace ids contain duplicates')
    }
    for (const id of state.deletedWorkspaceIds) {
      if (!order.has(id)) {
        throw new Error(`workspace domain is inconsistent: deleted workspace '${id}' is absent from registry order`)
      }
    }
    if (new Set(state.deletedWorkspaceIds).size !== state.deletedWorkspaceIds.length) {
      throw new Error('workspace domain is inconsistent: deleted workspace ids contain duplicates')
    }
    const deletedWorkspaces = new Set(state.deletedWorkspaceIds)
    if (state.archivedWorkspaceIds.some(id => deletedWorkspaces.has(id))) {
      throw new Error('workspace domain is inconsistent: a workspace cannot be both archived and deleted')
    }
    if (new Set(state.archivedSessionIds).size !== state.archivedSessionIds.length) {
      throw new Error('workspace domain is inconsistent: archived session ids contain duplicates')
    }
    const deleted = new Set(state.deletedSessionIds)
    if (deleted.size !== state.deletedSessionIds.length) {
      throw new Error('workspace domain is inconsistent: deleted session ids contain duplicates')
    }
    if (state.archivedSessionIds.some(id => deleted.has(id))) {
      throw new Error('workspace domain is inconsistent: a session cannot be both archived and deleted')
    }
