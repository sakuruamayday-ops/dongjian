...
export async function applyIndexInjections(
  ...
  for (const row of rows) {
    switch (row.kind) {
      ...
      case 'script-preload':
        ...
        break
      case 'boot-ready':
        // The Host table can gather this row before later subscribers append
        // their injections. Settle readiness only after the whole table ran.
        readyScripts.push(row.src)
        break
      case 'style': {
  ...
  for (const src of readyScripts) await loadScript(src)
}
