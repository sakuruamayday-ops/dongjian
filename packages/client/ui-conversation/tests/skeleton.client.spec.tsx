...
import {
  ...
  workspaceSnapshot as workspaceFixture,
...
function mount(
  ...
  const renderSlot = ((key: string, owner: object, opts?: { only?: string; fallback?: ReactNode }) => {
    ...
    if (key === 'conversation.hero.headline' || key === 'conversation.hero.preview') {
...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    ...
    expect(renderSlot.mock.calls.find(call => call[0] === 'conversation.hero.headline')?.[2]?.fallback)
      .toBe('Into the Unknown')
    expect(renderSlot.mock.calls.find(call => call[0] === 'conversation.hero.preview')?.[2]?.fallback)
      .toBeTruthy()
  })
})
