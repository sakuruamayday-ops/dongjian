...
import {
  ...
  sessionSnapshot as sessionFixture, workspaceSnapshot as workspaceFixture,
...
function bench(over?: BenchOptions) {
  ...
  const renderSlot = ((key: string, owner: object) => {
    ...
    if (key === 'conversation.input.payload') return over?.payload ?? null
  ...
  const props: InputBarProps = {
    ...
    useWorkspaces: bindSnapshotSelector(createSnapshotStore(workspaceFixture())),
...
describe('strips and variants', () => {
  it.each(['hero', 'composer'] as const)('places draft payload inside the %s card before the editor', (variant) => {
    const { view, textarea, props, shell, slotCalls } = bench({
      variant, payload: <i data-testid="draft-payload" />,
    })
    const payload = view.getByTestId('draft-payload')
    expect(payload.closest('[data-composer-card]')).toBe(textarea.closest('[data-composer-card]'))
    expect(payload.compareDocumentPosition(textarea) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0)
    expect(slotCalls.findLast(call => call.key === 'conversation.input.payload')!.owner).toEqual({ input: shell.snapshot })
    view.rerender(<InputBar {...props} sessionId={undefined} />)
    expect(view.queryByTestId('draft-payload')).toBeNull()
  })
...
describe('command launcher chrome and control seats', () => {
  it('gives left-side imports the same image intake and current lock as the resident composer', () => {
    const addFiles = vi.fn(() => null)
    const { slotCalls } = bench({ addFiles })
    const left = slotCalls.findLast(call => call.key === 'conversation.input.left')!.owner as {
      locked: boolean
      onAddFiles: ComposerAttachmentsOwnerProps['onAddFiles']
    }
    const rail = slotCalls.findLast(call => call.key === 'conversation.input.attachments')!.owner as ComposerAttachmentsOwnerProps
    expect(left.locked).toBe(false)
    expect(left.onAddFiles).toBe(rail.onAddFiles)
    const image = new File(['image'], 'image.png', { type: 'image/png' })
    act(() => { left.onAddFiles([image]) })
    expect(addFiles).toHaveBeenCalledExactlyOnceWith([image])
    const blocked = bench({ blocked: { reason: '等待当前操作结束' } })
    expect(blocked.slotCalls.findLast(call => call.key === 'conversation.input.left')!.owner).toMatchObject({ locked: true })
  })
  ...
  it('renders the command launcher; the Access chip is absent without the permissions projection; the control seats render EMPTY without entries', () => {
    ...
    expect([...new Set(slotCalls.map(c => c.key))]).toEqual([
      'conversation.input.overlay', 'conversation.input.payload', 'conversation.input.attachments',
