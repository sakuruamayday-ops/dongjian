...
import type { ConversationPromptAdmissionOutcome } from '../src/client/contract/input.ts'
...
describe('ConversationController', () => {
  it('routes operations through the public Session binding', async () => {
    const b = await bench()
    await expect(b.scoped.send('hello')).resolves.toBe('test-submission-1')
    ...
    expect(b.prompt).toHaveBeenCalledWith(
      [{ type: 'text', text: 'hello' }], 'queue', undefined, 'test-submission-1',
    )
  ...
  it('transactionally prepares the next ordinary prompt without changing the visible draft', async () => {
    const b = await bench()
    const settle = vi.fn()
    b.runtime.ctx.provide('conversationPromptPreparation', {
      prepare: (sessionId, text) => ({
        text: `prepared:${sessionId}:${text}`,
        settle,
      }),
    })
    const beginSubmission = vi.spyOn(b.runtime.sessions.behavior('s1'), 'beginSubmission')
    b.shell.setDraft('real user task')
    b.shell.submit()
    await vi.waitFor(() => {
      expect(b.prompt).toHaveBeenCalledWith(
        [{ type: 'text', text: 'prepared:s1:real user task' }],
        'queue',
    ...
    expect(beginSubmission).toHaveBeenCalledWith(expect.objectContaining({
      text: 'real user task', attachments: [],
    }))
    expect(settle).toHaveBeenCalledWith('success')
    await b.runtime.dispose()
  })

  it('submits product-owned attachments without requiring visible composer text', async () => {
    const b = await bench()
    const settle = vi.fn()
    b.runtime.ctx.provide('conversationPromptPreparation', {
      hasPayload: () => true,
      prepare: (_sessionId, text) => ({
        text: text === '' ? 'internal document reference' : text,
        displayText: '@"共创导入资料/申报材料.xlsx"',
        settle,
      }),
    })
    const beginSubmission = vi.spyOn(b.runtime.sessions.behavior('s1'), 'beginSubmission')
    b.shell.actions.refreshExternalPayload?.()
    expect(b.shell.snapshot.externalPayload).toBe(true)
    ...
    await vi.waitFor(() => {
      expect(b.prompt).toHaveBeenCalledWith(
        [{
          type: 'text',
          text: 'internal document reference',
        }],
        'queue',
        expect.any(AbortSignal),
        'test-submission-1',
    ...
    expect(beginSubmission).toHaveBeenCalledWith(expect.objectContaining({
      text: '@"共创导入资料/申报材料.xlsx"', attachments: [],
    }))
    expect(settle).toHaveBeenCalledWith('success')
    await b.runtime.dispose()
  })

  it('asks product admission before image serialization and preserves the draft when consent is rejected', async () => {
    const b = await bench()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:consent')
    const decision = Promise.withResolvers<ConversationPromptAdmissionOutcome>()
    const admit = vi.fn(() => decision.promise)
    b.runtime.ctx.provide('conversationPromptAdmission', { admit })
    b.shell.setDraft('分析这张图片')
    const [image] = b.root.createDrafts('s1' as SessionId, [
      new File([Uint8Array.of(1)], 'consent.png', { type: 'image/png' }),
    ])
    if (image === undefined) throw new Error('image draft missing')
    expect(b.shell.addAttachments([image.id])).toBe(true)

    b.shell.submit()
    await vi.waitFor(() => {
      expect(admit).toHaveBeenCalledWith({
        sessionId: 's1', text: '分析这张图片', imageCount: 1,
      }, expect.any(AbortSignal))
    })
    expect(b.prompt).not.toHaveBeenCalled()

    decision.resolve({ kind: 'reject', text: '已取消图片传输；本次草稿和附件已保留。' })
    await vi.waitFor(() => {
      expect(b.shell.snapshot.draft).toBe('分析这张图片')
      expect(b.shell.snapshot.attachmentIds).toEqual([image.id])
    })
    expect(b.prompt).not.toHaveBeenCalled()
    expect(b.shell.notices.getSnapshot()).toMatchObject({
      level: 'error', text: '已取消图片传输；本次草稿和附件已保留。',
    })
    created.mockRestore()
    await b.runtime.dispose()
  })

  it('does not request image-transfer consent for a generic file attachment', async () => {
    const b = await bench()
    const admit = vi.fn(() => Promise.resolve({ kind: 'allow' as const }))
    b.runtime.ctx.provide('conversationPromptAdmission', { admit })
    const session = b.runtime.sessions.binding('s1')!.session
    ;(session as { uploadFile?: unknown }).uploadFile = vi.fn(() => Promise.resolve({
      ok: true as const,
      value: {
        receiptId: 'pdf-receipt' as never,
        file: { attachmentId: 'pdf-file' as never, name: 'report.pdf', bytes: 1 },
      },
    }))
    const [file] = b.root.createDrafts(session.sessionId, [
      new File([Uint8Array.of(1)], 'report.pdf', { type: 'application/pdf' }),
    ])
    if (file === undefined) throw new Error('file draft missing')
    await vi.waitFor(() => {
      expect(b.root.fileUploads.getSnapshot()[file.id]?.status).toBe('ready')
    })
    b.shell.setDraft('分析这份报告')
    expect(b.shell.addAttachments([file.id])).toBe(true)
    b.shell.submit()
