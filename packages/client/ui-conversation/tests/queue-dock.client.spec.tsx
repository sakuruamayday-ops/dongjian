...
describe('QueueDock', () => {
  ...
  it('uses complete prepared display text for both pending and admitted queue summaries', () => {
    const displayText = `[metadata]${'quote'.repeat(100)}\nUser comment`
    const summarizeDisplayText = vi.fn((text: string) => text === displayText ? '1 条注释 · User comment' : undefined)
    const pending = {
      ...snapshotWith([]),
      pendingSubmissions: [{
        requestId: 'req-annotation' as never, placement: 'queued' as const,
        time: 1, text: displayText, attachments: [],
      }],
    }
    const source = liveSession(pending)
    const view = render(<QueueDock {...kitFor(pending, { summarizeDisplayText })} useSession={source.useSession} />)
    expect(view.getByText('1 条注释 · User comment')).toBeTruthy()
    expect(view.container.textContent).not.toContain('[metadata]')
    act(() => {
      source.push({
        ...pending,
        queue: [{ ...row('annotation', displayText, displayText.slice(0, 200)), text: null, rpcId: 'req-annotation' as never }],
      })
    })
    expect(summarizeDisplayText).toHaveBeenLastCalledWith(displayText)
    expect(view.getAllByText('1 条注释 · User comment')).toHaveLength(1)
    expect(view.container.textContent).not.toContain('[metadata]')
    expect(view.getByRole('button', { name: '编辑排队消息' })).toHaveProperty('disabled', true)
    expect(view.getByRole('button', { name: '删除排队消息' })).toHaveProperty('disabled', false)
    act(() => { source.push(snapshotWith([row('plain', 'Unchanged prose')])) })
    expect(view.getByText('Unchanged prose')).toBeTruthy()
