...
`conversation.input.payload` renders full-width draft file and annotation entries inside the composer card, before its editor. `conversation.input.dock` remains above the card for task and queue status; the upstream image-attachment entry retains its own intake and preview lifecycle.

`ctx.conversationDraftText` exposes a stable per-session text source and empty-only restoration for a product-owned persistence adapter. `ctx.conversationDraftImages` exposes ordered image ids, ordinary-send serialization, and empty-only restoration into the same browser image registry. Neither exposes the Lexical editor or submission state. The product persistence adapter owns durable bytes; the conversation registry owns preview URLs and send/remove lifecycle.
...
- Prepared queue messages cannot be edited inline until their owner can preserve the associated context. Removal and steering remain available.
