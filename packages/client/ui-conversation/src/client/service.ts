...
export interface IConversation {
   ...
   */
  send(text: string): Promise<SessionRequestId>
  /**
  ...
  /**
   * Count image-kind draft attachments without treating generic files as images.
   * @param ids - Draft attachment identities owned by this conversation.
   * @returns Number of identities that resolve to image drafts.
   */
  countDraftImages(ids: readonly DraftAttachmentId[]): number
}
...
export class ConversationController extends Service implements IConversation {
   ...
   * @param text - prompt text, sent verbatim as one text block.
   */
  async send(text: string): Promise<SessionRequestId> {
   ...
   * @param displayText - optional user-facing text retained by the local submission echo.
  ...
  async sendSession(
    ...
    if (snapshot.subagent !== null) {
      ...
      const result = await session.prompt(content, mode, signal, undefined, displayText)
    ...
    const result = await session.prompt(content, mode, signal, submission.requestId, displayText)
  ...
  /** Count only image-kind drafts for product transfer-consent UI and admission. */
  countDraftImages(ids: readonly DraftAttachmentId[]): number {
    return this.resolveDraftAttachments(ids).filter(attachment => attachment.kind === 'image').length
  }
