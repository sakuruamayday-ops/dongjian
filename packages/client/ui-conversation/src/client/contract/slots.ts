...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /** Optional product-owned blank-session headline. */
    'conversation.hero.headline': { kind: 'single'; scope: 'root' }
    /** Optional product-owned replacement for the upstream preview badge. */
    'conversation.hero.preview': { kind: 'single'; scope: 'root' }
    /** Agent-preset control staged for a New Session. */
    'conversation.hero.agentPreset': { kind: 'single'; scope: 'root'; owner: HeroAgentPresetOwnerProps }
    /** Full-width entries above the composer card. */
    'conversation.input.dock': { kind: 'list'; scope: 'session'; owner: InputZone }
    /** Draft file and annotation entries inside the composer card, before the editor. */
    'conversation.input.payload': { kind: 'list'; scope: 'session'; owner: Pick<InputZone, 'input'> }
    ...
    'conversation.input.left': {
      kind: 'list'
      scope: 'session'
      owner: InputControlOwnerProps & Pick<ComposerAttachmentsOwnerProps, 'onAddFiles'>
    }
