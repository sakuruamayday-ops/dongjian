...
/** One product-owned prompt prepared transactionally from the visible draft. */
export interface PreparedConversationPrompt {
  /** Model-facing text admitted to the Session. */
  readonly text: string
  /** Optional presentation-only prefix retained before the user's original draft. */
  readonly displayPrefix?: string
  /** Optional user-facing text retained by the local submission echo. */
  readonly displayText?: string
  /** Settle product-owned state after Host admission completes. */
  settle(outcome: 'success' | 'failure'): void
}

/** Optional product-owned one-shot preparation applied by the ordinary composer sink. */
export interface ConversationPromptPreparation {
  /**
   * Summarize prepared display text for compact queue rows without exposing metadata.
   * @param displayText - Complete user-facing message before queue truncation.
   * @returns A plain-text summary, or undefined to keep the ordinary preview.
   */
  summarize?(displayText: string): string | undefined
  /**
   * Whether product state can form a prompt without visible composer text.
   * @param sessionId - Session whose product-owned draft state is inspected.
   * @returns true when preparation can produce a prompt without visible text.
   */
  hasPayload?(sessionId: SessionId): boolean
  /**
   * Prepare one visible draft without mutating the editor.
   * @param sessionId - target Session.
   * @param text - visible composer text.
   * @returns the prepared prompt, or undefined when product state contributes nothing.
   */
  prepare(sessionId: SessionId, text: string): PreparedConversationPrompt | undefined
}

/** Product-owned admission request issued before an image is serialized or uploaded. */
export interface ConversationPromptAdmissionRequest {
  readonly sessionId: SessionId
  readonly text: string
  readonly imageCount: number
}

/** Result of a product-owned pre-send admission decision. */
export type ConversationPromptAdmissionOutcome =
  | { readonly kind: 'allow' }
  | { readonly kind: 'reject'; readonly text?: string }

/** Optional asynchronous product boundary that can reject a send without consuming its draft. */
export interface ConversationPromptAdmission {
  /**
   * Decide whether an ordinary composer send may proceed before upload starts.
   * @param request - prompt identity and attachment count to evaluate.
   * @param signal - cancellation for a superseded admission.
   * @returns the product-owned allow or reject decision.
   */
  admit(
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Optional product workflow that transactionally prepares the next ordinary prompt. */
    conversationPromptPreparation: ConversationPromptPreparation
    /** Optional product workflow that runs before image serialization and Host submission. */
    conversationPromptAdmission: ConversationPromptAdmission
  }
...
export interface InputActions {
  ...
  /** Drop ids whose browser-owned objects no longer exist. */
  pruneAttachments(ids: readonly DraftAttachmentId[]): void
  /** Re-project product-owned payload eligibility after an accessory changes. */
  refreshExternalPayload?(): void
...
export interface InputState {
  ...
  /** Product-owned pending content that can be sent without visible text. */
  readonly externalPayload?: boolean
