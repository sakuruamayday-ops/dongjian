...
export interface QueueDockInjected {
  ...
  /** Project complete prepared display text before a compact row truncates it. */
  summarizeDisplayText?: (text: string) => string | undefined
}
 ...
 */
export function QueueDock({ useSession, updateQueue, notify, loadImage, summarizeDisplayText, t }: QueueDockProps) {
      ...
      <div className={css.panel}>
        ...
        <ul id={listId} className={css.list} hidden={!listVisible}>
          {listVisible && queue.map((row) => {
            ...
            const displayText = row.content.flatMap(block => block.type === 'text' ? [block.text] : []).join('')
            const preview = summarizeDisplayText?.(displayText) ?? row.preview
            return (
          ...
          {listVisible && pendingQueue.map((submission) => {
            return (
              <li key={submission.requestId} className={css.row} data-submission-echo="">
                ...
                <span className={css.preview}>{projectUserText(summarizeDisplayText?.(submission.text) ?? submission.text, [])}</span>
              </li>
            )
          })}
        </ul>
    ...
    ctx.slots.inject('conversation.input.dock', () => ctx.slots.register({
      ...
      inject: (sessionId: SessionId): QueueDockInjected => {
        ...
        return {
          ...
          summarizeDisplayText: text => ctx.get('conversationPromptPreparation')?.summarize?.(text),
        }
      },
    }, QueueDock))
  },
