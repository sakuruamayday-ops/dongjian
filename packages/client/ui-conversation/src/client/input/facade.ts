...
export interface SessionInputDeps {
  ...
  /** Whether product-owned state can submit while the visible draft is empty. */
  hasExternalPayload?: (() => boolean) | undefined
  /**
...
export class SessionInputShell implements SessionInput {
  ...
  readonly actions: InputActions = {
    ...
    refreshExternalPayload: () => { this.publish() },
  ...
  /** Prevent duplicate sends while product-owned attachment state is awaiting Host admission. */
  private externalPayloadFlight = false
  ...
  submit(mode: InputSubmitMode = 'queue'): void {
    const externalPayload = this.deps.hasExternalPayload?.() === true
    if (this.snapshot.draft.trim() === '' && (this.attachmentIds.length > 0 || externalPayload)) {
      if (this.snapshot.phase === 'plain' && !this.externalPayloadFlight) {
        ...
        this.externalPayloadFlight = externalPayload
        ...
        void this.deps.defaultSink('', attachmentIds, mode, controller.signal).then((outcome) => {
          ...
          this.externalPayloadFlight = false
          this.publish()
  ...
  private compose(): InputState {
    ...
    return {
      ...
      externalPayload: !this.externalPayloadFlight && this.deps.hasExternalPayload?.() === true,
      draftRev: this.rev,
      phase: core.phase,
