...
interface ConversationAttachmentFace {
  ...
  countDraftImages(attachmentIds: readonly DraftAttachmentId[]): number
...
export class InputHub implements SessionInputResolver {
  ...
  shellFor(binding: SessionBinding): SessionInputShell {
    ...
    const shell = new SessionInputShell({
      ...
      hasExternalPayload: () => this.rootCtx.get('conversationPromptPreparation')
        ?.hasPayload?.(id) === true,
  ...
  private async sink(
    ...
    const preparation = this.rootCtx.get('conversationPromptPreparation')
    const prepared = preparation?.prepare(session.sessionId, text)
    if (text === '' && attachmentIds.length === 0 && prepared === undefined) {
      return Promise.resolve({ kind: 'success' })
    }
    const imageCount = attachmentIds.length === 0
      ? 0
      : this.conversation().countDraftImages(attachmentIds)
    if (imageCount > 0) {
      const admission = this.rootCtx.get('conversationPromptAdmission')
      if (admission !== undefined) {
        ...
        if (outcome.kind === 'reject') return { kind: 'error', ...(outcome.text === undefined ? {} : { text: outcome.text }) }
      }
    }
    try {
      const outcome = await this.conversation().sendSession(
        session,
        prepared?.text ?? text,
        attachmentIds,
        mode,
        signal,
        prepared === undefined ? undefined : prepared.displayText ?? text,
      )
      prepared?.settle(outcome.kind === 'success' ? 'success' : 'failure')
      return outcome
    } catch (error) {
