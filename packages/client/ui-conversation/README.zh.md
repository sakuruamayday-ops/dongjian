...
`conversation.input.payload` 在输入框内部、编辑器之前展示占满内容宽度的待发送文件和注释。`conversation.input.dock` 仍位于输入框上方，用于任务和队列状态；上游图片附件入口保留独立的接收与预览生命周期。

`ctx.conversationDraftText` 为产品持久化适配器提供稳定的逐会话文字源，以及仅在草稿为空时执行的恢复操作。`ctx.conversationDraftImages` 提供有序图片标识、常规发送序列化和空草稿恢复，恢复后的图片进入同一浏览器图片注册表。二者都不暴露 Lexical 编辑器或提交状态。产品持久化适配器拥有持久字节，Conversation 注册表拥有预览 URL 及发送、移除生命周期。
...
- 在所属方能够保留相关上下文之前，已准备的队列消息不能内联编辑。仍可移除或引导这些消息。
