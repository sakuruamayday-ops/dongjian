...
export function apply(ctx: ClientContext): void {
  ...
  // Each surface owns an independent declaration lifetime. Product shells may
  // replace the stock sidebar while retaining the conversation picker, so one
  // absent hole must not withhold the occupant from the other.
  for (const name of [
    'conversation.hero.workspace.directoryFlow',
    'sidebar.workspaces.directoryFlow',
  ] as const) {
    ctx.slots.inject(name, () => ctx.slots.register({ name, inject: injected }, BrowseDirectoryFlow))
  }
}
