...
async function bench() {
  ...
  const declare = (...holes: typeof HOLES[number][]) => slots.register({
    name: 'root',
    children: Object.fromEntries((holes.length === 0 ? HOLES : holes)
      .map(name => [name, { kind: 'single', scope: 'root' }])),
  } as never, () => null)
...
describe('directory-picker-browse client half', () => {
  ...
  it('fills the conversation flow without waiting for the stock-sidebar hole', async () => {
    const b = await bench()
    b.declare(HOLES[0])
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    expect(b.slots.entries(HOLES[0])).toHaveLength(1)
    expect(b.slots.entries(HOLES[1])).toHaveLength(0)
  })
