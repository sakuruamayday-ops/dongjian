...
function messageTurn(match: Parameters<ConversationNodeDefinition['start']>[1]): number | null {
  if (match.location.kind === 'turn' || match.location.kind === 'step') return match.location.turn.turn
  return null
}
