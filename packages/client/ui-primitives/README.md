...
`AnnotationSummary` renders a localized count disclosure and a viewport-constrained numbered list. Its source-message marker uses neutral text and background tokens in both themes, including hover; annotation numbers do not represent an error or notification state. Read-only messages show quotations and comments; draft consumers supply comment and removal callbacks. Escape and explicit close restore trigger focus, outside interaction dismisses the panel, and Enter inside it never bubbles into a surrounding composer submission handler. The atom owns no persistence or model requests.
