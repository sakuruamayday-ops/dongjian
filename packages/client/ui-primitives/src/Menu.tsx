 ...
 * @param props.footerContent - a composed product action rendered before the
 * ordinary pinned footer rows.
 * @param props.header - non-scrolling controls rendered above the item viewport.
 * @returns anchor wrapper with the conditional list.
 */
export function Menu({
  open, anchor, items, selectedId, selectedIds, onSelect, onClose,
  align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false,
  dense = false, compact = false, getAnchorRect, header, footer, footerContent, className,
}: {
  open: boolean
  anchor: ReactNode
  ...
  const list = open && (
    ...
    >
      ...
      {(footerContent !== undefined || (footer !== undefined && footer.length > 0)) && (
