...
`AnnotationSummary` 渲染本地化的数量入口和受视口约束的编号列表。来源消息编号在两种主题及悬停状态下都使用中性的文字与背景样式，不表示错误或通知状态。只读消息展示引文和评论；草稿使用方提供评论及移除回调。Escape 和关闭按钮把焦点还给入口，外部交互关闭面板，面板内的 Enter 不会触发外层输入框发送。该组件不负责持久化或模型请求。
