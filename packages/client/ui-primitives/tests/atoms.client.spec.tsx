...
describe('Menu', () => {
  ...
  it('selected item shows the trailing check; align=end, side=top, and className apply', () => {
    const { container } = render(
      <Menu
        ...
        header={<input aria-label="Filter menu" />}
        items={items}
        selectedId="a"
        onSelect={() => {}}
        onClose={() => {}}
    ...
    expect(screen.getByRole('textbox', { name: 'Filter menu' })).toBeTruthy()
  ...
  it('renders a composed product action before ordinary footer rows', () => {
    render(
      <Menu
        open
        anchor={<span>trigger</span>}
        items={items}
        footer={[{ id: 'none', label: 'No workspace' }]}
        footerContent={<button type="button" role="menuitem">Create enterprise</button>}
        onSelect={() => {}}
        onClose={() => {}}
      />)
    const create = screen.getByRole('menuitem', { name: 'Create enterprise' })
    const none = screen.getByRole('menuitem', { name: 'No workspace' })
    expect(create.closest('div[class*="footer"]')).toBe(none.closest('div[class*="footer"]'))
    expect(create.compareDocumentPosition(none) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0)
  })
