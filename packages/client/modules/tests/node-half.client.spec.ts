...
import { bootThemeInjection } from '../../ui-theme/src/boot-theme.ts'
...
/** Fetch and execute the exact same-origin facade emitted by the Host boot rows. */
async function injectedFacade(graph: WebBootGraph): Promise<{ html: string; target: ClientModuleLoaderTarget }> {
  const html = renderIndexInjections(
    '<html><head></head><body><script type="module" src="/index.js"></script></body></html>',
    [...bootInjections(graph), bootThemeInjection('dark', 16)],
  )
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'dsh-client-modules-bootstrap-')))
  const { route } = constructWithRoute([])
  const response = await routeRequest(route, '/plugins/bootstrap.js')
  const source = response.body.toString('utf8')
  if (response.status !== 200 || source === '') throw new Error('missing same-origin ModuleLoader facade')
  const window: { __ModuleLoader__?: ClientModuleLoaderTarget } = {}
  runInNewContext(source, { window })
  if (window.__ModuleLoader__ === undefined) throw new Error('external facade did not install __ModuleLoader__')
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const { html, target } = await injectedFacade(graph)
    const facadeAt = html.indexOf('<script src="/plugins/bootstrap.js"></script>')
    const manifestAt = html.indexOf('<script id="dsh-boot-manifest" type="application/json">')
    ...
    const themeAt = html.indexOf('/theme-bootstrap.js?preference=dark&amp;fontSize=16')
    const readyAt = html.indexOf('<script src="/plugins/bootstrap-ready.js"></script>')
    const entryAt = html.indexOf('<script type="module" src="/index.js"></script>')
    expect([facadeAt, manifestAt, applicationAt, bootstrapAt, themeAt, readyAt, entryAt]).toEqual([...new Set([
      facadeAt, manifestAt, applicationAt, bootstrapAt, themeAt, readyAt, entryAt,
    ])].sort((a, b) => a - b))
    expect(html.match(/<script(?![^>]+src=)(?![^>]+type="application\/json")[^>]*>/g)).toBeNull()
  ...
  it('rejects a page that did not preload the modules bundle', async () => {
    const graph = bootGraph()
    const { target } = await injectedFacade(graph)
    expect(() => target.create({ boot: graph, staticModules: {} }))
      .toThrow(`HTML did not preload ${MODULES_ID}/client.js`)
  })

  it('rejects a bootstrap bundle with a runtime external', async () => {
    const graph = bootGraph()
    const { target } = await injectedFacade(graph)
    target.load({
      id: MODULES_ID,
  ...
  ])('rejects a bootstrap bundle without the complete module face', async (exports) => {
    const graph = bootGraph()
    const { target } = await injectedFacade(graph)
  ...
  it('escapes graph text in inert JSON and serves both same-origin bootstrap scripts', async () => {
    const graph = bootGraph()
    graph.entries.push({ id: '</script><script>bad', url: '/safe.js', rev: 'safe' })
    const html = renderIndexInjections('<head></head><body></body>', bootInjections(graph))
    expect(html).toContain('id="dsh-boot-manifest" type="application/json"')
    expect(html).toContain('\\u003c/script>\\u003cscript>bad')
    expect(html).not.toContain('</script><script>bad')
    expect(html).not.toContain('__DSH_BOOT__')

    root ??= realpathSync(mkdtempSync(join(tmpdir(), 'dsh-client-modules-bootstrap-routes-')))
    const { route } = constructWithRoute([])
    const facade = await routeRequest(route, '/plugins/bootstrap.js')
    const ready = await routeRequest(route, '/plugins/bootstrap-ready.js')
    ...
    expect(facade.body.toString('utf8')).toContain('window.__ModuleLoader__=')
    expect(ready.status).toBe(200)
    expect(ready.body.toString('utf8')).toContain('__DSH_BOOT_READY__')
    expect((await routeRequest(route, '/plugins/bootstrap-ready.js', 'HEAD')).body).toHaveLength(0)
  })
})
