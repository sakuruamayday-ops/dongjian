 ...
 * inert boot-manifest entry graph (wire single source: {@link WebBootEntry}
...
/** Same-origin registration facade served by this package's existing route. */
const CLIENT_BOOTSTRAP_PATH = '/plugins/bootstrap.js'

/** Same-origin boot-readiness resolver executed after every injection row. */
const CLIENT_BOOTSTRAP_READY_PATH = '/plugins/bootstrap-ready.js'

/**
 * Install the registration facade without executable inline script. The
 * source is response-invariant; the Host graph stays in an inert JSON element.
 */
const CLIENT_BOOTSTRAP_SOURCE = `(()=>{
const pendingQueue=[]
...
/** Resolve the optional asynchronous-transport gate without inline script. */
const CLIENT_BOOTSTRAP_READY_SOURCE = '(globalThis.__DSH_BOOT_READY__ ??= Promise.withResolvers()).resolve()'

/**
 * The CSP-safe boot protocol as index injection rows. A same-origin facade
 * precedes application preloads and blocking bootstrap batches. The Host graph
 * remains inert JSON for the shell to parse, and a same-origin readiness row
 * settles after every ordinary injection.
 * @param graph - the composed entry graph.
 * @returns external bootstrap, inert graph, preload hints, bootstrap batches,
 * and the delayed readiness row.
 */
export function bootInjections(graph: WebBootGraph): IndexInjection[] {
  ...
  const rows: IndexInjection[] = [
    { kind: 'script-src', placement: 'head', src: CLIENT_BOOTSTRAP_PATH },
    {
      kind: 'html',
      placement: 'head',
      html: `<script id="dsh-boot-manifest" type="application/json">${json}</script>`,
    },
  ]
  ...
  rows.push({ kind: 'boot-ready', src: CLIENT_BOOTSTRAP_READY_PATH })
  return rows
}
...
export class ClientModuleRegistry extends Service {
   ...
   * @returns the graph serialized into the inert `dsh-boot-manifest` script.
   */
  graph(): WebBootGraph {
  ...
  private readonly serveBundle = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    const bootstrapSource = requestUrl.pathname === CLIENT_BOOTSTRAP_PATH
      ? CLIENT_BOOTSTRAP_SOURCE
      : requestUrl.pathname === CLIENT_BOOTSTRAP_READY_PATH ? CLIENT_BOOTSTRAP_READY_SOURCE : undefined
    if (bootstrapSource !== undefined) {
      res.writeHead(200, {
        'content-type': 'text/javascript; charset=utf-8',
        'cache-control': 'no-cache',
      })
      res.end(req.method === 'HEAD' ? undefined : bootstrapSource)
      return
    }
