...
import { TurnDeliverables } from '../src/client/TurnDeliverables.tsx'
import {
  basename, deliverablesDefinition, producedFileMentions, producedForClosing, selectProducedFiles, selectProducedFileEntries,
  type DeliverablesTurnData, selectTurnDeliverables,
...
describe('produced-file Turn data', () => {
  it('replays a terminal text draft without a file or a final assistant message', () => {
    const events = [
      at(1, 'turn/start', { turn: 1 }),
      at(2, 'user/message', {
        content: [], source: { kind: 'user', delivery: { turn: 1, phase: 'draft', files: [], issues: [], draftText: '伪造正文' } },
      }),
    ]
    const value = assembler(events)
    expect(selectTurnDeliverables(tailOwner(deliverablesOf(value), 2))).toBeNull()
    const notice = at(3, 'user/message', {
      content: [{ type: 'text', text: '检查未通过' }],
      source: { kind: 'plugin', plugin: 'gongchuang-policy-gate', form: 'notice', delivery: {
        turn: 1, phase: 'draft', files: [], issues: ['e5 原文不匹配'], draftText: '已保留的修改文案',
      } },
    })
    value.append(notice)
    value.flush()
    const expected = { files: [], issues: ['e5 原文不匹配'], draftText: '已保留的修改文案' }
    expect(selectTurnDeliverables(tailOwner(deliverablesOf(value), 3))).toEqual(expected)
    expect(selectTurnDeliverables(tailOwner(deliverablesOf(assembler([...events, notice])), 3))).toEqual(expected)
  })

  it('replays late Host quality outcomes without advertising the unmarked draft as another final file', () => {
    const entries = [
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'openable', 'gongchuang_artifact_probe', { artifactPath: '/reports/report.docx' }),
      result(3, 'openable'),
    ]
    const value = assembler(entries)
    const notice = at(5, 'user/message', {
      id: 'quality-notice', content: [{ type: 'text', text: '文件待完善' }],
      source: {
        kind: 'plugin', plugin: 'gongchuang-policy-gate', form: 'notice', summary: '文件待完善',
        delivery: {
          turn: 1, phase: 'draft', issues: ['缺少政策来源'],
          files: [{ originalPath: '/reports/report.docx', path: '/reports/report（待完善）.docx' }],
        },
      },
    })
    value.append(notice)
    value.flush()
    const expected = [{ path: '/reports/report（待完善）.docx', phase: 'draft', issues: ['缺少政策来源'] }]
    expect(selectProducedFileEntries(tailOwner(deliverablesOf(value), 4))).toEqual(expected)
    const replayed = assembler([...entries, notice])
    expect(selectProducedFileEntries(tailOwner(deliverablesOf(replayed), 4))).toEqual(expected)
    expect(selectProducedFiles(tailOwner(deliverablesOf(replayed), 4))).not.toContain('/reports/report.docx')
  })

  it('does not infer professional approval from ordinary publication or untrusted message metadata', () => {
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'ordinary', 'gongchuang_artifact_probe', { artifactPath: '/notes/session-log.md' }),
      result(3, 'ordinary'),
      at(4, 'user/message', {
        content: [], source: { kind: 'user', delivery: { turn: 1, phase: 'formal', files: [], issues: [] } },
      }),
    ])
    expect(selectProducedFileEntries(tailOwner(deliverablesOf(value), 5))).toEqual([{ path: '/notes/session-log.md' }])
  })
  ...
  it('folds relative publication into the absolute classified artifact path', () => {
    const data: DeliverablesTurnData = {
      produced: [
        { seq: 3, path: '/Users/demo/workspace/out/report.docx', published: true },
        { seq: 4, path: 'out/report.docx', published: true },
      ],
      classification: {
        turn: 1,
        phase: 'formal',
        files: [{
          originalPath: '/Users/demo/workspace/out/report.docx',
          path: '/Users/demo/workspace/out/report.docx',
        }],
        issues: [],
      },
    }

    expect(producedForClosing(data, Infinity, '/Users/demo/workspace')).toEqual(['/Users/demo/workspace/out/report.docx'])
    expect(selectProducedFileEntries({ ...tailOwner(data, 5), cwd: '/Users/demo/workspace' })).toEqual([{
      path: '/Users/demo/workspace/out/report.docx',
      phase: 'formal',
      issues: [],
    }])
  ...
  it('does not collapse distinct absolute files that share one basename', () => {
    const data: DeliverablesTurnData = {
      produced: [
        { seq: 2, path: 'report.docx', published: true },
        { seq: 3, path: '/workspace/a/report.docx', published: true },
        { seq: 4, path: '/workspace/b/report.docx', published: true },
      ],
    }

    expect(producedForClosing(data)).toEqual([
      'report.docx',
      '/workspace/a/report.docx',
      '/workspace/b/report.docx',
    ])
  })
  ...
  it('uses a successful explicit publication as the final multi-file delivery contract', () => {
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'build-script', 'write', { file_path: '/tmp/build-report.ts', content: 'build' }),
      result(3, 'build-script'),
      call(4, 'publish', 'gongchuang_publish_files', {
        paths: [' /workspace/report.docx ', '/workspace/report.pdf', '/workspace/report.docx'],
      }),
      result(5, 'publish'),
    ])
  ...
  it('renders a successful nested PTC publication through the existing Host opener', () => {
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'run-code', 'run_code', { code: 'publish()', description: 'Publish reports' }),
      at(3, 'tool/code-dispatch', {
        rootCallId: 'run-code',
        parentCallId: 'run-code',
        subCallId: 'run-code:code:1',
        turn: 1,
        step: 1,
        name: 'gongchuang_publish_files',
        arguments: { paths: ['/workspace/report.docx', '/workspace/report.pdf'] },
        isError: false,
        content: [{ type: 'text', text: 'model-facing text is not parsed' }],
      }),
      result(4, 'run-code'),
    ])
    const paths = producedForClosing(deliverablesOf(value))
    expect(paths).toEqual(['/workspace/report.docx', '/workspace/report.pdf'])
    ...
    const view = render(
      <ProducedFiles
        ...
        useWorkspacePathOpen={selector => selector(undefined)}
        t={makeTranslate(zh)}
      />,
    )
    fireEvent.click(view.getByRole('button', { name: '打开 /workspace/report.docx' }))
    expect(openFile).toHaveBeenCalledWith('/workspace/report.docx')
  })

  it('publishes root and nested probe-only files and deduplicates explicit publication', () => {
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'probe', 'gongchuang_artifact_probe', { artifactPath: '/workspace/report.docx' }),
      result(3, 'probe'),
      at(4, 'tool/code-dispatch', {
        rootCallId: 'run-code', parentCallId: 'run-code', subCallId: 'run-code:code:1',
        turn: 1, step: 1, name: 'gongchuang_artifact_probe',
        arguments: { artifactPath: '/workspace/report.pdf' }, isError: false, content: [],
      }),
      call(5, 'publish', 'gongchuang_publish_files', { paths: ['/workspace/report.docx'] }),
      result(6, 'publish'),
      call(7, 'bad-probe', 'gongchuang_artifact_probe', { artifactPath: '/workspace/failed.docx' }),
      result(8, 'bad-probe', true),
      call(9, 'malformed-probe', 'gongchuang_artifact_probe', { artifactPath: 42 }),
      result(10, 'malformed-probe'),
    ])
    expect(producedForClosing(deliverablesOf(value), 3)).toEqual(['/workspace/report.docx'])
    expect(producedForClosing(deliverablesOf(value))).toEqual(['/workspace/report.docx', '/workspace/report.pdf'])
  })

  it('ignores failed, malformed, unlocated, and prose-only publication claims', () => {
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'failed', 'gongchuang_publish_files', { paths: ['/workspace/failed.docx'] }),
      result(3, 'failed', true),
      call(4, 'empty', 'gongchuang_publish_files', { paths: ['   '] }),
      result(5, 'empty'),
      at(6, 'tool/code-dispatch', {
        rootCallId: 'run-code', parentCallId: 'run-code', subCallId: 'run-code:code:1',
        turn: 1, step: 1, name: 'gongchuang_publish_files',
        arguments: { paths: ['/workspace/nested-failed.pdf'] }, isError: true,
        content: [{ type: 'text', text: '/workspace/forged-from-prose.pdf' }],
      }),
      at(7, 'tool/code-dispatch', {
        rootCallId: 'run-code', parentCallId: 'run-code', subCallId: 'run-code:code:2',
        turn: 1, step: 1, name: 'gongchuang_publish_files',
        arguments: { paths: [42] }, isError: false,
        content: [{ type: 'text', text: '/workspace/also-forged.docx' }],
      }),
      at(8, 'tool/code-dispatch', {
        rootCallId: 'run-code', parentCallId: 'run-code', subCallId: 'run-code:code:3',
        name: 'gongchuang_publish_files',
        arguments: { paths: ['/workspace/unlocated.docx'] }, isError: false,
        content: [{ type: 'text', text: 'success without a turn is not guessed' }],
      }),
      at(9, 'assistant/message', {
        turn: 1, step: 1,
        message: { role: 'assistant', content: [{ type: 'text', text: '/workspace/prose-only.docx' }] },
      }),
    ])
  ...
  it('shows a tail-page file before pagination supplies its missing Turn start', () => {
    const value = assembler([
  ...
  it('shows native-style terminal draft files from a tail page and keeps classification after prepend', () => {
    const notice = at(13, 'user/message', {
      content: [{ type: 'text', text: '本轮已停止，结果待完善' }],
      source: { kind: 'plugin', plugin: 'gongchuang-policy-gate', form: 'notice', delivery: {
        turn: 1, phase: 'draft', files: [{ originalPath: '/qa/catalog.docx', path: '/qa/catalog（待完善）.docx' }],
        issues: ['缺少内容检查'],
      } },
    })
    const value = assembler([notice, at(14, 'turn/end', { turn: 1, reason: { kind: 'blocked' } })], true)
    const expected = [{ path: '/qa/catalog（待完善）.docx', phase: 'draft', issues: ['缺少内容检查'] }]
    expect(selectProducedFileEntries(tailOwner(deliverablesOf(value), 12))).toEqual(expected)
    expect(selectTurnDeliverables(tailOwner(deliverablesOf(value), 12))).toMatchInlineSnapshot(`
      {
        "files": [
          {
            "issues": [
    ...
    value.prepend([
      at(1, 'turn/start', { turn: 1 }),
      call(2, 'probe', 'gongchuang_artifact_probe', { artifactPath: '/qa/catalog.docx' }),
      result(3, 'probe'),
    ], false)
    value.flush()
    expect(selectProducedFileEntries(tailOwner(deliverablesOf(value), 12))).toEqual(expected)
  ...
  it('keeps multiple partial PTC publications in their own turn and excludes failed probes', () => {
    const dispatch = (seq: number, path: string, turn = 1, isError = false) => at(seq, 'tool/code-dispatch', {
      rootCallId: 'code', parentCallId: 'code', subCallId: `code:${seq}`, turn, step: 1,
      name: 'gongchuang_artifact_probe', arguments: { artifactPath: path }, isError, content: [],
    })
    const entries = [
      dispatch(10, '/qa/one.docx'), dispatch(11, '/qa/two.pdf'), dispatch(12, '/qa/failed.docx', 1, true),
      at(13, 'turn/end', { turn: 1, reason: { kind: 'completed' } }),
      at(14, 'turn/start', { turn: 2 }), dispatch(15, '/qa/other.xlsx', 2),
    ]
    const value = assembler(entries, true)
    expect(producedForClosing(deliverablesOf(value))).toEqual(['/qa/one.docx', '/qa/two.pdf'])
    expect(producedForClosing(deliverablesOf(value, 2))).toEqual(['/qa/other.xlsx'])
    value.prepend([at(1, 'turn/start', { turn: 1 })], false)
    value.flush()
    expect(producedForClosing(deliverablesOf(value))).toEqual(['/qa/one.docx', '/qa/two.pdf'])
  })

  it('retains text-only drafts without a loaded turn start and rejects untrusted fallback metadata', () => {
    const source = { kind: 'plugin', plugin: 'gongchuang-policy-gate', form: 'notice', delivery: {
      turn: 1, phase: 'draft', files: [], issues: ['e5 原文不匹配'], draftText: '保留的修改文案',
    } }
    const value = assembler([
      at(10, 'user/message', { content: [], source }),
      at(11, 'turn/end', { turn: 1, reason: { kind: 'blocked' } }),
    ], true)
    expect(selectTurnDeliverables(tailOwner(deliverablesOf(value), 11))).toEqual({
      files: [], issues: ['e5 原文不匹配'], draftText: '保留的修改文案',
    })
    const untrusted = assembler([
      at(10, 'user/message', { content: [], source: { ...source, kind: 'user' } }),
      result(11, 'unloaded-probe'),
    ], true)
    expect(selectTurnDeliverables(tailOwner(deliverablesOf(untrusted), 11))).toBeNull()
  })
...
describe('ProducedFiles row', () => {
  ...
  it('shows and copies a Host-retained text draft without manufacturing a file or success state', async () => {
    const draftText = '## 修改后的文案\n\nRD11 使用环境条件控制降解周期，具体指标待核验。'
    const owner = tailOwner({ produced: [], classification: {
      turn: 1, phase: 'draft', files: [], issues: ['e5 原文片段不匹配'], draftText,
    } }, 9)
    const matched = selectTurnDeliverables(owner)
    expect(matched).not.toBeNull()
    expect(selectProducedFiles(owner)).toBeNull()
    const writeText = vi.fn().mockResolvedValue(undefined)
    vi.stubGlobal('navigator', { clipboard: { writeText } })
    if (matched === null) throw new Error('draft not selected')
    const view = render(<TurnDeliverables matched={matched} openFile={vi.fn()} {...capability(true)} t={t} />)
    expect(view.getByRole('region', { name: '待核验文案' })).toBeTruthy()
    expect(view.getByText('e5 原文片段不匹配').closest('details')?.open).toBe(true)
    expect(view.getByText('RD11 使用环境条件控制降解周期，具体指标待核验。')).toBeTruthy()
    expect(view.queryByText('已生成文件')).toBeNull()
    expect(view.queryByText('已通过')).toBeNull()
    fireEvent.click(view.getByRole('button', { name: '复制文案' }))
    await vi.waitFor(() => { expect(writeText).toHaveBeenCalledWith(draftText) })
    expect(selectTurnDeliverables(tailOwner({ produced: [], classification: {
      turn: 1, phase: 'formal', files: [], issues: [], draftText,
    } }, 9))).toBeNull()
  })

  it('shows a concise draft state and expandable issues while opening the marked copy', () => {
    const openFile = vi.fn()
    const view = render(<ProducedFiles matched={[{
      path: 'report（待完善）.docx', phase: 'draft', issues: ['缺少政策来源'],
    }]} openFile={openFile} {...capability(true)} t={t} />)
    expect(view.getByText('待完善')).toBeTruthy()
    expect(view.getByText('检查结果').closest('details')?.open).toBe(false)
    fireEvent.click(view.getByRole('button', { name: '打开 report（待完善）.docx' }))
    expect(openFile).toHaveBeenCalledWith('report（待完善）.docx')
    expect(view.queryByText('已通过')).toBeNull()
  })

  it('renders full file cards, expands the remaining files, and opens the system app or folder', () => {
    ...
    expect(view.getByText('已生成文件')).toBeTruthy()
    const row = view.container.querySelector('[data-produced-files-row]')
    if (!(row instanceof HTMLElement)) throw new Error('produced row missing')
    expect(row.querySelectorAll('[data-file-card]')).toHaveLength(6)
    expect(within(row).getByText('+ 2 个文件')).toBeTruthy()
    const chip = view.getByRole('button', { name: '打开 deep/a.html' })
    expect(within(chip).getByText('a.html')).toBeTruthy()
    expect(chip.textContent).toContain('HTML')
    ...
    fireEvent.click(within(row).getByRole('button', { name: '+ 2 个文件' }))
    expect(row.querySelectorAll('[data-file-card]')).toHaveLength(8)
    fireEvent.click(view.getByRole('button', { name: '打开 h.ts' }))
    expect(openFile).toHaveBeenLastCalledWith('h.ts')
  ...
  it('keeps the group folder action absent for one file or without a local native opener', () => {
    const openFile = vi.fn<(path: string) => void>()
    const view = render(
