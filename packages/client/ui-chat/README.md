...
Source annotation markers occupy a narrow side column and stick within their own reply while it scrolls. A submission ordinal distinguishes duplicate numbers across separate annotations without changing stored indices. Turn-tail owners carry the actual Session cwd so file renderers need not guess path identity.

The browser Chat target for Conversation assembly. It registers Chat event definitions and snapshot construction, supplies `useChat`, renders transcript nodes and details, and owns Chat-specific stores, actions, localization, and scroll restoration; historical image URLs resolve through the Conversation-owned per-session cache (`ctx.uiConversation.imageUrl`). Its Assistant and Turn Tail definitions fold packed historical Assistant runs without expanding their members. Steering classification retains only next-step Inbox IDs through persistent splice state; next-turn splices create no Chat Context. Local submission echoes (`SessionSnapshot.pendingSubmissions`) retain the surface selected when the submit begins: transcript echoes render at the flow tail, steering echoes render with the pending-steering marker, and queued echoes stay out of Chat. Each echo is hidden per render once a user/steering node or queue occurrence carries its prompt `rpcId`, so the handoff is atomic. A product may optionally project structured file controls from persisted user display text; Chat renders those controls above the original body and delegates activation back to the same product provider, while an absent provider preserves ordinary text exactly.
...
The optional `chatSelectionAnnotations` provider receives selected text and its settled source-row key. A product can keep numbered selections outside the draft; without that provider, Chat retains its ordinary Markdown-quote insertion. The same user-message projection renders file cards and read-only annotation disclosures for durable messages, submission echoes, and pending steering. File activation remains delegated to the product opener. [Numbered-annotation decision](../../../.agents/notes/implemented/feature/2026-09-03-gongchuang-numbered-annotations.md).

Native paragraph selections omit adjacent message actions, clocks, source-annotation controls, and the next context row's leading icons when no text in that row is selected. At the transcript end, an empty leading attachment icon in the same composer's seat is also ignored; selecting its filename or draft text is not. Prose spanning different message rows remains ineligible for one source annotation.

In the Chinese UI, English-dominant reasoning or an English first-line/streaming-tail preview uses a localized processing status without an expansion control. Chinese filenames embedded in English prose do not change that choice. Logged reasoning and the final answer remain unchanged.
