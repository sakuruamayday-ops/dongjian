...
来源注释编号占用回复右侧窄列，在所属回复范围内随滚动保持可见。不同次标注的同号项使用提交序号区分，不修改已存编号。轮次尾部拥有者传递实际会话目录，文件展示无需猜测路径身份。

Conversation 组装的浏览器 Chat target。本包注册 Chat event definition 与 snapshot 构造、提供 `useChat`、渲染 transcript node 和详情，并拥有 Chat 专属 store、action、本地化与滚动位置恢复；历史图片 URL 通过 Conversation 持有的按会话缓存（`ctx.uiConversation.imageUrl`）解析。其中 Assistant 与 Turn Tail definition 会直接 fold packed Assistant 历史 run，不展开其成员。steering 分类通过持久 splice state 只保留 next-step Inbox ID；next-turn splice 不创建 Chat Context。本地提交回显（`SessionSnapshot.pendingSubmissions`）保留提交开始时选定的区域：transcript 回显位于消息流末尾，steering 回显带 pending-steering 标记，queued 回显不进入 Chat。一旦 user/steering 节点或 queue occurrence 携带回显的 prompt `rpcId`，该回显即在同一渲染中隐藏，因此交接是原子的。产品可以按需从持久用户展示文本投影结构化文件控件；Chat 将控件渲染在用户原始正文上方，并把点击动作交回同一个产品提供方；没有提供方时，普通文本保持原样。
...
可选的 `chatSelectionAnnotations` 提供方接收选中文字及其稳定来源行标识。产品可将编号注释保存在草稿文字之外；没有该提供方时，Chat 保留普通 Markdown 引用插入。同一用户消息投影为持久消息、提交回显及待处理 steering 渲染文件卡和只读注释，文件打开仍交给产品提供方。[编号注释决策](../../../.agents/notes/implemented/feature/2026-09-03-gongchuang-numbered-annotations.zh.md)。

原生整段选区忽略相邻的消息操作、时钟、来源注释控件，以及没有选中文字的下一条上下文行前导图标。在消息流末尾，同一输入区开头的空附件图标也会被忽略，但不会忽略已选中的文件名或草稿文字。跨越不同消息正文的选区不能合并成一个来源注释。

中文界面中的主要为英文的推理内容，以及首行或流式末尾为英文的预览，显示本地化处理状态且不提供展开控件。英文正文内的中文文件名不会改变判断；推理日志和最终回复保持不变。
