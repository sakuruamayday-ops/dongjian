...
import {
  AnnotationSummary, DocumentFileIcon, FileCard, fileSizeText, JsonBlock, projectUserText, StateDot,
...
function UserMessageFiles({ files, openFile, fileActions, t }: {
  files: ReturnType<ChatNodeOwnerProps['projectUserMessage']>['files']
  openFile: ChatNodeOwnerProps['openFile']
  fileActions: ChatNodeOwnerProps['fileActions']
  t: ChatViewSlotProps['t']
}): ReactNode {
  if (files.length === 0) return null
  return (
    <div className={css.userFileRail}>
      {files.map(file => (
        <div key={file.path} className={css.userFile} data-user-file-chip={file.path}>
          <FileCard name={file.name} path={file.path} onOpen={() => { openFile(file.path) }} actions={fileActions?.(file.path)}
            labels={{
              open: t('files.open', { name: file.name }), openWith: t('files.openWith'),
              defaultApplication: t('files.default'), chooseOther: t('files.other'),
              reveal: t('files.reveal'), saveCopy: t('files.save'), loading: t('files.loading'),
              failed: t('files.failed'), file: t('files.file'),
            }} />
        </div>
      ))}
    </div>
...
function UserStyleBubble({
  content, openFile, openAttachment, fileActions, projectUserMessage, renderMessageImages, actions, pending = false, echo = false,
  referenceLabels = [], skillNames = [], previewAttachments, t,
}: {
  content: readonly unknown[]
  openFile: ChatNodeOwnerProps['openFile']
  openAttachment?: ChatNodeOwnerProps['openAttachment']
  fileActions?: ChatNodeOwnerProps['fileActions']
  projectUserMessage: ChatNodeOwnerProps['projectUserMessage']
  ...
  const showBubble = display.text !== '' || rest.length > 0
  return (
    <div
    ...
    >
      <div className={css.userStack}>
              ...
              : (
                openAttachment === undefined
                  ...
                  : <button
                    ...
                    aria-label={t('files.open', { name: attachment.file.name })}
        ...
        <UserMessageFiles files={display.files} openFile={openFile} fileActions={fileActions} t={t} />
        {display.annotations !== undefined && (
          <div data-user-annotations>
            <AnnotationSummary
              annotations={display.annotations}
              countLabel={t('annotations.count', { count: display.annotations.length })}
              itemLabel={index => t('annotations.item', { index })}
              closeLabel={t('annotations.close')}
            />
          </div>
        )}
        {showBubble && <div className={css.bubble} data-user-message-body>
          {projectUserText(display.text, referenceLabels, skillNames)}
 ...
 */
export function PendingSteeringBubble({ content, openFile, fileActions, projectUserMessage, renderMessageImages, t }: {
  content: readonly unknown[]
  openFile: ChatNodeOwnerProps['openFile']
  fileActions?: ChatNodeOwnerProps['fileActions']
  projectUserMessage: ChatNodeOwnerProps['projectUserMessage']
 ...
 */
export function PendingSubmissionBubble({ submission, openFile, fileActions, projectUserMessage, renderMessageImages, t }: {
...
export const UserMessageNodeView = memo(function UserMessageNodeView({
  node, openFile, openAttachment, fileActions, projectUserMessage, renderMessageImages, t,
