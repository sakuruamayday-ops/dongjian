...
import type { ChatTextSelection, ChatViewSlotProps } from '../contract/slots.ts'
...
const SELECTION_ACTION_EDGE = 12

interface SelectionActionState extends ChatTextSelection {
  /** Viewport anchor for the floating action. */
  left: number
  top: number
}

/** Append a Markdown quote without rewriting any existing user draft. */
export function draftWithQuotedSelection(draft: string, selectedText: string): string {
  const quote = selectedText.trim().split(/\r?\n/u).map(line => `> ${line}`).join('\n')
  if (quote === '') return draft
  const separator = draft === '' || draft.endsWith('\n\n') ? '' : draft.endsWith('\n') ? '\n' : '\n\n'
  return `${draft}${separator}${quote}\n\n`
}

/** Closest settled message row for a Selection endpoint. */
function selectionFlowItem(node: Node | null): HTMLElement | null {
  const element = node instanceof Element ? node : node?.parentElement
  return element?.closest<HTMLElement>('[data-chat-flow-kind]') ?? null
}

/** Resolve the selected side of an element boundary, including a paragraph's trailing newline. */
function selectedBoundary(container: Node, offset: number, start: boolean, column: HTMLElement): Node | null {
  let node: Node | null = container
  if (container.nodeType === Node.TEXT_NODE) {
    const edge = start ? container.textContent?.length === offset : offset === 0
    if (!edge) return container
  } else {
    const child = container.childNodes[start ? offset : offset - 1]
    if (child !== undefined) {
      node = child
      for (;;) {
        const next: ChildNode | null = start ? node.firstChild : node.lastChild
        if (next === null) break
        node = next
      }
      return node
  ...
  // Chromium can place a triple-click range at offset zero of the following
  // paragraph/message. That unselected endpoint is not a second quoted message.
  while (node !== null && node !== column && (start ? node.nextSibling : node.previousSibling) === null) node = node.parentNode
  if (node === null || node === column) return null
  node = start ? node.nextSibling : node.previousSibling
  while (node !== null && (start ? node.firstChild : node.lastChild) !== null) node = start ? node.firstChild : node.lastChild
  return node
}

/** Ignore message controls included by a native whole-paragraph gesture. */
function selectedProseBoundary(node: Node | null, start: boolean, column: HTMLElement): Node | null {
  while (node !== null) {
    const element = node instanceof Element ? node : node.parentElement
    const composer = element?.closest('[data-composer-seat]')
    if (!start && composer !== undefined && composer !== null
      && composer.closest('[data-conversation-scroll]') === column.closest('[data-conversation-scroll]')
      && (column.compareDocumentPosition(composer) & Node.DOCUMENT_POSITION_FOLLOWING) !== 0) {
      // A native triple-click at the transcript end may include the first
      // attachment icon. Only ignore that empty prefix, never a filename or draft.
      const prefix = document.createRange()
      prefix.selectNodeContents(composer)
      prefix.setEndAfter(node)
      if (prefix.toString().trim() === '') {
        node = selectedBoundary(column, column.childNodes.length, false, column)
        continue
      }
    }
    const chrome = element?.closest('[data-message-actions], [data-source-annotations]')
    if (chrome !== undefined && chrome !== null) {
      node = selectedBoundary(chrome, start ? chrome.childNodes.length : 0, start, column)
      continue
    }
    const row = selectionFlowItem(node)
    if (!start && row !== null && !isQuotableFlowItem(row)) {
      // macOS 三击可能带上下一行开头的图标；只有未选中该行文字时才退回正文。
      const prefix = document.createRange()
      prefix.selectNodeContents(row)
      prefix.setEndAfter(node)
      if (prefix.toString().trim() === '') {
        node = selectedBoundary(row, 0, false, column)
        continue
      }
    }
    return node
...
/** User/assistant prose is quotable; Tool controls and synthetic rows are not. */
function isQuotableFlowItem(item: HTMLElement): boolean {
  return item.dataset.chatFlowKind === 'user' || item.dataset.chatFlowKind === 'assistant-step'
}

/** Interactive controls never become quoted prose; visible link and code text do. */
function selectionEndpointIsControl(node: Node | null): boolean {
  const element = node instanceof Element ? node : node?.parentElement
  if ((element?.closest('button, input, textarea, select, [contenteditable="true"]') ?? null) !== null) return true
  const roleButton = element?.closest<HTMLElement>('[role="button"]') ?? null
  return roleButton !== null && !roleButton.hasAttribute('data-disclosure-row')
}
...
export function ChatView({
  useSession, useChat, useChatNode, useChatNodeProcess, useChatAnnotationMarkers,
  useSessions, useSessionPendingInteraction, useInput, inputActions,
  useStore, actions,
  renderSlot, sessionId, openFile, openAttachment, fileActions, loadOlder, loadThrough, loadImage, openView, chatScroll, forkAt,
  fileMentions, projectUserMessage, addSelectionAnnotation, useTranscriptView, useProjection, t,
}: ChatViewSlotProps) {
  ...
  const waitingForUser = useSessionPendingInteraction(s => s.has(sessionId))
  ...
  const requestOpenAttachment = useCallback((attachment: Parameters<NonNullable<typeof openAttachment>>[0]) => {
    if (openAttachment === undefined) return
  ...
  const openSelectionAction = useCallback((): void => {
    ...
    const start = selectedProseBoundary(selectedBoundary(range.startContainer, range.startOffset, true, column), true, column)
    const end = selectedProseBoundary(selectedBoundary(range.endContainer, range.endOffset, false, column), false, column)
    if (selectionEndpointIsControl(start) || selectionEndpointIsControl(end)) {
      setSelectionAction(null)
      return
    }
    const anchorItem = selectionFlowItem(start)
    const focusItem = selectionFlowItem(end)
    if (anchorItem === null || anchorItem !== focusItem || !isQuotableFlowItem(anchorItem)) {
      setSelectionAction(null)
      return
    }
    const nodeKey = anchorItem.dataset.chatAnchorKey
    if (!column.contains(anchorItem) || nodeKey === undefined) {
      setSelectionAction(null)
      return
    }
    // Chromium may include following action icons or the clock in a triple
    // click. Quote the selected prose only, never those sibling controls.
    if (start !== range.startContainer && start !== null) range.setStartBefore(start)
    if (end !== range.endContainer && end !== null) range.setEndAfter(end)
    ...
    const rect = typeof range.getBoundingClientRect === 'function'
      ...
      : anchorItem.getBoundingClientRect()
    const viewportWidth = Math.max(document.documentElement.clientWidth, window.innerWidth)
    const viewportHeight = Math.max(document.documentElement.clientHeight, window.innerHeight)
    const left = Math.min(
      viewportWidth - SELECTION_ACTION_EDGE,
      Math.max(SELECTION_ACTION_EDGE, rect.left + rect.width / 2),
    )
    const below = rect.bottom + SELECTION_ACTION_GAP
    const top = below + SELECTION_ACTION_HEIGHT <= viewportHeight - SELECTION_ACTION_EDGE
      ? below
      : Math.max(SELECTION_ACTION_EDGE, rect.top - SELECTION_ACTION_HEIGHT - SELECTION_ACTION_GAP)
    setSelectionAction({
      text, left, top,
      source: { nodeKey, kind: anchorItem.dataset.chatFlowKind === 'user' ? 'user' : 'assistant' },
    })
  }, [])
  ...
  useEffect(() => {
    ...
    const schedule = (): void => {
      ...
      // Selection settles after the originating double-click, pointer gesture,
      // or keyboard edit. Reading on the next task avoids Chromium's
      // intermediate collapsed Selection during those native gestures.
      timer = setTimeout(openSelectionAction, 0)
    }
  ...
  const addSelectionToDraft = (): void => {
    if (selectionAction === null) return
    const handled = addSelectionAnnotation?.({ text: selectionAction.text, source: selectionAction.source })
    if (handled === true) inputActions.refreshExternalPayload?.()
    else inputActions.setDraft(draftWithQuotedSelection(draft, selectionAction.text))
    setSelectionAction(null)
    window.getSelection()?.removeAllRanges()
    requestAnimationFrame(() => {
      const local = listRef.current
      if (local === null) return
      const composer = scrollerOf(local).querySelector<HTMLElement>('[data-composer-input]')
    ...
    <div className={css.root}>
      <div ref={listRef} className={css.scroll}>
        <TurnNavigator
          ...
          t={t}
        />
        <div ref={columnRef} className={css.column} data-chat-flow="" onDoubleClick={openSelectionAction}>
          ...
          <ChatNodeList
            ...
            useChatAnnotationMarkers={useChatAnnotationMarkers}
            ...
            {...openAttachment === undefined ? {} : { openAttachment: requestOpenAttachment }}
            fileActions={fileActions}
            projectUserMessage={projectUserMessage}
          ...
          {waitingForUser
            ? <div className={css.turnStatus} role="status" aria-live="polite">{t('chat.waitingForUser')}</div>
      ...
      {selectionAction !== null && (
        ...
        >
          <button type="button" onClick={addSelectionToDraft}>{t('selection.addToConversation')}</button>
        </div>
      )}
      {fileOpenError !== null && (
        <FileOpenErrorDialog
