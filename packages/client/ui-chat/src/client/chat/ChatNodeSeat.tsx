...
import { AnnotationSummary, JsonBlock } from '@deepseek-ai/dsh-client-ui-primitives'
import type { ConversationLocationDataStore, ConversationTurnDataMap } from '@deepseek-ai/dsh-client-ui-conversation/client'
import type { ChatAnnotationMarker, ChatNodeOwnerProps, ChatViewSlotProps } from '../contract/slots.ts'
...
interface ChatNodeSeatProps extends ChatNodeOwnerProps {
  ...
  readonly useChatAnnotationMarkers: ChatViewSlotProps['useChatAnnotationMarkers']
...
const NO_ANNOTATIONS: readonly ChatAnnotationMarker[] = Object.freeze([])
...
export const ChatNodeSeat = memo(function ChatNodeSeat({
  nodeKey, useChatNode, useChatNodeProcess, useChatAnnotationMarkers, historyIncomplete, compactTranscript,
  selectedCallId, cwd, openFile, openAttachment, fileActions, projectUserMessage, inspectCall, forkAt,
  loadImage, renderMessageImages, fileMentions, useStore, actions, renderSlot, t,
}: ChatNodeSeatProps) {
  const node = useChatNode(nodeKey)
  const annotations = useChatAnnotationMarkers(nodeKey) ?? NO_ANNOTATIONS
  ...
  const owner = useMemo<ChatNodeOwnerProps | null>(() => node === undefined
    ...
    node, selectedCallId, cwd, openFile, openAttachment, fileActions, projectUserMessage, inspectCall, forkAt,
  ...
  return (
    <div
      ...
      data-chat-turn={turn}
      data-has-annotations={annotations.length > 0 || undefined}
    ...
    >
      {annotations.length > 0 && <div className={css.sourceAnnotations} data-source-annotations>
        {annotations.map(annotation => <AnnotationSummary
          key={annotation.id}
          variant="marker"
          annotations={[annotation]}
          countLabel={annotation.round === undefined ? String(annotation.index) : `${String(annotation.round)}.${String(annotation.index)}`}
          itemLabel={index => annotation.round === undefined
            ? t('annotations.item', { index })
            : t('annotations.roundItem', { index, round: annotation.round })}
