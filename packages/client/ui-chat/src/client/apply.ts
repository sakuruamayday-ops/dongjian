...
import type {
  ...
  TurnTailOwnerProps, ChatAnnotationMarker,
...
const EMPTY_MARKERS: readonly ChatAnnotationMarker[] = Object.freeze([])
const EMPTY_MARKER_SOURCE = { getSnapshot: () => EMPTY_MARKERS, subscribe: () => () => {} }
  ...
  ctx.slots.inject('conversation.view', () => {
    const disposeView = ctx.slots.register({
      ...
      inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
        ...
        const userMessageFiles = ctx.get('chatUserMessageFiles')
        const openAttachment = userMessageFiles?.openAttachment?.bind(userMessageFiles, sessionId)
        return {
          hooks: { transcriptView: transcriptView.mode },
          keyedHooks: {
            chatNode: key => chat.getSnapshot().nodes.source(key),
            chatNodeProcess: key => chat.getSnapshot().nodes.processSource(key),
            chatAnnotationMarkers: key => ctx.get('chatSelectionAnnotations')?.source(sessionId, key) ?? EMPTY_MARKER_SOURCE,
          },
          openDetails: (target) => {
          ...
          projectUserMessage: text => ctx.get('chatUserMessageFiles')?.project(text) ?? {
            text,
            files: [],
          },
          ...
          addSelectionAnnotation: (selection) => {
            const annotations = ctx.get('chatSelectionAnnotations')
            if (annotations === undefined) return false
            annotations.add(sessionId, selection)
            return true
          },
          fileActions: path => ctx.get('chatUserMessageFiles')?.actions?.(sessionId, path),
          openFile: async (path) => {
            if (await ctx.get('chatUserMessageFiles')?.open(sessionId, path) === true) return
