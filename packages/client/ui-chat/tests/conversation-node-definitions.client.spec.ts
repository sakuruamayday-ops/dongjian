...
describe('built-in conversation node Definitions', () => {
  ...
  it('keeps one keyed Assistant node while streaming settles and materializes interruption from Location', () => {
    ...
    expect((settled?.data as AssistantChatData).finalNode?.provenance).toEqual({
      provider: 'fake',
      model: 'fake',
    })
  ...
  it('keeps prepared context in the log while projecting the imported file before composer text', () => {
    const message = {
      ...textMessage(
        'prepared-user',
        '核对材料\n\n本次任务可读取以下企业工作区文件：\n- `共创导入资料/材料.xlsx`',
      ...
      source: {
        kind: 'user',
        rpcId: 'prepared-rpc',
        displayText: '@"共创导入资料/材料.xlsx"\n核对材料',
      },
    }
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      at(2, 'user/message', message, { surfaceOp: 'append' }),
    ])

    expect(node(snapshot(value), 'user')?.data).toMatchObject({
      content: [{ type: 'text', text: '@"共创导入资料/材料.xlsx"\n核对材料' }],
  ...
  it('projects an attachment-only prompt as a file reference instead of an empty message', () => {
    const message = {
      ...textMessage(
        'prepared-document-only',
        '本次任务可读取以下企业工作区文件：\n- `共创导入资料/申报材料.xlsx`',
      ),
      source: {
        kind: 'user',
        rpcId: 'prepared-document-only-rpc',
    ...
    expect(node(snapshot(value), 'user')?.data).toMatchObject({
      content: [{ type: 'text', text: '@"共创导入资料/申报材料.xlsx"' }],
      source: message.source,
    })
  })
