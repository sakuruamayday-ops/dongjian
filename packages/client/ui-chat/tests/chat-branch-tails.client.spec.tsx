...
import type { ChatNodeViewProps, UserMessageDisplay } from '../src/client/contract/slots.ts'
import {
...
interface MessageItemProps {
  ...
  readonly openAttachment?: ChatNodeViewProps['openAttachment']
  readonly projectUserMessage?: ChatNodeViewProps['projectUserMessage']
}
...
function MessageItem({
  ...
  openFile = vi.fn(),
  openAttachment,
  projectUserMessage = text => ({ text, files: [] }),
}: MessageItemProps) {
  ...
  const props = {
    node: viewNode, t: translate, openFile, openAttachment, projectUserMessage, renderMessageImages,
...
describe('MessageItem arms', () => {
  ...
  it('renders imported documents above the body, opens them, and copies only user text', () => {
    ...
    const view = render(
      <MessageItem
        t={t}
        openFile={openFile}
        projectUserMessage={(text): UserMessageDisplay => ({
          text: text.split('\n').slice(2).join('\n'),
          files: [
            { path: '共创导入资料/申请书.docx', name: '申请书.docx' },
            { path: '共创导入资料/审计报告.pdf', name: '审计报告.pdf' },
          ],
        })}
        node={{
    ...
    const chips = [...view.container.querySelectorAll('[data-user-file-chip]')]
    expect(chips.map(chip => chip.querySelector('button')?.getAttribute('aria-label'))).toEqual(['打开 申请书.docx', '打开 审计报告.pdf'])
    const body = view.container.querySelector('[data-user-message-body]')!
    expect(body.textContent).toBe('检查这些材料')
    expect(chips[0]!.compareDocumentPosition(body) & Node.DOCUMENT_POSITION_FOLLOWING).not.toBe(0)

    fireEvent.click(view.getByRole('button', { name: '打开 申请书.docx' }))
    expect(openFile).toHaveBeenCalledWith('共创导入资料/申请书.docx')
    fireEvent.click(view.getByRole('button', { name: '复制' }))
    expect(writeText).toHaveBeenCalledWith('检查这些材料')
  })

  it('renders an attachment-only message without an empty text bubble', () => {
    const view = render(
      <MessageItem
        t={t}
        projectUserMessage={(): UserMessageDisplay => ({
          text: '',
          files: [{ path: '共创导入资料/材料.xlsx', name: '材料.xlsx' }],
        })}
        node={{
          kind: 'user', seq: 1, time: 1_000,
          content: [{ type: 'text', text: '@"共创导入资料/材料.xlsx"' }] as never,
          source: null,
        }}
      />,
    )
    expect(view.getByRole('button', { name: '打开 材料.xlsx' })).toBeTruthy()
    expect(view.container.querySelector('[data-user-message-body]')).toBeNull()
  })

  it('opens a durable uploaded file from its message card when the Host operation is mounted', () => {
    const openAttachment = vi.fn()
    const attachment = { attachmentId: 'attachment-1', name: 'notes.pdf', bytes: 42 }
    const view = render(
      <MessageItem
        t={t}
        ...
        node={{
          kind: 'user', seq: 1, time: 1_000,
          content: [{ type: 'file', attachment }] as never,
          source: null,
        }}
      />,
    )

    fireEvent.click(view.getByRole('button', { name: '打开 notes.pdf' }))
    expect(openAttachment).toHaveBeenCalledExactlyOnceWith(attachment)
  })
