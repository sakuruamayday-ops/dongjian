...
import {
  bindSnapshotSelector, makeTranslate, workspaceSnapshot,
...
import { ChatView, draftWithQuotedSelection } from '../src/client/chat/ChatView.tsx'
...
import type { ChatAnnotationMarker } from '../src/client/contract/slots.ts'
...
function emptyWorkspaces() {
  const store = createSnapshotStore<WorkspaceSnapshot>(workspaceSnapshot())
  return bindSnapshotSelector(store)
}
...
function makeHarness(
  ...
  initialDraft = '',
  projectUserMessage: ChatNodeOwnerProps['projectUserMessage'] = text => ({ text, files: [] }),
) {
  const {
  ...
  const setDraft = vi.fn((draft: string) => {
    ...
    input.set({ ...current, draft, draftRev: current.draftRev + 1 })
  })
  const submit = vi.fn()
  ...
  return {
    ...
    chatScroll, forkAt, setSelection, toolOwners, setDraft, submit,
...
/** Product-like fixture projection; ui-chat itself remains unaware of product path conventions. */
const projectImportedFixture: ChatNodeOwnerProps['projectUserMessage'] = (text) => {
  const match = /^@"(共创导入资料\/([^"/]+))"\n?/u.exec(text)
  return match === null
    ? { text, files: [] }
    : {
      text: text.slice(match[0].length),
      files: [{ path: match[1] as string, name: match[2] as string }],
    }
}
...
describe('ChatView', () => {
  it('hands selections to the product annotation owner without rewriting or submitting the draft', async () => {
    const h = makeHarness({ nodes: [assistant(2, '保留原文作为注释')] }, {}, undefined, '请看注释 1')
    const addSelectionAnnotation = vi.fn(() => true)
    const refreshExternalPayload = vi.fn()
    h.props.addSelectionAnnotation = addSelectionAnnotation
    h.props.inputActions.refreshExternalPayload = refreshExternalPayload
    const view = render(<h.ChatView {...h.props} />)
    const message = view.getByText('保留原文作为注释')
    ...
    expect(addSelectionAnnotation).toHaveBeenCalledExactlyOnceWith({
      text: '保留原文作为注释', source: {
        nodeKey: message.closest<HTMLElement>('[data-chat-anchor-key]')!.dataset.chatAnchorKey,
        kind: 'assistant',
      },
    })
    expect(h.setDraft).not.toHaveBeenCalled()
    expect(h.submit).not.toHaveBeenCalled()
    expect(refreshExternalPayload).toHaveBeenCalledOnce()
  })

  it('renders sent annotations separately from prose while file cards keep their system opener', () => {
    const h = makeHarness({ nodes: [user(1, 'persisted display payload')] }, {}, undefined, '', () => ({
      text: '请看注释 1', files: [{ path: '共创导入资料/申请书.docx', name: '申请书.docx' }],
      annotations: [{ index: 1, text: '缺少第七章', comment: '这是为什么？', source: { nodeKey: 'assistant-step:2', kind: 'assistant' } }],
    }))
    const view = render(<h.ChatView {...h.props} />)
    expect(view.queryByText('persisted display payload')).toBeNull()
    expect(view.getByText('请看注释 1').closest('[data-user-message-body]')).not.toBeNull()
    fireEvent.click(view.getByRole('button', { name: '打开 申请书.docx' }))
    expect(h.openFile).toHaveBeenCalledExactlyOnceWith('共创导入资料/申请书.docx')
    expect(h.openView).not.toHaveBeenCalled()
    fireEvent.click(view.getByRole('button', { name: '1 条注释' }))
    expect(screen.getByRole('dialog', { name: '1 条注释' })).toBeTruthy()
    expect(screen.getByText('缺少第七章')).toBeTruthy()
    expect(screen.getByText('这是为什么？')).toBeTruthy()
    expect(screen.queryByRole('textbox')).toBeNull()
  })

  it('formats selected lines as a quote without rewriting an existing draft', () => {
    expect(draftWithQuotedSelection('', '第一行\n第二行')).toBe('> 第一行\n> 第二行\n\n')
    expect(draftWithQuotedSelection('请继续分析', '关键结论')).toBe('请继续分析\n\n> 关键结论\n\n')
  })

  it('adds one native selection to the existing draft without sending it', async () => {
    const h = makeHarness({ nodes: [assistant(2, '保留这段关键结论')] }, {}, undefined, '请继续分析')
    const view = render(
      <div data-conversation-scroll="">
        <h.ChatView {...h.props} />
        <div data-composer-input="" contentEditable role="textbox" aria-label="composer" />
      </div>,
    )
    const message = view.getByText('保留这段关键结论')
    const range = document.createRange()
    range.selectNodeContents(message)
    Object.defineProperty(range, 'getBoundingClientRect', {
      value: () => ({ left: 120, right: 240, top: 80, bottom: 100, width: 120, height: 20 } as DOMRect),
    })
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(range)

    fireEvent.doubleClick(message)
    const action = await screen.findByRole('button', { name: '添加到对话' })
    expect(action.closest('[role="toolbar"]')?.getAttribute('aria-label')).toBe('选中文字操作')
    fireEvent.click(action)

    expect(h.setDraft).toHaveBeenCalledWith('请继续分析\n\n> 保留这段关键结论\n\n')
    expect(h.submit).not.toHaveBeenCalled()
    await waitFor(() => {
      expect(document.activeElement).toBe(view.getByRole('textbox', { name: 'composer' }))
    })
  })

  it('opens from native selection changes for visible link and code text', async () => {
    const h = makeHarness({ nodes: [assistant(2, '查看[政策原文](https://example.com)与`关键指标`')] })
    const view = render(<h.ChatView {...h.props} />)

    for (const text of ['政策原文', '关键指标']) {
      const prose = view.getByText(text)
      const range = document.createRange()
      range.selectNodeContents(prose)
      Object.defineProperty(range, 'getBoundingClientRect', {
        value: () => ({ left: 100, right: 180, top: 50, bottom: 70, width: 80, height: 20 } as DOMRect),
      })
      window.getSelection()?.removeAllRanges()
      window.getSelection()?.addRange(range)
      document.dispatchEvent(new Event('selectionchange'))
      expect(await screen.findByRole('button', { name: '添加到对话' })).toBeTruthy()
      fireEvent.keyDown(window, { key: 'Escape' })
      expect(screen.queryByRole('button', { name: '添加到对话' })).toBeNull()
    }
  })

  it('quotes visible reasoning disclosure text instead of treating its row as a control', async () => {
    const h = makeHarness({ nodes: [reasoningAssistant(2, '观察企业画像判定结果')] })
    const view = render(<h.ChatView {...h.props} />)
    const summary = view.getByText('观察企业画像判定结果')
    const range = document.createRange()
    range.selectNodeContents(summary)
    Object.defineProperty(range, 'getBoundingClientRect', {
      value: () => ({ left: 100, right: 220, top: 50, bottom: 70, width: 120, height: 20 } as DOMRect),
    })
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(range)
    ...
    expect(h.setDraft).toHaveBeenCalledWith('> 观察企业画像判定结果\n\n')
    expect(h.submit).not.toHaveBeenCalled()
  })

  it('does not offer a native selection that crosses message rows', async () => {
    const h = makeHarness({ nodes: [assistant(2, '第一条消息'), assistant(3, '第二条消息')] })
    const view = render(<h.ChatView {...h.props} />)
    const first = view.getByText('第一条消息')
    const second = view.getByText('第二条消息')
    const range = document.createRange()
  ...
  it('offers a whole-paragraph selection ending at offset zero of the following message', async () => {
    const h = makeHarness({ nodes: [assistant(2, '整段选中的内容'), assistant(3, '下一条消息')] })
    const view = render(<h.ChatView {...h.props} />)
    const first = view.getByText('整段选中的内容')
    const next = view.getByText('下一条消息')
    const range = document.createRange()
    range.setStart(first.firstChild!, 0)
    range.setEnd(next.firstChild!, 0)
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(range)
    fireEvent.mouseUp(first, { detail: 3 })
    fireEvent.click(await screen.findByRole('button', { name: '添加到对话' }))
    expect(h.setDraft).toHaveBeenCalledWith('> 整段选中的内容\n\n')
  })

  it.each(['icon', 'clock'] as const)('quotes the final paragraph when native selection includes trailing action %s', async (endpoint) => {
    const h = makeHarness({
      nodes: [assistant(2, '整段回复末尾')],
      turnEnds: new Map([[1, 3]]),
    })
    const view = render(<h.ChatView {...h.props} />)
    const prose = view.getByText('整段回复末尾')
    const tail = view.container.querySelector<HTMLElement>('[data-turn-tail]')!
    const copy = within(tail).getByRole('button', { name: '复制' })
    const end = endpoint === 'icon' ? copy.querySelector('svg')! : tail.querySelector('time, [class*="timeEnd"]')!
    const range = document.createRange()
    range.setStart(prose.firstChild!, 0)
    range.setEndAfter(end)
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(range)
    fireEvent.mouseUp(prose, { detail: 3 })
    fireEvent.click(await screen.findByRole('button', { name: '添加到对话' }))
    expect(h.setDraft).toHaveBeenCalledWith('> 整段回复末尾\n\n')
  })

  it('does not turn a selection limited to the message clock into a quotation', async () => {
    const h = makeHarness({ nodes: [assistant(2, '原回复')], turnEnds: new Map([[1, 3]]) })
    const view = render(<h.ChatView {...h.props} />)
    const clock = view.container.querySelector<HTMLElement>('[data-turn-tail] [class*="timeEnd"]')!
    const range = document.createRange()
  ...
  it.each(['leading-icon', 'file-name', 'draft-text'] as const)(
    'handles a native paragraph selection reaching composer %s without quoting draft data', async (endpoint) => {
      const h = makeHarness({ nodes: [assistant(2, '末段合成回复')], turnEnds: new Map([[1, 3]]) })
      const view = render(
        <div data-conversation-scroll="">
          <h.ChatView {...h.props} />
          <div data-composer-seat="">
            <button type="button" aria-label="打开附件"><svg data-testid="leading-icon" /><span>synthetic.docx</span></button>
            <div data-composer-input="" contentEditable role="textbox">未发送文字</div>
          </div>
        </div>,
      )
      const prose = view.getByText('末段合成回复')
      const end = endpoint === 'leading-icon' ? view.getByTestId('leading-icon')
        : endpoint === 'file-name' ? view.getByText('synthetic.docx') : view.getByRole('textbox')
      const range = document.createRange()
      range.setStart(prose.firstChild!, 0)
      range.setEndAfter(end)
      window.getSelection()?.removeAllRanges()
      window.getSelection()?.addRange(range)
      fireEvent.mouseUp(prose, { detail: 3 })
      if (endpoint === 'leading-icon') {
        fireEvent.click(await screen.findByRole('button', { name: '添加到对话' }))
        expect(h.setDraft).toHaveBeenCalledWith('> 末段合成回复\n\n')
      } else {
        await new Promise(resolve => setTimeout(resolve, 10))
        expect(screen.queryByRole('button', { name: '添加到对话' })).toBeNull()
      }
  ...
  it('keeps a paragraph quote when macOS includes the next context row leading icons', async () => {
    const h = makeHarness({ nodes: [user(1, '整段用户消息'), context(2, '工作区指令', 1)] })
    const view = render(<h.ChatView {...h.props} />)
    const prose = view.getByText('整段用户消息')
    const icon = view.container.querySelector('[data-chat-flow-kind="context"] svg')!
    const range = document.createRange()
    range.setStart(prose.firstChild!, 0)
    ...
    fireEvent.mouseUp(prose, { detail: 3 })
    fireEvent.click(await screen.findByRole('button', { name: '添加到对话' }))
    expect(h.setDraft).toHaveBeenCalledWith('> 整段用户消息\n\n')
  })

  it('rejects a selection extending into the following context label', async () => {
    const h = makeHarness({ nodes: [user(1, '用户消息'), context(2, '工作区指令', 1)] })
    const view = render(<h.ChatView {...h.props} />)
    const prose = view.getByText('用户消息')
    const contextRow = view.getByRole('button', { name: '上下文注入' })
    const range = document.createRange()
    range.setStart(prose.firstChild!, 0)
    range.setEndAfter(contextRow)
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(range)
    fireEvent.mouseUp(prose, { detail: 3 })
    await new Promise(resolve => setTimeout(resolve, 10))
  ...
  it('places the numbered marker on its source message and opens the matching quotation', () => {
    const h = makeHarness({ nodes: [assistant(2, '第一条原文'), assistant(3, '被标注原文')] })
    const markers = createSnapshotStore<readonly ChatAnnotationMarker[]>([{
      id: 'message:4:1', index: 1, text: '被标注原文', comment: '请核对这句话',
      source: { nodeKey: 'fixture:assistant:3', kind: 'assistant' },
    }])
    const emptyMarkers = createSnapshotStore<readonly ChatAnnotationMarker[]>([])
    h.props.useChatAnnotationMarkers = bindKeyedSnapshotSelector(
      key => key === 'fixture:assistant:3' ? markers : emptyMarkers,
    )
    const view = render(<h.ChatView {...h.props} />)
    const marker = view.getByRole('button', { name: '注释 1' })
    expect(marker.closest('[data-chat-anchor-key]')?.getAttribute('data-chat-anchor-key')).toBe('fixture:assistant:3')
    expect(marker.textContent).toBe('1')
    fireEvent.click(marker)
    expect(within(screen.getByRole('dialog')).getByText('请核对这句话')).toBeTruthy()
  })

  it('labels a pending user decision instead of claiming the model is still thinking', () => {
    const h = makeHarness({ nodes: [assistant(2, '请选择一种报告类型')], running: true })
    h.props.useSessionPendingInteraction = selector => selector(new Map([[SID, { kind: 'question' } as never]]))
    const view = render(<h.ChatView {...h.props} />)
    expect(view.getByRole('status').textContent).toBe('等待你回复')
    expect(view.queryByText('深度求索中...')).toBeNull()
  })

  it('dismisses the selection action with Escape and never offers it for tool rows', async () => {
    const h = makeHarness({ nodes: [assistant(2, '可引用内容'), toolResult(3, 'tool-selection')] })
    const view = render(<h.ChatView {...h.props} />)
    const message = view.getByText('可引用内容')
    const range = document.createRange()
    range.selectNodeContents(message)
    Object.defineProperty(range, 'getBoundingClientRect', {
      value: () => ({ left: 100, right: 160, top: 50, bottom: 70, width: 60, height: 20 } as DOMRect),
    })
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(range)
    fireEvent.doubleClick(message)
    ...
    const tool = view.getByTestId('tool-seat-tool-selection')
    const toolRange = document.createRange()
    toolRange.selectNodeContents(tool)
    window.getSelection()?.removeAllRanges()
    window.getSelection()?.addRange(toolRange)
    fireEvent.doubleClick(tool)
    expect(screen.queryByRole('button', { name: '添加到对话' })).toBeNull()
  })
  ...
  it('renders Host-pending steering at the flow tail and hands off to the durable node', () => {
    const displayText = '@"共创导入资料/补充材料.pdf"\ninterrupt now'
    ...
    fireEvent.click(within(pendingBubble as HTMLElement).getByRole('button', { name: '打开 补充材料.pdf' }))
    expect(h.openFile).toHaveBeenCalledWith('共创导入资料/补充材料.pdf')
      ...
      h.setChat({
        nodes: [
          assistant(1, 'working'),
          {
            ...
            seq: 2, time: 2_000,
            content: [{ type: 'text', text: displayText }], source: null,
          },
        ],
      })
    })
    expect(view.getAllByText('interrupt now')).toHaveLength(1)
    expect(view.getAllByRole('button', { name: '打开 补充材料.pdf' })).toHaveLength(1)
  ...
  it('renders local submission echoes at the flow tail and swaps atomically with the durable node', () => {
    const displayText = '@"共创导入资料/申请书.docx"\n即发即显'
    const h = makeHarness(
      { nodes: [assistant(1, 'working')] },
      {
        pendingSubmissions: [
          {
            ...
            time: 5_000, text: displayText, attachments: [],
          },
        ],
      },
      undefined,
    ...
    expect(view.getByRole('button', { name: '打开 申请书.docx' }).closest('[data-submission-echo]')).not.toBeNull()
    fireEvent.click(view.getByRole('button', { name: '打开 申请书.docx' }))
    expect(h.openFile).toHaveBeenCalledWith('共创导入资料/申请书.docx')
    ...
    act(() => {
      h.setChat({
        nodes: [
          ...
          {
            kind: 'user', seq: 2, time: 2_000,
            content: [{ type: 'text', text: displayText }] as never,
            source: { kind: 'user', rpcId: 'req-1', displayText },
          },
        ],
      })
    })
    expect(view.getAllByText('即发即显')).toHaveLength(1)
    expect(view.getAllByRole('button', { name: '打开 申请书.docx' })).toHaveLength(1)
