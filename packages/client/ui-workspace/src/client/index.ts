...
export { recentWorkspace } from './navigation.ts'
export type {
  DirectoryFlowOwnerProps, DirectoryFlowSlotName, DirectoryPickingHooks, DirectoryPickingInjected,
  EnterpriseCreationOwnerProps,
...
export function apply(ctx: Context): void {
  ...
  const enterpriseCreationSource: HostObservable<boolean> = {
    getSnapshot: () => ctx.slots.entries('conversation.hero.workspace.createEnterprise').length > 0,
    subscribe: listener => ctx.slots.subscribe('conversation.hero.workspace.createEnterprise', listener),
  }
  ...
  const pickerInjected = (): WorkspacePickerInjected => ({
    createWorkspace: input => workspaces.create(input),
    hooks: { directoryFlow: pickerFlowSource, enterpriseCreation: enterpriseCreationSource },
  })
  ...
  ctx.slots.inject('conversation.hero.workspace', () => ctx.slots.register(
    {
      ...
      children: {
        ...
        'conversation.hero.workspace.createEnterprise': { kind: 'single', scope: 'root' },
      },
