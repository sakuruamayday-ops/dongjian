 ...
 * The conversation picker additionally declares one optional product action
 * hole that can replace its generic directory-add footer row with the product's
 * real enterprise-creation workflow. Two directory holes exist because the two menu surfaces are independent slot entries
...
/** Product action rendered in the Workspace picker footer. */
export interface EnterpriseCreationOwnerProps {
  /** Close the Workspace picker before opening the product-owned flow. */
  onClose: () => void
}
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /** Optional product-owned enterprise creation action in the Workspace picker. */
    'conversation.hero.workspace.createEnterprise': {
      kind: 'single'
      scope: 'root'
      owner: EnterpriseCreationOwnerProps
    }
  }
}
...
export type WorkspacePickerInjected = DirectoryPickingInjected & {
  hooks: DirectoryPickingInjected['hooks'] & {
    /** Product enterprise-creation footer action occupancy. */
    enterpriseCreation: HostObservable<boolean>
  }
  /** Adopt a picked host directory as a real Workspace before targeting a Session. */
  createWorkspace: (input: { path: string }) => Promise<WorkspaceView>
}

/** Component-side hooks for the conversation Workspace picker. */
export type WorkspacePickerHooks = PropsHooks<WorkspacePickerInjected['hooks']>
...
export type WorkspacePickerProps =
  ...
  & PropsRenderSlots<
    'conversation.hero.workspace.directoryFlow' | 'conversation.hero.workspace.createEnterprise'
  >
  & Omit<WorkspacePickerInjected, 'hooks'>
  & WorkspacePickerHooks
  & PropsLocale<'workspace'>
