 ...
 * adopts the picked path, and owns the error surface. Generic compositions
 * add a Workspace by picking a host directory; a product may replace that
 * footer row through the dedicated enterprise-creation child slot while
 * retaining the same searchable target list and unscoped action.
 */
import type { ReactNode, RefObject } from 'react'
import { useCallback, useEffect, useState } from 'react'
import {
  Button, IconFolderClose16, IconPlusOutline16, IconSearchOutline16,
...
const EMPTY_WORKSPACES = '::empty-workspaces'
...
export interface WorkspacePickFlowProps {
  ...
  /** Whether the product supplies its own enterprise-creation workflow. */
  enterpriseCreationAvailable?: boolean
  /** Product enterprise-creation menu item rendered in the pinned footer. */
  enterpriseCreationAction?: ReactNode
}
...
export function WorkspacePickFlow({
  ...
  side = 'bottom',
  selectedId,
  enterpriseCreationAvailable = false,
  enterpriseCreationAction,
  ...
  const visibleWorkspaces = normalizedQuery === ''
    ? workspaces
    : workspaces.filter(workspace => workspace.title.toLocaleLowerCase().includes(normalizedQuery))
  ...
  const addEntries: MenuEntry[] = flowAvailable && !enterpriseCreationAvailable
    ? [{ id: ADD_WORKSPACE, label: t('menu.addWorkspace'), icon: <IconPlusOutline16 size={16} />, disabled: flowBusy }]
    : []
  // Conversation pickers pin their creation and unscoped actions below the
  // searchable Workspace viewport. The browser's add-only surface keeps its
  // one-action direct-open behavior.
  const pinActions = !addOnly
    && (enterpriseCreationAvailable || addEntries.length > 0)
  const workspaceEntries: MenuEntry[] = visibleWorkspaces.length === 0
    ? [{ type: 'label', id: EMPTY_WORKSPACES, text: t('picker.empty') }]
  ...
  const items: MenuEntry[] = addOnly ? addEntries : workspaceEntries
  const footerEntries = pinActions ? addEntries : undefined
  ...
  const menuIsEmpty = items.length === 0
    && (footerEntries?.length ?? 0) === 0
    && enterpriseCreationAction === undefined
  ...
  const addIsTheOnlyEntry = addOnly && listSettled && addEntries.length === 1
  ...
  const search = !addOnly ? <label className={css.menuSearch}>
    ...
    <input
      ...
      onKeyDown={(event) => {
        ...
        const first = event.currentTarget.closest('[role="menu"]')
  ...
  return (
    <>
      <Menu
        ...
        anchor={null}
        items={items}
        {...search === undefined ? {} : { header: search }}
        {...footerEntries === undefined ? {} : { footer: footerEntries }}
        {...enterpriseCreationAction === undefined ? {} : { footerContent: enterpriseCreationAction }}
...
export function WorkspacePicker({
  ...
  createWorkspace,
  useDirectoryFlow,
  useEnterpriseCreation,
  renderSlot,
  t,
}: WorkspacePickerProps) {
  const enterpriseCreationAvailable = useEnterpriseCreation(occupied => occupied)
  return (
    <WorkspacePickFlow
      t={t}
      open={open}
      ...
      enterpriseCreationAvailable={enterpriseCreationAvailable}
      enterpriseCreationAction={enterpriseCreationAvailable
        ? renderSlot('conversation.hero.workspace.createEnterprise', { onClose })
        : undefined}
      selectedId={selectedId}
      onPick={onPick}
      onClose={onClose}
