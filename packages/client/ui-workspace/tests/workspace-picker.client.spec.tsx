...
import {
  bindSnapshotSelector, makeTranslate, workspaceSnapshot as workspaceFixture,
  workspaceView as workspaceViewFixture,
...
function workspace(id: string, title = id): WorkspaceView {
  return workspaceViewFixture({ workspaceId: wid(id), path: `/projects/${id}`, title })
}
...
function mount(
  ...
  const renderPicker = (nextItems: readonly WorkspaceView[]) => (
    <WorkspacePicker
      ...
      useEnterpriseCreation={noEnterpriseCreation}
      onPick={onPick}
      onClose={onClose}
...
function chooseAdd(): void {
  fireEvent.click(screen.getByRole('menuitem', { name: '新建企业空间' }))
}
...
describe('WorkspacePicker', () => {
  ...
  it('keeps the creation action in the pinned footer when no Workspace exists', () => {
    const b = mount([])
  ...
  it('disables every menu action from flow open through adoption, and reports busy to the flow', async () => {
    ...
    expect(screen.getByRole<HTMLButtonElement>('menuitem', { name: '新建企业空间' }).disabled).toBe(true)
  ...
  it('keeps the menu up while the list baseline is still in flight', () => {
    ...
    expect(screen.getByRole('status').textContent).toBe('正在加载企业空间…')
    expect(screen.queryByTestId('directory-flow')).toBeNull()
    expect(screen.getByRole('menuitem', { name: '新建企业空间' })).toBeTruthy()
  })

  it('shows an empty searchable list when nothing is listed and nothing can be added', () => {
    const b = mount([], vi.fn(), occupancySource(false))
    expect(screen.getByRole('menu')).toBeTruthy()
    expect(screen.getByText('无')).toBeTruthy()
    expect(screen.queryByRole('menuitem', { name: '新建企业空间' })).toBeNull()
    expect(screen.queryByTestId('directory-flow')).toBeNull()
    expect(b.createWorkspace).not.toHaveBeenCalled()
  })
  ...
  it('holds the anchor gesture while an adoption is still settling', async () => {
    ...
    act(() => { b.probe.owner!.onPicked('/tmp/project') })
    expect(b.probe.owner!.busy).toBe(true)
    // The list can empty under the still-settling adoption without reopening
    // the pinned creation action or starting a second adoption.
    act(() => { b.rerenderItems([]) })
