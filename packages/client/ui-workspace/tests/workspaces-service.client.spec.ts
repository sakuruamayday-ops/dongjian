...
import {
  workspaceSnapshot as workspaceFixture, workspaceView as workspaceViewFixture,
...
function workspace(
  ...
  return workspaceViewFixture({
    workspaceId: wid(id), path: `/w/${id}`, title: id, sessionIds, createdAt, updatedAt: createdAt,
  })
}
...
function workspaceState(
  ...
  return {
    ...workspaceFixture(), items, archivedSessionIds,
    phase,
...
class FakeWorkspaces implements IWorkspaces {
  ...
  declare readonly deleteWorkspace: IWorkspaces['deleteWorkspace']
...
describe('UiWorkspaceService', () => {
  ...
  it('clears an explicitly hidden current Session and preserves an Ungrouped current Session', () => {
    const current = summary('current')
    const idle = summary('idle')
    const owningWorkspace = workspace('one', [current.id, idle.id])
    const b = bench({
      sessions: sessionState([current, idle], current.id),
      workspaces: workspaceState([owningWorkspace]),
    })
    ...
    b.workspaces.list.update(state => ({
      ...state,
      archivedSessionIds: [],
      deletedSessionIds: [idle.id],
    }))
    expect(b.sessions.clear).toHaveBeenCalledTimes(2)
    expect(b.sessions.list.getSnapshot().current).toBeUndefined()

    b.sessions.open(current.id)
    b.workspaces.list.set({
      ...workspaceState([]),
      archivedItems: [owningWorkspace],
      archivedWorkspaceIds: [owningWorkspace.workspaceId],
    })
    expect(b.sessions.clear).toHaveBeenCalledTimes(3)

    b.workspaces.list.set(workspaceState([owningWorkspace]))
    b.sessions.open(current.id)
    b.workspaces.list.set({
      ...workspaceState([]),
      deletedWorkspaceIds: [owningWorkspace.workspaceId],
    })
    expect(b.sessions.clear).toHaveBeenCalledTimes(3)
    ...
    const archived = bench({
      ...
      workspaces: workspaceState([owningWorkspace], [current.id]),
    })
    expect(archived.sessions.clear).toHaveBeenCalledOnce()
  })

  it('waits for both baselines and keeps a current Session in an active Workspace path', () => {
    const current = summary('current', { cwd: '/w/one' })
    const b = bench({
      sessions: sessionState([current], current.id),
    ...
    expect(b.sessions.clear).not.toHaveBeenCalled()
    b.workspaces.list.set(workspaceState([workspace('one')]))
    expect(b.sessions.clear).not.toHaveBeenCalled()
    expect(b.sessions.list.getSnapshot().current).toBe(current.id)
  })
