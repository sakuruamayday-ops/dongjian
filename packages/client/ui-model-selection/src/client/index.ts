...
export type { ModelCatalogRefreshService } from './refresh.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.plugin(ModelDirectoryResolver, {
    ...
    emptyCatalogReason: () => t('error.emptyRefresh'),
  })
    ...
    scope.slots.inject('conversation.input.model', () => scope.slots.register({
      ...
      inject: (sessionId): ModelSelectInjected => {
        ...
        return {
          ...
          refresh: async () => {
            if (!available) return
            // This lookup intentionally happens at click time. Generic DSH has
            // no product refresh service, while a product plugin may arrive
            // after this contribution and provide one without creating a
            // feature-package dependency in the opposite direction.
            const productRefresh = scope.get('modelCatalogRefresh')
            await directory.refresh(productRefresh === undefined
              ? undefined
