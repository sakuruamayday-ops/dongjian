...
export class ModelDirectory {
   ...
   * @param emptyCatalogReason - localized recovery copy for an explicit empty refresh.
   */
  constructor(
    ...
    private readonly emptyCatalogReason: () => string,
  ) {
  ...
  async load(): Promise<ModelDirectoryState> {
    ...
    if (this.catalog.selectionsCleared()) {
      this.syncInputs()
  ...
  /**
   * Force a new global catalog read after an explicit user refresh.
   * @param beforeReload - optional provider refresh completed before reading its catalog.
   * @returns the fresh directory value.
   */
  async refresh(beforeReload?: () => Promise<void>): Promise<ModelDirectoryState> {
    this.assertAvailable()
    this.catalog.beginExplicitReload()
    try {
      await beforeReload?.()
      // A provider refresh may itself emit catalog-change events. Invalidate
      // again after it settles so this user action always awaits a read made
      // after the provider refresh, rather than accepting an event-raced read.
      const catalog = await this.catalog.reload()
      if (catalog.groups.every(group => group.models.length === 0)) {
        throw new Error(this.emptyCatalogReason())
      }
      this.catalog.completeExplicitReload()
      this.syncInputs()
      return this.store.getSnapshot()
    } catch (error) {
      this.catalog.failExplicitReload(error)
      this.syncInputs()
      throw error
    }
  }
   ...
   * projection frame and the persisted default update as one Host-accepted
   * operation; failures surface on the store and throw so each entry's own
   * retry surface engages.
