...
async function bench() {
  ...
  let groups: readonly ModelProviderGroup[] = GROUPS
  let nextCatalogFailure: string | null = null
  ...
  const sessionRemote = {
    modelCatalog: () => {
      calls.models += 1
      if (nextCatalogFailure !== null) {
        const message = nextCatalogFailure
  ...
  return {
    ...
    setGroups: (next: readonly ModelProviderGroup[]) => { groups = next },
    failNextCatalog: (message: string) => { nextCatalogFailure = message },
...
describe('ui-model-selection dual entry', () => {
  ...
  it('reads the optional product refresher only for an explicit refresh, then reloads the global catalog', async () => {
    const b = await bench()
    b.mint('s1')
    const productRefresh = vi.fn().mockResolvedValue(undefined)
    b.ctx.provide('modelCatalogRefresh', { refresh: productRefresh })
    const face = b.seat().inject!(sid('s1'))

    face.load()
    await vi.waitFor(() => { expect(b.calls.models).toBe(1) })
    expect(productRefresh).not.toHaveBeenCalled()

    await face.select({ provider: 'deepseek-official', model: 'deepseek-v4-pro' })
    expect(productRefresh).not.toHaveBeenCalled()

    await face.refresh()
    expect(productRefresh).toHaveBeenCalledTimes(1)
    expect(b.calls.models).toBe(2)
  })

  it('clears stale rows when the product refresh rejects and recovers on retry', async () => {
    const b = await bench()
    b.mint('s1')
    const productRefresh = vi.fn()
      .mockRejectedValueOnce(new Error('provider refresh rejected'))
      .mockResolvedValueOnce(undefined)
    b.ctx.provide('modelCatalogRefresh', { refresh: productRefresh })
    const face = b.seat().inject!(sid('s1'))
    face.load()
    await vi.waitFor(() => { expect(face.directory.getSnapshot().groups).toHaveLength(1) })

    await expect(face.refresh()).rejects.toThrow('provider refresh rejected')
    ...
    await expect(b.ctx.modelDirectories.directoryFor(sid('s1')).load())
      .rejects.toThrow('provider refresh rejected')
    b.remote.emit('settings/document-updated', ['llm-deepseek', 2])
    await Promise.resolve()
    await Promise.resolve()
    expect(b.calls.models).toBe(1)
    expect(face.directory.getSnapshot()).toMatchObject({ groups: [], status: 'error' })
    ...
    await expect(face.refresh()).resolves.toBeUndefined()
    expect(productRefresh).toHaveBeenCalledTimes(2)
    expect(b.calls.models).toBe(2)
    expect(face.directory.getSnapshot()).toMatchObject({
      groups: GROUPS,
      status: 'ready',
  ...
  it('treats an empty explicit refresh as an error and lets a full refresh retry recover', async () => {
    const b = await bench()
    b.mint('s1')
    const face = b.seat().inject!(sid('s1'))
    face.load()
    ...
    await expect(face.refresh()).rejects.toThrow(zh['error.emptyRefresh'])
    expect(face.directory.getSnapshot()).toMatchObject({
      groups: [],
      failures: [],
      status: 'error',
      error: zh['error.emptyRefresh'],
    })
    await expect(b.ctx.modelDirectories.directoryFor(sid('s1')).load())
      .rejects.toThrow(zh['error.emptyRefresh'])
    expect(b.calls.models).toBe(2)
  ...
  it('clears stale rows when the post-refresh catalog reload fails and retry recovers', async () => {
    const b = await bench()
    b.mint('s1')
    const face = b.seat().inject!(sid('s1'))
    face.load()
    await vi.waitFor(() => { expect(face.directory.getSnapshot().groups).toHaveLength(1) })
    b.failNextCatalog('reload offline')

    await expect(face.refresh()).rejects.toThrow('unavailable: reload offline')
    expect(face.directory.getSnapshot()).toMatchObject({
      groups: [],
      failures: [],
      status: 'error',
      error: 'unavailable: reload offline',
    })
    await expect(b.ctx.modelDirectories.directoryFor(sid('s1')).load())
      .rejects.toThrow('unavailable: reload offline')
    expect(b.calls.models).toBe(2)
