...
async function mounted(config?: ConnectionConfig, credentialStore = new RecordCredentials()): Promise<{
  routes: WebRoute[]
  upgrades: WebUpgradeRoute[]
  connection: HostConnectionHandle
  credentialStore: RecordCredentials
  dispose: () => Promise<void>
}> {
  const ctx = new Context()
  const routes: WebRoute[] = []
  const upgrades: WebUpgradeRoute[] = []
  provideBrowserCredentials(ctx, credentialStore)
...
describe('connection node half', () => {
  it('persists browser sessions by default', async () => {
    const { credentialStore, dispose } = await mounted()
    expect(credentialStore).toMatchObject({ reads: 0, modifies: 1 })
    expect(credentialStore.record).toMatchObject({ kind: 'grant' })
    await dispose()
  })

  it('keeps process-local browser sessions out of the credential provider', async () => {
    const credentialStore = new RecordCredentials()
    credentialStore.record = { kind: 'api-key', key: 'account-secret' }
    const first = await mounted({ persistentBrowserSession: false }, credentialStore)
    const cookie = browserCookie(first.connection, '127.0.0.1:3080')

    expect(credentialStore).toMatchObject({ reads: 0, modifies: 0 })
    expect(credentialStore.record).toEqual({ kind: 'api-key', key: 'account-secret' })
    expect(first.connection.requestRejection(fakeRequest({
      host: '127.0.0.1:3080', cookie,
    }))).toBeUndefined()
    await first.dispose()

    const restarted = await mounted({ persistentBrowserSession: false }, credentialStore)
    expect(restarted.connection.requestRejection(fakeRequest({
      host: '127.0.0.1:3080', cookie,
    }))).toBe(401)
    expect(credentialStore).toMatchObject({ reads: 0, modifies: 0 })
    expect(credentialStore.record).toEqual({ kind: 'api-key', key: 'account-secret' })
