...
interface FixtureWorkspaceRequests {
  ...
  archiveWorkspace: { readonly workspaceId: WorkspaceId }
  unarchiveWorkspace: { readonly workspaceId: WorkspaceId }
  deleteWorkspace: { readonly workspaceId: WorkspaceId }
}

interface FixtureWorkspaceManagementValue {
...
interface FixtureWorkspaceValues {
  ...
  setSessionPinned: { readonly workspace: FixtureWorkspaceView }
  archiveSession: FixtureWorkspaceManagementValue
  unarchiveSession: FixtureWorkspaceManagementValue
  deleteSession: FixtureWorkspaceManagementValue
  archiveWorkspace: FixtureWorkspaceManagementValue
  unarchiveWorkspace: FixtureWorkspaceManagementValue
  deleteWorkspace: FixtureWorkspaceManagementValue
}
...
type FixtureWorkspaceFrame =
  | {
    ...
    readonly value: {
      ...
      readonly management: FixtureWorkspaceManagementValue
    }
  }
  ...
  | { readonly type: 'management'; readonly value: FixtureWorkspaceManagementValue }
...
function createWorkspaceApi(rpc: ClientConnectionRpc): FixtureWorkspaceApi {
  ...
  return {
    ...
    setSessionPinned: (request, signal) => call('setSessionPinned', request, signal),
    ...
    archiveWorkspace: (request, signal) => call('archiveWorkspace', request, signal),
    unarchiveWorkspace: (request, signal) => call('unarchiveWorkspace', request, signal),
    deleteWorkspace: (request, signal) => call('deleteWorkspace', request, signal),
  }
}
...
function createWorkspaceClient(rpc: ClientConnectionRpc): FixtureWorkspaceClient {
  ...
  return {
    ...
    setSessionPinned: (request, signal) => api.setSessionPinned(req(request), signal),
    ...
    archiveWorkspace: (request, signal) => api.archiveWorkspace(req(request), signal),
    unarchiveWorkspace: (request, signal) => api.unarchiveWorkspace(req(request), signal),
    deleteWorkspace: (request, signal) => api.deleteWorkspace(req(request), signal),
  }
}
...
describe('createFixtureApi', () => {
  ...
  it('workspace/follow serves the resident baseline and create reuses on path collision', async () => {
    ...
    expect(baseline.management).toEqual({
  ...
  it('workspace.setSessionPinned commits pin order, preserves position on unpin, and rejects invalid ids', async () => {
    const api = createFixtureApi()
    const wsid = 'fx-ws-fixture' as WorkspaceId
    const missing = await api.workspace.setSessionPinned(req({
      workspaceId: 'fx-ws-void' as WorkspaceId,
      sessionId: sid('fx-alpha'),
      pinned: true,
    }))
    expect(missing.result).toMatchObject({ ok: false, error: { code: 'workspace-not-found' } })
    const ghost = await api.workspace.setSessionPinned(req({
      workspaceId: wsid,
      sessionId: sid('fx-ghost'),
      pinned: true,
    }))
    expect(ghost.result).toMatchObject({
      ok: false,
      error: { code: 'workspace-move-invalid', details: { sessionId: 'fx-ghost' } },
    })

    const pinned = await api.workspace.setSessionPinned(req({
      workspaceId: wsid,
      sessionId: sid('fx-gamma'),
      pinned: true,
    }))
    if (!pinned.result.ok) throw new Error('pin failed')
    expect(pinned.result.value.workspace.sessionIds).toEqual(['fx-gamma', 'fx-alpha', 'fx-beta'])
    expect(pinned.result.value.workspace.pinnedSessionIds).toEqual(['fx-gamma'])

    const unpinned = await api.workspace.setSessionPinned(req({
      workspaceId: wsid,
      sessionId: sid('fx-gamma'),
      pinned: false,
    }))
    if (!unpinned.result.ok) throw new Error('unpin failed')
    expect(unpinned.result.value.workspace.sessionIds).toEqual(['fx-gamma', 'fx-alpha', 'fx-beta'])
    expect(unpinned.result.value.workspace.pinnedSessionIds).toEqual([])
  })

  it('workspace management RPCs publish and return the complete four-set state', async () => {
    const api = createFixtureApi()
    const abort = new AbortController()
    const consuming = collectValues(
      api.workspaceRemote.follow(abort.signal),
      abort,
      frames => frames.some(frame => frame.type === 'management'
        && frame.value.archivedSessionIds.includes(sid('fx-alpha'))),
    )
    await new Promise(resolve => setTimeout(resolve, 10))

    const archivedSession = await api.workspace.archiveSession(req({ sessionId: sid('fx-alpha') }))
    expect(archivedSession.result).toEqual({
      ok: true,
      value: {
        archivedWorkspaceIds: [],
        deletedWorkspaceIds: [],
        archivedSessionIds: ['fx-alpha'],
        deletedSessionIds: [],
      },
    })
    expect((await consuming).at(-1)).toEqual({
      type: 'management',
      value: archivedSession.result.ok ? archivedSession.result.value : undefined,
    })

    const restoredSession = await api.workspace.unarchiveSession(req({ sessionId: sid('fx-alpha') }))
    expect(restoredSession.result).toMatchObject({ ok: true, value: { archivedSessionIds: [] } })
    const deletedSession = await api.workspace.deleteSession(req({ sessionId: sid('fx-alpha') }))
    expect(deletedSession.result).toMatchObject({ ok: true, value: { deletedSessionIds: ['fx-alpha'] } })

    const workspaceId = 'fx-ws-fixture' as WorkspaceId
    const archivedWorkspace = await api.workspace.archiveWorkspace(req({ workspaceId }))
    expect(archivedWorkspace.result).toMatchObject({ ok: true, value: { archivedWorkspaceIds: [workspaceId] } })
    const restoredWorkspace = await api.workspace.unarchiveWorkspace(req({ workspaceId }))
    expect(restoredWorkspace.result).toMatchObject({ ok: true, value: { archivedWorkspaceIds: [] } })
    const deletedWorkspace = await api.workspace.deleteWorkspace(req({ workspaceId }))
    expect(deletedWorkspace.result).toMatchObject({ ok: true, value: { deletedWorkspaceIds: [workspaceId] } })
  })
