...
import { BrowserAuth, isBrowserAuthCookieName } from '../src/browser-auth.ts'
...
describe('BrowserAuth', () => {
  it('keeps a process-local secret across reloads and rotates it for a new process owner', () => {
    const processOwner = {}
    const first = BrowserAuth.createProcessLocal(processOwner, 30)
    const login = exchange(first)

    const reloaded = BrowserAuth.createProcessLocal(processOwner, 30)
    ...
    const restarted = BrowserAuth.createProcessLocal({}, 30)
    expect(restarted.isAuthenticated(request('/', '127.0.0.1:3080', { cookie: login.cookie }))).toBe(false)
    const fresh = exchange(restarted)
    expect(restarted.isAuthenticated(request('/', '127.0.0.1:3080', { cookie: fresh.cookie }))).toBe(true)
  })

  it('uses one process-local cookie name across random desktop ports while retaining the port audience', () => {
    const auth = BrowserAuth.createProcessLocal({}, 30)
    const first = exchange(auth, '127.0.0.1:3080')
    const second = exchange(auth, '127.0.0.1:49152')
    const firstName = first.cookie.split('=', 1)[0]
    const secondName = second.cookie.split('=', 1)[0]

    expect(firstName).toBe(secondName)
    expect(firstName).toBe('dsh-auth-process-local')
    expect(isBrowserAuthCookieName(firstName ?? '')).toBe(true)
    expect(isBrowserAuthCookieName('account-session')).toBe(false)
    expect(auth.isAuthenticated(request('/', '127.0.0.1:49152', { cookie: first.cookie }))).toBe(false)
    expect(auth.isAuthenticated(request('/', '127.0.0.1:49152', { cookie: second.cookie }))).toBe(true)
  })
