...
interface WorkspaceSetSessionPinnedRequest {
  readonly workspaceId: WorkspaceId
  readonly sessionId: SessionId
  readonly pinned: boolean
}
interface WorkspaceArchiveSessionRequest { readonly sessionId: SessionId }
interface WorkspaceManagementRequest { readonly workspaceId: WorkspaceId }
interface WorkspaceSessionManagementRequest { readonly sessionId: SessionId }
interface WorkspaceManagementValue {
  readonly archivedWorkspaceIds: readonly WorkspaceId[]
  readonly deletedWorkspaceIds: readonly WorkspaceId[]
  readonly archivedSessionIds: readonly SessionId[]
  readonly deletedSessionIds: readonly SessionId[]
}

// This standalone browser fixture intentionally has no dependency on the Host
// controller package. Keep this wire shape in lockstep with WorkspaceBaseline:
// partial legacy archive frames leave the real Client model permanently pending.
type WorkspaceFollowFrame =
  | {
    readonly type: 'baseline'
    readonly value: {
      readonly items: readonly WorkspaceView[]
      readonly management: WorkspaceManagementValue
    }
  }
  ...
  | { readonly type: 'management'; readonly value: WorkspaceManagementValue }
...
interface FixtureWorkspaceApi {
  ...
  setSessionPinned(request: WorkspaceSetSessionPinnedRequest): Promise<ConnectionRpcResult<WorkspaceValue>>
  archiveSession(request: WorkspaceArchiveSessionRequest): Promise<ConnectionRpcResult<WorkspaceManagementValue>>
  unarchiveSession(request: WorkspaceSessionManagementRequest): Promise<ConnectionRpcResult<WorkspaceManagementValue>>
  deleteSession(request: WorkspaceSessionManagementRequest): Promise<ConnectionRpcResult<WorkspaceManagementValue>>
  archiveWorkspace(request: WorkspaceManagementRequest): Promise<ConnectionRpcResult<WorkspaceManagementValue>>
  unarchiveWorkspace(request: WorkspaceManagementRequest): Promise<ConnectionRpcResult<WorkspaceManagementValue>>
  deleteWorkspace(request: WorkspaceManagementRequest): Promise<ConnectionRpcResult<WorkspaceManagementValue>>
}
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  let nextWorkspace = 1
  // Complete management state mirrors the Host baseline. Keeping these four
  // arrays together prevents a new reconnect generation from losing tombstones.
  const archivedWorkspaceIds: WorkspaceId[] = []
  const deletedWorkspaceIds: WorkspaceId[] = []
  const archivedSessionIds: SessionId[] = []
  const deletedSessionIds: SessionId[] = []
  const managementSnapshot = (): WorkspaceManagementValue => ({
  ...
  const removeManagedId = <T>(items: T[], id: T): boolean => {
    const index = items.indexOf(id)
    if (index === -1) return false
    items.splice(index, 1)
    return true
  }
  const publishManagement = (): Promise<ConnectionRpcResult<WorkspaceManagementValue>> => {
    const value = managementSnapshot()
    emitWorkspace({ type: 'management', value })
    return sessionOk(value)
  }
  const unchangedManagement = (): Promise<ConnectionRpcResult<WorkspaceManagementValue>> =>
    sessionOk(managementSnapshot())
  ...
  const workspaceApi: FixtureWorkspaceApi = {
    ...
    delete: async (request) => {
      ...
      const archivedRemoved = removeManagedId(archivedWorkspaceIds, request.workspaceId)
      const deletedRemoved = removeManagedId(deletedWorkspaceIds, request.workspaceId)
      if (archivedRemoved || deletedRemoved) await publishManagement()
    ...
    setSessionPinned: (request) => {
      ...
      if (!workspace.sessionIds.includes(request.sessionId)) {
        return sessionErr({
          code: 'workspace-move-invalid',
          message: `session is not accounted by workspace ${request.workspaceId}`,
          details: { workspaceId: request.workspaceId, sessionId: request.sessionId },
        })
      }
      const pinnedSessionIds = request.pinned
        ? [request.sessionId, ...workspace.pinnedSessionIds.filter(id => id !== request.sessionId)]
        : workspace.pinnedSessionIds.filter(id => id !== request.sessionId)
      const sessionIds = request.pinned
        ? [request.sessionId, ...workspace.sessionIds.filter(id => id !== request.sessionId)]
        : workspace.sessionIds
      if (!sessionIds.every((id, index) => id === workspace.sessionIds[index])
        || !pinnedSessionIds.every((id, index) => id === workspace.pinnedSessionIds[index])
        || pinnedSessionIds.length !== workspace.pinnedSessionIds.length) {
        workspace.sessionIds = sessionIds
        workspace.pinnedSessionIds = pinnedSessionIds
        workspace.updatedAt = new Date().toISOString()
        emitWorkspace({ type: 'upsert', workspace: workspaceSnapshot(workspace) })
      }
      return sessionOk({ workspace: workspaceSnapshot(workspace) })
    ...
    archiveSession: (request) => {
      ...
      if (deletedSessionIds.includes(request.sessionId)) {
        return sessionErr({
          code: 'session-not-found',
          message: `no active session ${request.sessionId}`,
          details: { sessionId: request.sessionId },
        })
      }
      if (archivedSessionIds.includes(request.sessionId)) return unchangedManagement()
      archivedSessionIds.push(request.sessionId)
      return publishManagement()
    },
    unarchiveSession: (request) => {
      if (summaryOf(request.sessionId) === undefined || deletedSessionIds.includes(request.sessionId)) {
        return sessionErr({
          code: 'session-not-found',
          message: `no restorable session ${request.sessionId}`,
          details: { sessionId: request.sessionId },
        })
      }
      if (!removeManagedId(archivedSessionIds, request.sessionId)) return unchangedManagement()
      return publishManagement()
    },
    deleteSession: (request) => {
      ...
      if (deletedSessionIds.includes(request.sessionId)) return unchangedManagement()
      removeManagedId(archivedSessionIds, request.sessionId)
      deletedSessionIds.push(request.sessionId)
      return publishManagement()
    },
    archiveWorkspace: (request) => {
      if (!workspaces.some(workspace => workspace.workspaceId === request.workspaceId)
        || deletedWorkspaceIds.includes(request.workspaceId)) {
        return sessionErr({
          code: 'workspace-not-found',
          message: `no active workspace ${request.workspaceId}`,
          details: { workspaceId: request.workspaceId },
        })
      }
      if (archivedWorkspaceIds.includes(request.workspaceId)) return unchangedManagement()
      archivedWorkspaceIds.push(request.workspaceId)
      return publishManagement()
    },
    unarchiveWorkspace: (request) => {
      if (!workspaces.some(workspace => workspace.workspaceId === request.workspaceId)
        ...
        return sessionErr({
          code: 'workspace-not-found',
          message: `no restorable workspace ${request.workspaceId}`,
          details: { workspaceId: request.workspaceId },
        })
      }
      if (!removeManagedId(archivedWorkspaceIds, request.workspaceId)) return unchangedManagement()
      return publishManagement()
    },
    deleteWorkspace: (request) => {
      ...
      if (deletedWorkspaceIds.includes(request.workspaceId)) return unchangedManagement()
      removeManagedId(archivedWorkspaceIds, request.workspaceId)
      deletedWorkspaceIds.push(request.workspaceId)
      return publishManagement()
    },
  }
  ...
  const rpc: ClientConnectionRpc = {
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        ...
        case 'workspace/setSessionPinned': return workspaceApi.setSessionPinned(
          request as WorkspaceSetSessionPinnedRequest,
        )
        case 'workspace/archiveSession': return workspaceApi.archiveSession(request as WorkspaceArchiveSessionRequest)
        case 'workspace/unarchiveSession': return workspaceApi.unarchiveSession(request as WorkspaceSessionManagementRequest)
        case 'workspace/deleteSession': return workspaceApi.deleteSession(request as WorkspaceSessionManagementRequest)
        case 'workspace/archiveWorkspace': return workspaceApi.archiveWorkspace(request as WorkspaceManagementRequest)
        case 'workspace/unarchiveWorkspace': return workspaceApi.unarchiveWorkspace(request as WorkspaceManagementRequest)
        case 'workspace/deleteWorkspace': return workspaceApi.deleteWorkspace(request as WorkspaceManagementRequest)
        default:
