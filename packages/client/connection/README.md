...
`persistentBrowserSession` defaults to `true` and selects that credential-backed behavior. A composition that sets it to `false` keeps a random signing secret in the root application context without reading or modifying `ctx.credentials`; Connection reloads under the same root retain the cookie, while a new root rejects the old cookie and exchanges its fresh launch token for a new one. Process-local mode uses the fixed `dsh-auth-process-local` name so a random loopback port does not create another 30-day cookie on every launch; the signed payload still binds the exact authority. A desktop shell that reuses a persistent browser partition removes names recognized by `isBrowserAuthCookieName` before its first loopback navigation, clearing obsolete per-port transport cookies without selecting account, model, MCP, connector, or ordinary Web cookies.
...
- **There is no logout operation** — clearing the browser cookie ends one browser session; in persistent mode, deleting the owner credential record and restarting `dsh` revokes every session, while process-local mode revokes every prior cookie when its root application context ends.
