...
`persistentBrowserSession` 默认为 `true`，采用上述凭据持久化行为。组合将其设为 `false` 时，随机签名密钥仅保存在应用根 context 内，不读取或修改 `ctx.credentials`；同一根 context 下的 Connection 重载仍可使用原 cookie，新根 context 会拒绝旧 cookie，并用本次启动令牌交换新 cookie。进程级模式使用固定名称 `dsh-auth-process-local`，随机 loopback 端口不会在每次启动时新增一份 30 天 cookie；签名 payload 仍绑定精确 authority。复用持久浏览器分区的桌面壳会在首次 loopback 导航前删除 `isBrowserAuthCookieName` 识别的名称，清除过时的按端口传输 cookie，同时不选择账号、模型、MCP、连接器或普通 Web cookie。
...
- **没有 logout 操作**：清除浏览器 cookie 会结束单个浏览器会话；持久模式下删除 owner 凭据记录并重启 `dsh` 会撤销全部会话，进程级模式则在应用根 context 结束时撤销此前全部 cookie。
