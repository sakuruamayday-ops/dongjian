...
afterEach(() => {
  ...
  document.head.querySelector('#dsh-boot-manifest')?.remove()
})

/** Install the Host graph as the CSP-safe inert JSON element. */
function installManifest(entries: WebBootEntry[]): void {
  const element = document.createElement('script')
  element.id = 'dsh-boot-manifest'
  element.type = 'application/json'
  element.textContent = JSON.stringify({ rev: 'graph', entries })
...
describe('bootstrap failure rendering', () => {
  ...
  it('reads the Host graph from inert JSON when no compatibility global exists', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    let receivedBoot: unknown
    installFacade((options) => {
      receivedBoot = options.boot
      throw new Error('captured inert manifest')
    })
    delete win.__DSH_BOOT__
    installManifest([{ id: 'fixture', url: '/fixture.js', rev: '1' }])
    const entry = new AppWebEntry(container)

    await entry.run()

    expect(receivedBoot).toEqual({
      rev: 'graph',
      entries: [{ id: 'fixture', url: '/fixture.js', rev: '1' }],
    })
    await entry.dispose()
  })
