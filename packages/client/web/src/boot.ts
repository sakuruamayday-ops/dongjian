...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    try {
      ...
      const compatibilityWire = win.__DSH_BOOT__
      const manifestElement = document.getElementById('dsh-boot-manifest')
      const manifestText = manifestElement?.textContent
      const boot = compatibilityWire ?? (manifestText === undefined || manifestText === ''
        ? undefined
