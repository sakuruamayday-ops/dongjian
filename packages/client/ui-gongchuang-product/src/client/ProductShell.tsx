    ...
    <>
      ...
      {configuring !== null && <div className={css.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && busy === null) closeConfiguration() }}>
        <section ref={connectorDialogRef} className={css.connectionModal} role="dialog" aria-modal="true" aria-labelledby="connector-dialog-title">
          <div className={css.modalHead}><div><span>安全连接向导</span><h2 id="connector-dialog-title">连接{configuring.name}</h2></div><button type="button" disabled={busy !== null} aria-label="关闭" onClick={closeConfiguration}>×</button></div>
          {configuring.id === 'gongchuang-knowledge' && <label className={css.accountField}><span>{props.t('knowledge.endpoint')}</span><input type="url" value={knowledgeEndpoint} onChange={(event) => { setKnowledgeEndpoint(event.currentTarget.value) }} placeholder="https://example.com/mcp/" /></label>}
          ...
          <ol className={css.guideSteps}>
            <li><span>1</span><div>{configuring.id === 'gongchuang-knowledge' ? <><strong>{props.t('knowledge.credentialsTitle')}</strong><p>{props.t('knowledge.credentialsGuide')}</p></> : <><strong>登录{connectorSetup?.platform ?? '官方平台'}</strong><p>{connectorSetup?.guide ?? '登录官方平台并取得访问凭据。'}</p>{configuring.officialConfigUrl && <a href={configuring.officialConfigUrl} target="_blank" rel="noreferrer">{connectorSetup?.officialLinkLabel ?? '打开官方登录页'}<Icon name="external" size={14} /></a>}</>}</div></li>
            <li><span>2</span><div><strong>{configuring.credentialConfigured ? '验证现有凭据或替换' : '粘贴官方访问密钥'}</strong><p>密钥由 Windows 凭据管理器或 macOS 钥匙串保护，不进入页面存储、普通配置文件或对话。</p><label className={css.credentialField}><span>{connectorSetup?.credentialLabel ?? '官方访问密钥'}</span><input type="password" autoComplete="off" value={credential} onChange={(event) => { setCredential(event.target.value) }} placeholder={configuring.credentialConfigured ? '留空则验证系统安全存储中的现有凭据' : connectorSetup?.credentialPlaceholder ?? '粘贴官方平台提供的密钥'} /></label></div></li>
