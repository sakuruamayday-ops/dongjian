...
export interface IWorkspaces {
  ...
  /**
   * Hide a Session behind a recoverable client-history tombstone.
   ...
   */
  setSessionPinned(workspaceId: WorkspaceId, sessionId: SessionId, pinned: boolean): Promise<void>
  /**
   * Hide a Workspace registration while retaining its directory and record.
...
export class WorkspaceController extends Service implements IWorkspaces {
  ...
  async setSessionPinned(workspaceId: WorkspaceId, sessionId: SessionId, pinned: boolean): Promise<void> {
    const result = await this.model.setSessionPinned(workspaceId, sessionId, pinned)
    if (!result.ok) throw commandError('session pin', result.error)
  }

  async archiveWorkspace(workspaceId: WorkspaceId): Promise<void> {
    const result = await this.model.archiveWorkspace(workspaceId)
  ...
  async deleteWorkspace(workspaceId: WorkspaceId): Promise<void> {
    const result = await this.model.deleteWorkspace(workspaceId)
    if (!result.ok) throw commandError('soft delete', result.error)
  }
