...
function emptyManagement(overrides: Partial<WorkspaceManagementValue> = {}): WorkspaceManagementValue {
  return {
...
class ScriptedWorkspaceRemote implements WorkspaceRemote {
  ...
  archiveSession(_request: WorkspaceArchiveSessionRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    throw new Error('unused')
  }

  setSessionPinned(_request: WorkspaceSetSessionPinnedRequest): Promise<RemoteResult<WorkspaceValue>> {
    throw new Error('unused')
  }

  archiveWorkspace(_request: WorkspaceManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    throw new Error('unused')
  }

  unarchiveWorkspace(_request: WorkspaceManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    throw new Error('unused')
  }

  deleteWorkspace(_request: WorkspaceManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    throw new Error('unused')
  }

  unarchiveSession(_request: WorkspaceSessionManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    throw new Error('unused')
  }

  deleteSession(_request: WorkspaceSessionManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    throw new Error('unused')
  }
...
class CommandWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly setSessionPinned = vi.fn<WorkspaceRemote['setSessionPinned']>(request => Promise.resolve(remoteOk({
  ...
  readonly archiveWorkspace = vi.fn<WorkspaceRemote['archiveWorkspace']>(() =>
    Promise.resolve(remoteOk(emptyManagement())))
  readonly unarchiveWorkspace = vi.fn<WorkspaceRemote['unarchiveWorkspace']>(() =>
    Promise.resolve(remoteOk(emptyManagement())))
  readonly deleteWorkspace = vi.fn<WorkspaceRemote['deleteWorkspace']>(() =>
    Promise.resolve(remoteOk(emptyManagement())))
  readonly unarchiveSession = vi.fn<WorkspaceRemote['unarchiveSession']>(() =>
    Promise.resolve(remoteOk(emptyManagement())))
  readonly deleteSession = vi.fn<WorkspaceRemote['deleteSession']>(() =>
    Promise.resolve(remoteOk(emptyManagement())))
  ...
  it('delivers one baseline followed by increments', async () => {
    ...
    const remote = new ScriptedWorkspaceRemote([{
      frames: [
        ...
        { type: 'management', value: emptyManagement({ archivedSessionIds: ['session-one' as never] }) },
      ],
      hold: true,
    }])
    ...
    const replaceManagement = vi.fn<WorkspaceFollowSink['replaceManagement']>()
    ...
    stream.start()
    stream.start()
    await vi.waitFor(() => { expect(replaceManagement).toHaveBeenCalledOnce() })
    ...
    expect(replaceManagement).toHaveBeenCalledWith(emptyManagement({
...
describe('WorkspaceController', () => {
  it('publishes the model source and exposes successful Workspace commands', async () => {
    ...
    model.replaceBaseline({ items: [workspace('one')], management: emptyManagement() })
