...
function management(overrides: Partial<WorkspaceManagementValue> = {}): WorkspaceManagementValue {
  return {
...
class FakeWorkspaceRemote implements WorkspaceRemote {
  ...
  ) => Promise<RemoteResult<WorkspaceManagementValue>> = request =>
    Promise.resolve(remoteOk(management({ archivedSessionIds: [request.sessionId] })))
  onSetSessionPinned: (
    request: WorkspaceSetSessionPinnedRequest,
  ) => Promise<RemoteResult<WorkspaceValue>> = request => Promise.resolve(remoteOk({
    workspace: {
      ...workspace(String(request.workspaceId), [request.sessionId]),
      pinnedSessionIds: request.pinned ? [request.sessionId] : [],
    },
  }))
  onManagement: (
    _request: WorkspaceManagementRequest | WorkspaceSessionManagementRequest,
  ) => Promise<RemoteResult<WorkspaceManagementValue>> = () => Promise.resolve(remoteOk(management()))
  ...
  archiveSession(request: WorkspaceArchiveSessionRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    this.record('archiveSession', request)
    return this.onArchiveSession(request)
  }

  setSessionPinned(request: WorkspaceSetSessionPinnedRequest): Promise<RemoteResult<WorkspaceValue>> {
    this.record('setSessionPinned', request)
    return this.onSetSessionPinned(request)
  }

  archiveWorkspace(request: WorkspaceManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    this.record('archiveWorkspace', request)
    return this.onManagement(request)
  }

  unarchiveWorkspace(request: WorkspaceManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    this.record('unarchiveWorkspace', request)
    return this.onManagement(request)
  }

  deleteWorkspace(request: WorkspaceManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    this.record('deleteWorkspace', request)
    return this.onManagement(request)
  }

  unarchiveSession(request: WorkspaceSessionManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
    this.record('unarchiveSession', request)
    return this.onManagement(request)
  }

  deleteSession(request: WorkspaceSessionManagementRequest): Promise<RemoteResult<WorkspaceManagementValue>> {
...
function baseline(
  ...
  model.replaceBaseline({ items, management: management({ archivedSessionIds }) })
}
...
describe('ClientWorkspaceModel', () => {
  it('replaces reconnect state and applies ordered increments', () => {
    ...
    model.replaceManagement(management({ archivedSessionIds: [sid('hidden')] }))
  ...
  it('applies Workspace mutation echoes and leaves failed results unchanged', async () => {
    ...
    remote.onArchiveSession = request => Promise.resolve(remoteOk(management({
