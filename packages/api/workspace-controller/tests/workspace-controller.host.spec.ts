...
describe('WorkspaceController commands', () => {
  ...
  it('removes a Workspace registration and recreates the same retained directory without ghosts', async () => {
    const { controller, ctx, root } = await harness()
    const path = stageDir(root, 'reimported')
    ...
    expect(ctx.workspaceRegistry.get(first.workspace.workspaceId)).toBeUndefined()
    ...
    expect(ctx.workspaceRegistry.list().map(workspace => workspace.id)).toEqual([
      second.workspace.workspaceId,
    ])
  })

  it('restores a legacy deleted-path tombstone when that directory is imported again', async () => {
    const { controller, ctx, root } = await harness()
    const path = stageDir(root, 'legacy-deleted')
    const first = await controller.create({ path })
    await ctx.workspaceRegistry.deleteWorkspace(first.workspace.workspaceId)
    expect(ctx.workspaceRegistry.deletedWorkspaceIds).toEqual([first.workspace.workspaceId])

    await expect(controller.create({ path })).resolves.toEqual({
      workspace: first.workspace,
      created: false,
    })
    expect(ctx.workspaceRegistry.deletedWorkspaceIds).toEqual([])
    expect(ctx.workspaceRegistry.list().map(workspace => workspace.id)).toEqual([
