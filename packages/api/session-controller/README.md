...
Before a fork changes the stored workspace cwd, the Host emits `api-session/fork-preparing` with only the inherited events and source/target cwd. Product listeners can copy controlled imports; a rejected listener prevents child creation. The controller itself does not infer or copy filesystem dependencies.
...
Host-only artifact deletion requires the optional Session persistence service. Disposal resolves it with `ctx.get`, flushes and disposes an owned live `AgentHandle`, and waits for that handle's final durable drain before the caller moves the artifact. Cold Sessions do not activate an Agent; missing persistence rejects deletion before lifecycle changes.
...
- Queue rows with a separate `displayText` projection expose `text: null`: replacing only the visible text cannot preserve their prepared model context. Plain-text queue rows remain editable.
