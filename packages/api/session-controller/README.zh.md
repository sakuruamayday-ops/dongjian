...
分支改变已存工作目录前，Host 通过 `api-session/fork-preparing` 提供继承事件及源、目标目录。产品监听器负责复制受控附件；监听失败时不创建子会话。控制器本身不猜测或复制文件依赖。
...
仅 Host 可调用的产物删除操作需要可选的 Session persistence 服务。销毁通过 `ctx.get` 取得该服务，刷新并销毁自有 live `AgentHandle`，等待该句柄完成最后一次持久写入后才允许调用方移动产物。冷会话不会激活 Agent；缺少持久化服务时，在改变生命周期前拒绝删除。
...
- 带有独立 `displayText` 投影的队列行会公开 `text: null`：只替换可见文本无法保留其已准备的模型上下文。纯文本队列行仍可编辑。
