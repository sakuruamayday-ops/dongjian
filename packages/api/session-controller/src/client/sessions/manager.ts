...
export class SessionManager {
  ...
  */
   ...
   * parentSessionId so the list nests it under its source. An attachment
   * failure only synthesizes a child when the Host reports that rollback also
   * failed and the child therefore remains addressable.
   * @param opts - source session and the optional seq anchoring the cut.
   * @returns the fork result (the child session id).
  */
  async fork(
    opts: { sessionId: SessionId; atSeq?: SessionSeq; workspaceId?: WorkspaceId; cwd?: string },
  ): Promise<RemoteResult<{ sessionId: SessionId }>> {
