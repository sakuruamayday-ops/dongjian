...
function visibleUserContent(content: readonly ContentBlock[], displayText: string | undefined): readonly ContentBlock[] {
...
export class SessionQueueMirror {
  ...
  replace(items: QueueItems): void {
    this.current = items.map((item) => {
      const content = visibleUserContent(
        item.message.content as unknown as readonly ContentBlock[],
        item.displayText,
      )
      return {
        id: item.id,
        ...
        // A plain-text edit cannot preserve product-prepared model context and display metadata.
        text: item.displayText === undefined ? textOf(content) : null,
      }
    })
  }
