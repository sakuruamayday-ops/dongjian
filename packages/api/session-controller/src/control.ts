...
/** Browser presentation fields carried by a browser-submitted message's user source. */
function promptPresentation(message: UserMessage): Pick<SessionQueuedItem, 'rpcId' | 'displayText'> {
  const source = message.source
  if (source.kind !== 'user') return {}
  return {
    ...('rpcId' in source ? { rpcId: source.rpcId } : {}),
    ...('displayText' in source && typeof source.displayText === 'string'
