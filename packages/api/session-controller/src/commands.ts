...
import type { Agent, AgentHandle, ModelSelection as AgentModelSelection } from '@deepseek-ai/dsh-agent'
...
export class SessionCommandController {
      ...
      try {
        ...
        try {
          ...
          this.ctx.logger.warn(
            `session-controller: model selection was not committed because the default could not be saved: ${String(error)}`,
          )
          throw new RemoteError(
            'gateway/internal',
            'model selection could not be saved; the Session was not changed',
            {},
          )
        }
        // Persist the cross-Session default before appending the Session-local
        // intent. Returning success with only one side committed makes a later
        // Session appear to revert after restart.
  ...
  async fork(request: SessionForkRequest): Promise<SessionForkValue> {
    ...
    if (request.workspaceId === undefined) {
      ...
      workspace = this.ctx.workspaceRegistry.get(request.workspaceId)
      if (workspace === undefined
        || this.ctx.workspaceRegistry.archivedWorkspaceIds.includes(request.workspaceId)
        || this.ctx.workspaceRegistry.deletedWorkspaceIds.includes(request.workspaceId)) {
        throw new RemoteError('workspace/not-found', `workspace "${request.workspaceId}" not found`, {
          workspaceId: request.workspaceId,
        })
      }
    ...
    try {
      if (workspace !== undefined && source.header.cwd !== undefined && workspace.path !== source.header.cwd) {
        await this.ctx.parallel('api-session/fork-preparing', {
          events: source.events.slice(0, cut), sourceCwd: source.header.cwd, targetCwd: workspace.path,
        })
      }
      const { provider, model } = this.ctx.agentDefaultModel.currentSelection()
      child = await this.ctx.agents.create({
        sessionId: childId,
        ...
        throw new RemoteError(
          ...
          `session "${childId}" was forked but could not attach to workspace "${workspace.id}": ${String(error)}`
            + (rollbackFailure === undefined
              ? ''
              : `; child cleanup also failed: ${rollbackFailure instanceof Error ? rollbackFailure.message : 'unknown error'}`),
          rollbackFailure === undefined
  ...
  /**
   * Resolve one durable file to its Host path after proving that the addressed Session references it.
   * @param sessionId - Session whose log authorizes the file.
   * @param expected - Complete opaque reference supplied by the mounted client card.
   * @returns provider-owned absolute path for a host-file-backed attachment store.
   */
  async fileAttachmentPath(sessionId: SessionId, expected: FileAttachmentRef): Promise<string> {
    const source = await this.readSessionState(sessionId)
    const ref = referencedFile(source.events, candidate => (
      candidate.attachmentId === expected.attachmentId
      && candidate.name === expected.name
      && candidate.bytes === expected.bytes
    ))
    if (ref === undefined) throw new Error('File is not referenced by this session.')
    const path = this.ctx.attachments.fileHostPath(ref)
    if (path === undefined) throw new Error('The mounted attachment provider has no host file path.')
    return path
  }
...
function referencedFile(
  ...
  for (const event of events) {
    const found = fileInEvent(event, match)
    if (found !== undefined) return found
  }
  return undefined
}
