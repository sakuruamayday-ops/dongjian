...
export class ApiSessionAgentController {
  ...
  private readonly disposals = new Map<SessionId, Promise<void>>()
  ...
  constructor(private readonly ctx: Context) {
    ctx.on('agent/disposed', ({ agent }) => {
      ...
      if (handle?.agent === agent) this.handles.delete(agent.id)
    })
  ...
  /**
   * Quiesce an ordinary Session owned by this controller before a Host moves
   * its persistence artifact. Bare registry entries are not disposal
   * capabilities, so an Agent owned by another subsystem is rejected.
   * @param sessionId - ordinary Session identity to quiesce.
   */
  disposeSession(sessionId: SessionId): Promise<void> {
    // This barrier remains after writer retirement until the Host either
    // commits deletion or explicitly rolls the deletion transaction back.
    this.deleting.add(sessionId)
    const pending = this.disposals.get(sessionId)
    if (pending !== undefined) return pending
    const disposal = this.disposeOwnedSession(sessionId)
  ...
  /**
   * Release a deletion barrier only after the Host has durably rolled deletion back.
   * @param sessionId - ordinary Session identity whose deletion was rolled back.
   */
  releaseSessionDisposal(sessionId: SessionId): void {
    if (this.disposals.has(sessionId)) {
      throw new Error(`session "${sessionId}" disposal is still in progress`)
    }
    ...
    if (this.deleting.has(sessionId)) {
      return {
        error: new RemoteError(
          ...
          { reason: 'Host deletion barrier is active' },
        ),
      }
    }
  ...
  /** Keep the exact capability returned by the factory that this controller invoked. */
  private retainHandle(handle: AgentHandle): Agent {
    this.handles.set(handle.agent.id, handle)
    return handle.agent
  }

  /** Flush and stop one owned live lifecycle through its retained handle. */
  private async disposeOwnedSession(sessionId: SessionId): Promise<void> {
    // Persistence is optional for this controller's in-memory read APIs, but
    // artifact deletion must retain it through writer retirement. Optional
    // services use get(): direct property access requires a declared injection.
    const persistence = this.ctx.get('sessionPersistence')
    if (persistence === undefined) throw new Error('Session artifact deletion requires session persistence')
    // A resolve/create admitted before the barrier may still be publishing its
    // AgentHandle. Wait for those exact operations; the barrier rejects later ones.
    const activations = [this.resumes.get(sessionId), this.creations.get(sessionId)]
      .filter((operation): operation is Promise<Agent> => operation !== undefined)
    if (activations.length > 0) await Promise.allSettled(activations)
    ...
    if (handle === undefined) {
      if (liveAgent !== undefined || liveSession !== undefined) {
        throw new Error(`session "${sessionId}" is live but its AgentHandle belongs to another lifecycle owner`)
      }
      return
    }
    if (liveAgent !== handle.agent || liveSession !== handle.agent.session) {
      throw new Error(`session "${sessionId}" no longer matches its retained AgentHandle`)
    }
    await this.ctx.sessions.flush(handle.agent.session)
    // AgentHandle.dispose() closes the v2 persistence write handle and waits
    // for its final durable drain before it releases write ownership.
