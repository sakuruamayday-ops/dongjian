...
import type { SessionId, SessionSeqCursor, SessionEvent } from '@deepseek-ai/dsh-session/types'
...
export interface SessionForkRequest {
  ...
  /** Explicit active target for product-level cross-Workspace moves. */
  readonly workspaceId?: WorkspaceId
}
...
export interface SessionPromptRequest {
  ...
  /** Original composer text projected by clients instead of model-only prompt additions. */
...
declare module '@deepseek-ai/dsh-llm' {
  interface MessageSourceMap {
    /** Browser prompt correlation, display projection, and optional Host-validated time zone. */
    'user-rpc': {
      kind: 'user'
      rpcId: SessionRequestId
      clientTimeZone?: string
      /** Original composer text when the model-facing prompt contains hidden product context. */
      displayText?: string
    }
  }
}
...
export interface SessionQueuedItem {
  ...
  /** Original composer text when the queued model content includes hidden product context. */
  readonly displayText?: string
...
declare module '@deepseek-ai/cordis' {
  interface Events {
     ...
     */
    'api-session/fork-preparing'(input: {
