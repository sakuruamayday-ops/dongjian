...
import type { WorkspaceManagementValue } from '@deepseek-ai/dsh-api-workspace-controller/types'
...
const emptyWorkspaceManagement = (): WorkspaceManagementValue => ({
...
export class FakeApiClient {
  ...
  workspaceBaseline: Extract<WorkspaceFollowFrame, { type: 'baseline' }>['value'] = {
    items: [],
    management: emptyWorkspaceManagement(),
  }
  ...
  onWorkspaceArchiveSession: (payload: unknown) => Promise<RemoteResult<WorkspaceManagementValue>> =
    payload => Promise.resolve(ok({
      ...emptyWorkspaceManagement(),
      archivedSessionIds: [(payload as { sessionId: SessionId }).sessionId],
    }))

  onWorkspaceManagement: (payload: unknown) => Promise<RemoteResult<WorkspaceManagementValue>> =
    () => Promise.resolve(ok(emptyWorkspaceManagement()))
    ...
    return {
      ...
      workspace: {
        ...
        unarchiveSession: payload => this.record(
          'workspace.unarchiveSession', payload, this.onWorkspaceManagement(payload),
        ),
        deleteSession: payload => this.record(
          'workspace.deleteSession', payload, this.onWorkspaceManagement(payload),
        ),
        archiveWorkspace: payload => this.record(
          'workspace.archiveWorkspace', payload, this.onWorkspaceManagement(payload),
        ),
        unarchiveWorkspace: payload => this.record(
          'workspace.unarchiveWorkspace', payload, this.onWorkspaceManagement(payload),
        ),
        deleteWorkspace: payload => this.record(
          'workspace.deleteWorkspace', payload, this.onWorkspaceManagement(payload),
        ),
