...
describe('Session queue snapshot intake', () => {
  ...
  it('projects prepared display text without allowing a plain-text edit to discard its model context', () => {
    const session = makeSession()
    const message = createUserMessage({
      content: text('核对材料\n\n本次任务可读取以下企业工作区文件：\n- `共创导入资料/材料.xlsx`'),
      source: { kind: 'user', rpcId: rid('rpc-display'), displayText: '核对材料' } as never,
    })
    session.handleControlFrame(queueFrame([{
      id: 'q-display', body: '', displayText: '核对材料', message,
    }]))

    expect(session.getSnapshot().queue[0]).toMatchObject({
      content: [{ type: 'text', text: '核对材料' }],
      preview: '核对材料',
      text: null,
    })
