...
describe('remaining branches', () => {
  ...
  it('does not synthesize a fork child that the Host rolled back', async () => {
    const api = new FakeApiClient()
    api.onFork = () => Promise.resolve(err(new RemoteError(
      'session/workspace-attach-failed',
      'fork attachment failed and the child was rolled back',
      { workspaceId: 'w1' },
    )))
    const manager = new SessionManager(fakeRemote(api))
  ...
  it('reconciles a fork child when workspace attachment and rollback both fail', async () => {
    const api = new FakeApiClient()
    api.onFork = () => Promise.resolve(err(new RemoteError(
      'session/workspace-attach-failed',
      'forked but unattached',
  ...
  it('does not resurrect a fork child removed before the rollback failure arrives', async () => {
    const api = new FakeApiClient()
    const response = deferred<Awaited<ReturnType<FakeApiClient['onFork']>>>()
    api.onFork = () => response.promise
    const manager = new SessionManager(fakeRemote(api))
    const fork = manager.fork({ sessionId: S1 })

    manager.handleSessionAdded(summary(S2, { parentSessionId: S1 }))
    manager.handleSessionRemoved(S2)
    response.resolve(err(new RemoteError(
      'session/workspace-attach-failed',
      'fork rolled back after attachment failed',
      { workspaceId: 'w1' },
    )))
