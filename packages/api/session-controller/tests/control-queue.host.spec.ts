...
describe('Session control queue projection', () => {
  ...
  it('projects browser prompt identity and display text while omitting both elsewhere', async () => {
    const { control, inbox } = await harness()
    const identified = createUserMessage({
      content: [{ type: 'text', text: 'browser prompt' }],
      source: { kind: 'user', rpcId: 'req-42' as never, displayText: 'visible browser prompt' },
    })
    ...
    expect(items.map(item => ({
    ...
    }))).toEqual([
      {
        ...
        placement: 'queued',
        rpcId: 'req-42',
        displayText: 'visible browser prompt',
      },
      { id: items[1]?.id, placement: 'steering', rpcId: undefined, displayText: undefined },
    ])
    expect('rpcId' in (items[1] ?? {})).toBe(false)
    expect('displayText' in (items[1] ?? {})).toBe(false)
