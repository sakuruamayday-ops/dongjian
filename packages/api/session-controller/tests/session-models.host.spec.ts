...
describe('Web session model selection', () => {
  ...
  it('does not commit a Session selection when its persisted default cannot be saved', async () => {
    const { ctx, agent, sessionId } = await harness()
    const initial = { provider: 'deepseek-official', model: 'deepseek-chat' }
    const beforeSeq = agent.session.seq
    const remote = createSessionTestRemote(ctx, {
      defaultModelSelection: () => initial,
      saveDefaultModelSelection: () => Promise.reject(new Error('settings unavailable')),
      cwd: '/tmp',
    })
    ...
    await expect(remote.selectModel(request({
      ...
      error: {
        code: 'gateway/internal',
        message: 'model selection could not be saved; the Session was not changed',
      },
    })
    expect(agent.session.seq).toBe(beforeSeq)
    expect(agent.session.snapshotEvents().some(event => event.type === 'model/selection')).toBe(false)
    expect(currentSelection(ctx, sessionId)).toEqual(initial)
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and rejects a later storage failure', async () => {
    const { ctx, agent, sessionId } = await harness()
    const saved: unknown[] = []
    let reject = false
    const remote = createSessionTestRemote(ctx, {
    ...
    // A selection that cannot become the persisted default must not diverge
    // this Session from the model a fresh Session will restore.
    reject = true
    const beforeSeq = agent.session.seq
    expect(await remote.selectModel(request({
