...
describe('DeepSeekSearchProvider error handling', () => {
  ...
  it('maps a network failure to WEB_PROVIDER_ERROR', async () => {
    const fetchMock = vi.fn(() => Promise.reject(new TypeError('connection refused')))
    vi.stubGlobal('fetch', fetchMock)
    const error = await rejectedWebError(searchProvider(options).search({ query: 'q' }))
    expect(error.code).toBe('WEB_PROVIDER_ERROR')
    expect(error.message).toContain('DeepSeek search request failed after 2 attempts: TypeError: connection refused')
  ...
  it('recovers from one transient network failure and records both physical dispatches', async () => {
    const recordRequest = vi.fn()
    const fetchMock = vi.fn()
      .mockRejectedValueOnce(new TypeError('socket reset'))
      .mockResolvedValueOnce(jsonResponse(searchResponse()))
    vi.stubGlobal('fetch', fetchMock)
    await expect(searchProvider({ ...options, recordRequest }).search({ query: 'q' }))
  ...
  it('does not retry a deterministic authentication failure', async () => {
    const fetchMock = vi.fn(async () => jsonResponse({ error: { message: 'invalid key' } }, { status: 401 }))
    vi.stubGlobal('fetch', fetchMock)
    await expect(searchProvider(options).search({ query: 'q' }))
