...
/** One transparent retry for transient network/provider failures. */
...
export class DeepSeekSearchProvider implements WebSearchProvider {
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const response = await this.request(endpoint, apiKey, options.apiVersion, body, options.recordRequest, signal)
  ...
  /**
   * Dispatch the recorded request, retrying exactly once for network failures,
   * 408/429 throttling, and 5xx provider failures. Authentication and other
   * deterministic 4xx errors fail immediately. Each physical dispatch is
   * recorded separately so a retry never becomes invisible to the audit log.
   */
  private async request(
    ...
    recordRequest: DeepSeekSearchProviderOptions['recordRequest'],
    signal?: AbortSignal,
  ): Promise<Response> {
    for (let attempt = 1; attempt <= REQUEST_ATTEMPTS; attempt += 1) {
      recordRequest?.({ endpoint, apiVersion, body })
      throwIfSearchAborted(signal)
      ...
      try {
        ...
        if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
        if (attempt < REQUEST_ATTEMPTS) continue
        throw searchEndpointError(
          endpoint,
          `DeepSeek search request failed after ${REQUEST_ATTEMPTS} attempts: ${describeNetworkError(error)}`,
          error,
        )
      }
      ...
      if (attempt < REQUEST_ATTEMPTS && isRetryableStatus(status)) continue
      throw searchEndpointError(endpoint, message)
    }
    throw searchEndpointError(endpoint, 'DeepSeek search request did not dispatch')
  }
...
/** Statuses that can recover without a user changing credentials or input. */
function isRetryableStatus(status: number): boolean {
  return status === 408 || status === 429 || status >= 500
}

/** Retain the native fetch cause code in copyable diagnostics when available. */
function describeNetworkError(error: unknown): string {
  const detail = String(error)
  const cause = error instanceof Error ? error.cause : undefined
  if (typeof cause !== 'object' || cause === null || !('code' in cause)) return detail
