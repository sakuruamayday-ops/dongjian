...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Product-facing control for the provider-owned Files lifecycle.',
    description: 'Product-facing control for the provider-owned Files lifecycle.',
    methods: [
      {
        signature: 'releaseAll(signal?: AbortSignal): Promise<number>',
        description: 'Delete every Harness-owned remote file for the active endpoint and credential scope.',
  ...
  },
  {
    ...
    summary: 'Host-owned single-device account session backed by the operating-system credential store.',
    description: 'Host-owned single-device account session backed by the operating-system credential store. Renderer snapshots never contain the password, device id, or access token.',
    methods: [
      {
        signature: '@Remote(\'snapshot\') snapshot(): GongchuangAccountSnapshot',
        description: 'Read account presentation state without network activity.',
        parameters: [],
        returns: 'The current redacted account state.',
      },
      {
        signature: '@Remote(\'personalization\') async personalization(): Promise<GongchuangPersonalizationSnapshot>',
        description: 'Read the user-authored part of the product-owned global AGENTS.md.',
        parameters: [],
        returns: 'Personalization text without its fixed professional-boundary wrapper.',
      },
      {
        signature: '@Remote(\'savePersonalization\') async savePersonalization( request: GongchuangPersonalizationSaveRequest, ): Promise<GongchuangPersonalizationSnapshot>',
        description: 'Atomically replace the model-visible local preferences for future conversations.',
        parameters: [{ name: 'request', description: 'Complete user-authored preference text.' }],
        returns: 'The persisted redacted snapshot.',
      },
      {
        signature: '@Remote(\'login\') login(request: GongchuangAccountLoginRequest): Promise<GongchuangAccountSnapshot>',
        description: 'Authenticate the supplied account and make this machine the active device.',
        parameters: [{ name: 'request', description: 'Username, password source, and saved-password preference.' }],
        returns: 'Redacted connected state after credentials are committed.',
      },
      {
        signature: '@Remote(\'refresh\') refresh(): Promise<GongchuangAccountSnapshot>',
        description: 'Verify the saved session or perform permitted saved-password auto-login.',
        parameters: [],
        returns: 'Current connected, offline, superseded, or disconnected state.',
      },
      {
        signature: '@Remote(\'disconnect\') disconnect(request: GongchuangAccountDisconnectRequest): Promise<GongchuangAccountSnapshot>',
        description: 'Remove the active token and optionally forget all saved login material.',
        parameters: [{ name: 'request', description: 'Whether username and password are also removed from the credential store.' }],
  ...
  },
  {
    ...
    summary: 'Host-owned local graph memory with client-safe preferences and aggregate statistics.',
    description: 'Host-owned local graph memory with client-safe preferences and aggregate statistics.',
    methods: [
      {
        signature: '@Remote(\'snapshot\') async snapshot(): Promise<GongchuangGraphMemorySnapshot>',
        description: 'Read aggregate local-memory state without exposing content or filesystem paths.',
        parameters: [],
        returns: 'Current enabled state and aggregate graph counts.',
      },
      {
        signature: '@Remote(\'configure\') configure(request: GongchuangGraphMemoryConfigureRequest): Promise<GongchuangGraphMemorySnapshot>',
        description: 'Persist and immediately apply the user-controlled local-memory preferences.',
        parameters: [{ name: 'request', description: 'Complete enabled and tool-result preference values.' }],
        returns: 'Updated aggregate local-memory state.',
      },
      {
        signature: '@Remote(\'clearAll\') clearAll(): Promise<GongchuangGraphMemorySnapshot>',
        description: 'Permanently clear every personal and enterprise graph-memory database on this device.',
        parameters: [],
  ...
  },
  {
    ...
    summary: 'Host-owned persistent automation registry and claim broker.',
    description: 'Host-owned persistent automation registry and claim broker.',
    methods: [
      {
        signature: '@Remote(\'snapshot\') async snapshot(): Promise<AutomationSnapshot>',
        description: 'Read the complete local automation registry.',
        parameters: [],
        returns: 'Persisted tasks and aggregate run state after pending mutations settle.',
      },
      {
        signature: '@Remote(\'createTask\') async createTask(request: AutomationCreateRequest): Promise<AutomationTaskView>',
        description: 'Create a task or update the task already bound to the same template id.',
        parameters: [{ name: 'request', description: 'Validated local schedule, prompt, and enterprise workspace selection.' }],
        returns: 'Persisted client-safe task state.',
      },
      {
        signature: '@Remote(\'updateTask\') async updateTask(request: AutomationUpdateRequest): Promise<AutomationTaskView>',
        description: 'Edit an existing task without replacing its identity or run history.',
        parameters: [{ name: 'request', description: 'Existing task id plus its complete editable definition.' }],
        returns: 'Updated persisted task state.',
      },
      {
        signature: '@Remote(\'setTaskEnabled\') async setTaskEnabled(request: AutomationEnableRequest): Promise<AutomationTaskView>',
        description: 'Enable or disable one task and recompute its next local run time.',
        parameters: [{ name: 'request', description: 'Opaque task id and desired scheduler state.' }],
        returns: 'Updated persisted task state.',
      },
      {
        signature: '@Remote(\'deleteTask\') async deleteTask(request: AutomationDeleteRequest): Promise<{ id: AutomationTaskId }>',
        description: 'Stop owned execution and remove the task; conversation and files stay intact.',
        parameters: [{ name: 'request', description: 'Opaque task id.' }],
        returns: 'The removed task id for client confirmation.',
      },
      {
        signature: '@Remote(\'claimDue\') async claimDue(): Promise<AutomationClaim | null>',
        description: 'Atomically claim the earliest task whose local schedule is due.',
        parameters: [],
        returns: 'The earliest due one-time claim, or `null` when no task is due.',
      },
      {
        signature: '@Remote(\'runTaskNow\') async runTaskNow(request: AutomationRunRequest): Promise<AutomationClaim>',
        description: 'Issue a one-time manual execution claim without changing the periodic schedule.',
        parameters: [{ name: 'request', description: 'Opaque id of the task to run.' }],
        returns: 'New claim containing a secret completion token.',
      },
      {
        signature: '@Remote(\'bindRunSession\') async bindRunSession(request: AutomationBindSessionRequest): Promise<AutomationTaskView>',
        description: 'Persist the dedicated conversation before an active run sends its prompt. Repeating the same binding is idempotent. Replacement requires the active run and task to still match the caller\'s previous id, so deletion recovery cannot overwrite a newer conversation chosen by another client.',
        parameters: [{ name: 'request', description: 'Authenticated active run plus the materialized Session id.' }],
        returns: 'Updated task view carrying the durable conversation binding.',
      },
      {
        signature: '@Remote(\'dispatchRun\') async dispatchRun(request: AutomationDispatchRequest): Promise<{ requestId: SessionRequestId }>',
        description: 'Admit a claimed prompt through the upstream Session API under the same queue as deletion.',
        parameters: [{ name: 'request', description: 'Active claim and the exact task message to send.' }],
        returns: 'Identity of the admitted user message for completion tracking.',
      },
      {
        signature: '@Remote(\'completeRun\') async completeRun(request: AutomationCompleteRequest): Promise<AutomationTaskView | null>',
        description: 'Settle one active claim after authenticating its one-time token.',
        parameters: [{ name: 'request', description: 'Run id, claim token, terminal status, and bounded receipt message.' }],
  ...
  },
  {
    ...
    summary: 'Host-owned verified state for the product model choices.',
    description: 'Host-owned verified state for the product model choices.',
    methods: [
      {
        signature: '@Remote(\'snapshot\') async snapshot(): Promise<GongchuangModelConnectionsSnapshot>',
        description: 'Read the first settled startup state, then the latest redacted state without new network activity.',
        parameters: [],
        returns: 'Immutable provider status without keys or credential references.',
      },
      {
        signature: '@Remote(\'deepseekFiles\') deepseekFiles(): GongchuangDeepSeekFilesView',
        description: 'Read the current DeepSeek remote-image lifetime without exposing credentials or file ids.',
        parameters: [],
        returns: 'Current resolved retention and the three product-supported choices.',
      },
      {
        signature: '@Remote(\'setDeepSeekFileRetention\') setDeepSeekFileRetention( request: GongchuangDeepSeekFileRetentionRequest, ): Promise<GongchuangDeepSeekFilesView>',
        description: 'Apply one product-supported lifetime to future DeepSeek image uploads.',
        parameters: [{ name: 'request', description: 'One hour, seven days, or thirty days.' }],
        returns: 'Resolved redacted settings after the atomic update.',
      },
      {
        signature: '@Remote(\'clearDeepSeekFiles\') clearDeepSeekFiles(): Promise<GongchuangDeepSeekFilesCleanupReceipt>',
        description: 'Delete every Harness-owned DeepSeek file for the active credential and clear its local mapping.',
        parameters: [],
        returns: 'Redacted deletion count and completion time.',
      },
      {
        signature: '@Remote(\'refresh\') refresh(request: GongchuangModelRefreshRequest): Promise<GongchuangModelConnectionsSnapshot>',
        description: 'Re-check one or all configured providers using stored OS-keychain credentials.',
        parameters: [{ name: 'request', description: 'Optional provider filter; omission refreshes every product route.' }],
        returns: 'Redacted state after every requested probe settles.',
      },
      {
        signature: '@Remote(\'configure\') configure(request: GongchuangModelConfigureRequest): Promise<GongchuangModelConnectionsSnapshot>',
        description: 'Probe a supplied key and commit it only after the endpoint proves usable.',
        parameters: [{ name: 'request', description: 'Fixed official or validated custom-provider configuration.' }],
  ...
  },
  {
    ...
    summary: 'Capability-limited receipt ledger.',
    description: 'Capability-limited receipt ledger. Trusted producer plugins receive this service through declared Cordis injection and claim a receipt against the exact live tool execution. The post-execute gate commits the claim only when the tool succeeded and no later listener blocked its result.',
    methods: [
      {
        signature: 'claim( exec: ToolExecution, receiptId: string, subjectText?: string, artifactBinding?: ArtifactBinding, ledger?: ProfessionalLedger, ): void',
        description: 'Claim one receipt from an approved producer tool; settlement is deferred.',
        parameters: [{ name: 'exec', description: 'Live tool execution whose opaque token binds the claim.' }, { name: 'receiptId', description: 'Active receipt authorized for the executing tool name.' }, { name: 'subjectText', description: 'Exact normalized chat candidate to bind; omit for artifact receipts.' }, { name: 'artifactBinding', description: 'Exact file and extracted-content identity; only the professional kernel may bind it.' }, { name: 'ledger', description: 'Attested evidence checkpoint reused by the same task\'s artifact phase.' }],
      },
      {
        signature: 'checkpointProfessionalLedger(exec: ToolExecution, ledger: ProfessionalLedger): void',
        description: 'Persist attested evidence for the artifact phase without granting the professional receipt. The pending write is committed only after the current tool result is accepted, exactly like a normal receipt claim.',
        parameters: [{ name: 'exec', description: 'Live validation execution that owns the pending checkpoint.' }, { name: 'ledger', description: 'Host-attested evidence and calculations to reuse for the artifact.' }],
      },
      {
        signature: 'requiredProfessionalSkills(exec: ToolExecution): readonly string[]',
        description: 'Return the professional skills required by the live execution\'s current turn.',
        parameters: [{ name: 'exec', description: 'Live execution whose agent owns the active turn state.' }],
        returns: 'A sorted immutable snapshot of required skill names.',
      },
      {
        signature: 'professionalContractSkills(exec: ToolExecution): readonly string[]',
        description: 'Return only task-owning skills whose response structure applies.',
        parameters: [{ name: 'exec', description: 'Live execution whose agent owns the active turn state.' }],
        returns: 'A sorted immutable snapshot of task-owning skill names.',
      },
      {
        signature: 'professionalResponseDepth(exec: ToolExecution): \'query\' | \'analysis\' | \'formal\'',
        description: 'Return the Host-derived response depth for the live professional turn.',
        parameters: [{ name: 'exec', description: 'Tool execution attributed to the active agent.' }],
        returns: 'The response depth fixed for the current professional turn.',
      },
      {
        signature: 'assertSkillActivated(exec: ToolExecution, skill: string): void',
        description: 'Require a successful skill activation in the current attributed turn. The signed script runner calls this inside its executor, so an alternate caller cannot substitute a skill name without loading its instructions.',
        parameters: [{ name: 'exec', description: 'Live tool execution whose agent owns the active turn.' }, { name: 'skill', description: 'Exact owning skill declared by the signed operation registry.' }],
      },
      {
        signature: 'attestEvidence( exec: ToolExecution, evidence: readonly ProfessionalEvidence[], sourceText?: string, ): readonly ProfessionalEvidence[]',
        description: 'Bind model-declared evidence rows to user text or successful trusted tool output. A model-supplied label, URL, or digest is never sufficient on its own.',
        parameters: [{ name: 'exec', description: 'Live professional-validation execution.' }, { name: 'evidence', description: 'Candidate evidence rows supplied by the model.' }, { name: 'sourceText', description: 'Optional original text used by a rewriting task.' }],
        returns: 'Immutable evidence rows with Host-derived digests.',
      },
      {
        signature: 'sourceTextFromLocalRead(exec: ToolExecution, toolCallId: string): string',
        description: 'Resolve a long humanizer source from one exact trusted local-read receipt. This avoids asking the model to round-trip an entire customer document through JSON while keeping the fact lock bound to this attributed turn.',
        parameters: [{ name: 'exec', description: 'Live professional-validation execution.' }, { name: 'toolCallId', description: 'Exact call id injected by the Host after read/read_image succeeds.' }],
        returns: 'The normalized text captured from that trusted local read.',
      },
      {
        signature: 'recordEvidence( exec: ToolExecution, result: ToolExecutionResult, ): Readonly<EvidenceReceipt> | undefined',
        description: 'Capture successful trusted source output under its live tool call id.',
        parameters: [{ name: 'exec', description: 'Live tool execution carrying the agent and call identities.' }, { name: 'result', description: 'Successful result whose normalized text becomes evidence.' }],
        returns: 'The immutable receipt recorded for this turn, or undefined when the result is not eligible evidence.',
      },
      {
        signature: 'invalidateArtifact(exec: ToolExecution): void',
        description: 'Invalidate every content-bound receipt after a formal artifact write.',
        parameters: [{ name: 'exec', description: 'File mutation execution whose agent owns the stale receipts.' }],
      },
      {
        signature: 'artifactBinding(exec: ToolExecution): Readonly<ArtifactBinding>',
        description: 'Return the artifact identity committed by the current turn\'s professional kernel.',
        parameters: [{ name: 'exec', description: 'Live artifact-gate execution owned by the same agent.' }],
        returns: 'Immutable content and file identity.',
      },
      {
        signature: 'artifactBindingIfPresent(exec: ToolExecution): Readonly<ArtifactBinding> | undefined',
        description: 'Return a professional binding when this turn has one; generic file publication has none.',
        parameters: [{ name: 'exec', description: 'Live artifact-gate execution whose current-turn state is queried.' }],
        returns: 'Immutable content and file identity, or undefined for a generic publication.',
      },
      {
        signature: 'professionalSubject(exec: ToolExecution): string',
        description: 'Return the exact chat candidate already accepted by the professional kernel.',
        parameters: [{ name: 'exec', description: 'Live PDF-export execution owned by the same agent and turn.' }],
        returns: 'Exact normalized candidate text bound to the professional receipt.',
      },
      {
        signature: 'professionalSubjectIfPresent(exec: ToolExecution): string | undefined',
        description: 'Return the accepted chat candidate when the current turn has one. Signed Office generators use this optional form so ordinary, non-professional document creation keeps its existing parameter contract.',
        parameters: [{ name: 'exec', description: 'Live signed-operation execution owned by the same agent and turn.' }],
        returns: 'Exact normalized candidate text when the turn has a professional receipt.',
      },
      {
        signature: 'settle(exec: ToolExecution, acceptedSuccess: boolean): void',
        description: 'Commit or discard every claim bound to one completed execution.',
        parameters: [{ name: 'exec', description: 'Completed execution carrying the original claim token.' }, { name: 'acceptedSuccess', description: 'Whether execution and all post-execute listeners accepted the result.' }],
      },
    ],
  },
  {
    key: 'gongchuangSignedSkillRuntime',
    summary: 'Concrete Host service and sole executor for signed first-party skill scripts.',
    description: 'Concrete Host service and sole executor for signed first-party skill scripts.',
    methods: [
      {
        signature: 'async run(exec: ToolExecution, operationId: string, parameters: unknown): Promise<GongchuangSkillOperationResult>',
        description: 'Execute one registry operation after skill activation, hash, argv, path, environment, sandbox, timeout, and output checks.',
        parameters: [{ name: 'exec', description: 'Attributed model tool execution from the central broker.' }, { name: 'operationId', description: 'Exact signed operation identity.' }, { name: 'parameters', description: 'Closed structured values rendered by the Host.' }],
  ...
  },
  {
    ...
    summary: 'Host-side verified skill catalog and installer.',
    description: 'Host-side verified skill catalog and installer.',
    methods: [
      {
        signature: '@Remote(\'snapshot\') async snapshot(): Promise<SkillMarketplaceSnapshot>',
        description: 'Read marketplace state and refresh the rotating featured selection.',
        parameters: [],
        returns: 'Installed, featured, source, and custom-repository marketplace state.',
      },
      {
        signature: '@Remote(\'search\') async search(request: SkillSearchRequest): Promise<SkillSearchPage>',
        description: 'Search one official market or locally registered repository with pagination.',
        parameters: [{ name: 'request', description: 'Source, query, page, page size, and optional repository id.' }],
        returns: 'Normalized page with local installation state applied.',
      },
      {
        signature: '@Remote(\'addRepository\') async addRepository(request: SkillRepositoryAddRequest): Promise<SkillRepositoryAddReceipt>',
        description: 'Validate and persist one public HTTPS repository manifest.',
        parameters: [{ name: 'request', description: 'Manifest URL with no embedded credentials.' }],
        returns: 'Registered repository metadata and the checks applied.',
      },
      {
        signature: '@Remote(\'removeRepository\') async removeRepository(request: SkillRepositoryRemoveRequest): Promise<SkillRepositoryRemoveReceipt>',
        description: 'Remove a registered repository while preserving skills already installed from it.',
        parameters: [{ name: 'request', description: 'Repository identity from the current snapshot.' }],
        returns: 'Removed repository metadata and the count of retained installed skills.',
      },
      {
        signature: '@Remote(\'installSkill\') async install(request: SkillInstallRequest): Promise<SkillInstallReceipt>',
        description: 'Download, unpack, and register one selected skill version in the user community directory.',
        parameters: [{ name: 'request', description: 'Marketplace source and immutable skill coordinate selected by the user.' }],
        returns: 'Installed skill metadata, or a reused receipt.',
      },
      {
        signature: '@Remote(\'removeSkill\') async remove(request: SkillRemoveRequest): Promise<SkillRemoveReceipt>',
        description: 'Remove one user-installed community skill from the active registry. Its directory is moved to the marketplace\'s recoverable `removed` area; bundled V1.6.7 skills are intentionally outside this registry.',
        parameters: [{ name: 'request', description: 'Installed community skill identity to remove.' }],
        returns: 'Identity and source of the recoverably removed skill.',
      },
      {
        signature: '@Remote(\'setSkillEnabled\') async setEnabled(request: SkillEnabledSetRequest): Promise<SkillEnabledSetReceipt>',
        description: 'Enable or disable an installed community skill without deleting its files.',
        parameters: [{ name: 'request', description: 'Installed skill identity and requested enabled state.' }],
  ...
  },
  {
    ...
    summary: 'Immutable Host facts produced only after skill and runtime verification.',
    description: 'Immutable Host facts produced only after skill and runtime verification.',
    methods: [
      {
        signature: 'readonly paddleOcrMcpVersion: string',
        description: 'Reviewed PaddleOCR MCP distribution in the same verified Python tree.',
        parameters: [],
      },
    ],
  },
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'disposeSession(sessionId: SessionId): Promise<void>',
        description: 'Host-only lifecycle boundary used before moving a Session persistence artifact. The browser Remote does not expose this capability.',
        parameters: [{ name: 'sessionId', description: 'ordinary Session identity to quiesce.' }],
      },
      {
        signature: 'releaseSessionDisposal(sessionId: SessionId): void',
        description: 'Release a Host deletion barrier after its recoverable transaction rolls back.',
        parameters: [{ name: 'sessionId', description: 'ordinary Session identity whose deletion was rolled back.' }],
      },
      {
      ...
      {
        signature: 'fileAttachmentPath(sessionId: SessionId, attachment: FileAttachmentRef): Promise<string>',
        description: 'Resolve one Session-authorized durable file for a same-host desktop integration. The browser Remote cannot call this method.',
        parameters: [{ name: 'sessionId', description: 'Session whose durable log owns the reference.' }, { name: 'attachment', description: 'Complete file reference shown by the client.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: '@Remote(\'archiveSession\') archiveSession(request: WorkspaceArchiveSessionRequest): Promise<WorkspaceManagementValue>',
      ...
      {
        signature: '@Remote(\'setSessionPinned\') setSessionPinned(request: WorkspaceSetSessionPinnedRequest): Promise<WorkspaceValue>',
        description: 'Pin or unpin one accounted Session.',
        parameters: [{ name: 'request', description: 'Session identity and desired pinned state.' }],
        returns: 'the updated Workspace projection.',
      },
      {
        signature: '@Remote(\'archiveWorkspace\') archiveWorkspace(request: WorkspaceManagementRequest): Promise<WorkspaceManagementValue>',
        description: 'Hide one Workspace registration from active lists.',
        parameters: [{ name: 'request', description: 'Workspace identity to archive.' }],
        returns: 'the complete resulting management state.',
      },
      {
        signature: '@Remote(\'unarchiveWorkspace\') unarchiveWorkspace(request: WorkspaceManagementRequest): Promise<WorkspaceManagementValue>',
        description: 'Restore one archived Workspace registration.',
        parameters: [{ name: 'request', description: 'Workspace identity to restore.' }],
        returns: 'the complete resulting management state.',
      },
      {
        signature: '@Remote(\'deleteWorkspace\') deleteWorkspace(request: WorkspaceManagementRequest): Promise<WorkspaceManagementValue>',
        description: 'Hide one Workspace registration behind a recoverable tombstone.',
        parameters: [{ name: 'request', description: 'Workspace identity to hide.' }],
        returns: 'the complete resulting management state.',
      },
      {
        signature: '@Remote(\'unarchiveSession\') unarchiveSession(request: WorkspaceSessionManagementRequest): Promise<WorkspaceManagementValue>',
        description: 'Restore one archived Session to active grouping surfaces.',
        parameters: [{ name: 'request', description: 'Session identity to restore.' }],
        returns: 'the complete resulting management state.',
      },
      {
        signature: '@Remote(\'deleteSession\') deleteSession(request: WorkspaceSessionManagementRequest): Promise<WorkspaceManagementValue>',
        description: 'Hide one Session behind a recoverable client-history tombstone.',
        parameters: [{ name: 'request', description: 'Session identity to hide.' }],
        returns: 'the complete resulting management state.',
      },
      {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'archiveWorkspace(workspaceId: WorkspaceId): Promise<void>',
        description: 'Archive a Workspace registration while retaining its directory, record, and session account.',
        parameters: [{ name: 'workspaceId', description: 'The Workspace registration to archive.' }],
        returns: 'resolution after the archive state is durable.',
      },
      {
        signature: 'unarchiveWorkspace(workspaceId: WorkspaceId): Promise<void>',
        description: 'Restore an archived Workspace registration to the active list.',
        parameters: [{ name: 'workspaceId', description: 'The archived Workspace registration to restore.' }],
        returns: 'resolution after the active state is durable.',
      },
      {
        signature: 'deleteWorkspace(workspaceId: WorkspaceId): Promise<void>',
        description: 'Soft-delete a Workspace registration while retaining its record, directory, files, and sessions.',
        parameters: [{ name: 'workspaceId', description: 'The Workspace registration to hide behind a recoverable tombstone.' }],
        returns: 'resolution after the tombstone is durable.',
      },
      {
        signature: 'restoreDeletedWorkspace(workspaceId: WorkspaceId): Promise<void>',
        description: 'Restore a soft-deleted Workspace registration to active projections.',
        parameters: [{ name: 'workspaceId', description: 'The soft-deleted Workspace registration to restore.' }],
      ...
      },
      {
        ...
        description: 'Soft-delete one Session from client history. The persistence artifact and enterprise directory remain untouched; only a durable tombstone is added.',
        parameters: [{ name: 'sessionId', description: 'The Session to hide behind a recoverable tombstone.' }],
        returns: 'resolution after the tombstone is durable.',
      },
      {
        signature: 'restoreSession(sessionId: SessionId): Promise<void>',
        description: 'Restore a soft-deleted Session to client history without touching its log or files.',
        parameters: [{ name: 'sessionId', description: 'The soft-deleted Session to restore.' }],
        returns: 'resolution after the tombstone is removed durably.',
      },
      {
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'api-session/fork-preparing',
    mode: 'parallel',
    signature: '\'api-session/fork-preparing\'(input: { readonly events: readonly SessionEvent[] readonly sourceCwd: string readonly targetCwd: string }): Promise<void>',
    summary: 'Prepare referenced workspace files before a cross-workspace fork creates its child.',
    description: 'Prepare referenced workspace files before a cross-workspace fork creates its child.',
    parameters: [{ name: 'input', description: 'Immutable inherited events and source/target working directories.' }],
  },
  {
  ...
  {
    name: 'gongchuang-connectors/changed',
    mode: 'emit',
    signature: '\'gongchuang-connectors/changed\'(revision: number): void',
    summary: 'The redacted connector snapshot changed.',
    description: 'The redacted connector snapshot changed. Consumers must re-read the snapshot rather than retaining a stale ready state.',
    parameters: [{ name: 'revision', description: 'Monotonic snapshot revision available after the change.' }],
  },
  {
  ...
  {
    name: 'mcp/connection-state',
    mode: 'emit',
    signature: '\'mcp/connection-state\'(state: McpConnectionState): void',
    summary: 'Reports a credential-free lifecycle snapshot for one supervised MCP connection.',
    description: 'Reports a credential-free lifecycle snapshot for one supervised MCP connection.',
    parameters: [{ name: 'state', description: 'Current transport generation, retry count, and user-safe message.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'AutomationBindSessionRequest',
    declaration: 'export interface AutomationBindSessionRequest {\n    readonly runId: AutomationRunId;\n    readonly runToken: string;\n    readonly sessionId: string;\n    readonly previousSessionId?: string;\n}',
  },
  {
    name: 'AutomationClaim',
    declaration: 'export interface AutomationClaim {\n    readonly runId: AutomationRunId;\n    readonly runToken: string;\n    readonly taskId: AutomationTaskId;\n    readonly taskName: string;\n    readonly prompt: string;\n    readonly workspaceId: string;\n    readonly conversationSessionId: string | null;\n    readonly scheduledAt: string;\n    readonly manual: boolean;\n}',
  },
  {
    name: 'AutomationCompleteRequest',
    declaration: 'export interface AutomationCompleteRequest {\n    readonly runId: AutomationRunId;\n    readonly runToken: string;\n    readonly status: \'dispatched\' | \'failed\';\n    readonly message: string;\n    readonly sessionId?: string;\n}',
  },
  {
    name: 'AutomationCreateRequest',
    declaration: 'export interface AutomationCreateRequest {\n    readonly templateId?: string;\n    readonly name: string;\n    readonly prompt: string;\n    readonly workspaceId: string;\n    readonly everySeconds: number;\n    readonly cadenceLabel: string;\n    readonly enabled: boolean;\n    readonly firstRunAt?: string;\n}',
  },
  {
    name: 'AutomationDeleteRequest',
    declaration: 'export interface AutomationDeleteRequest {\n    readonly id: AutomationTaskId;\n}',
  },
  {
    name: 'AutomationDispatchRequest',
    declaration: 'export interface AutomationDispatchRequest {\n    readonly runId: AutomationRunId;\n    readonly runToken: string;\n    readonly content: string;\n}',
  },
  {
    name: 'AutomationEnableRequest',
    declaration: 'export interface AutomationEnableRequest {\n    readonly id: AutomationTaskId;\n    readonly enabled: boolean;\n}',
  },
  {
    name: 'AutomationLastRunView',
    declaration: 'export type AutomationLastRunView = AutomationRunView;',
  },
  {
    name: 'AutomationRunId',
    declaration: 'export type AutomationRunId = Branded<\'GongchuangAutomationRunId\'>;',
  },
  {
    name: 'AutomationRunRequest',
    declaration: 'export interface AutomationRunRequest {\n    readonly id: AutomationTaskId;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface AutomationSnapshot {\n    readonly revision: number;\n    readonly tasks: readonly AutomationTaskView[];\n    readonly running: number;\n    readonly dispatched: number;\n    readonly failed: number;\n}',
  },
  {
    name: 'AutomationTaskId',
    declaration: 'export type AutomationTaskId = Branded<\'GongchuangAutomationTaskId\'>;',
  },
  {
  ...
  {
    name: 'AutomationUpdateRequest',
    declaration: 'export interface AutomationUpdateRequest extends AutomationCreateRequest {\n    readonly id: AutomationTaskId;\n}',
  },
  {
  ...
  {
    name: 'GongchuangAccountDisconnectRequest',
    declaration: 'export interface GongchuangAccountDisconnectRequest {\n    readonly forgetSavedLogin: boolean;\n}',
  },
  {
    name: 'GongchuangAccountLoginRequest',
    declaration: 'export interface GongchuangAccountLoginRequest {\n    readonly username: string;\n    readonly password: string;\n    readonly useSavedPassword: boolean;\n    readonly rememberPassword: boolean;\n}',
  },
  {
    name: 'GongchuangAccountPhase',
    declaration: 'export type GongchuangAccountPhase = \'checking\' | \'disconnected\' | \'authenticating\' | \'connected\' | \'upgrade-required\' | \'offline\' | \'superseded\' | \'error\';',
  },
  {
    name: 'GongchuangAccountSnapshot',
    declaration: 'export interface GongchuangAccountSnapshot {\n    readonly revision: number;\n    readonly phase: GongchuangAccountPhase;\n    readonly portalUrl: string;\n    readonly username: string | null;\n    readonly message: string;\n    readonly hasSavedPassword: boolean;\n    readonly autoLoginBlocked: boolean;\n    readonly singleDevice: true;\n    readonly clientCompatibility: GongchuangClientCompatibility | null;\n    readonly minimumSupportedVersion: string | null;\n}',
  },
  {
    name: 'GongchuangClientCompatibility',
    declaration: 'export type GongchuangClientCompatibility = \'supported\' | \'upgrade-advised\' | \'upgrade-required\';',
  },
  {
    name: 'GongchuangCustomApiProtocol',
    declaration: 'export type GongchuangCustomApiProtocol = GongchuangProviderProtocol;',
  },
  {
    name: 'GongchuangCustomModelConfigureRequest',
    declaration: 'export interface GongchuangCustomModelConfigureRequest {\n    readonly provider: \'custom\';\n    readonly apiKey?: string;\n    readonly displayName: string;\n    readonly baseURL: string;\n    readonly protocol: GongchuangCustomApiProtocol;\n    readonly modelId?: string;\n}',
  },
  {
    name: 'GongchuangDeepSeekFileRetentionRequest',
    declaration: 'export interface GongchuangDeepSeekFileRetentionRequest {\n    readonly retentionSeconds: GongchuangDeepSeekFileRetentionSeconds;\n}',
  },
  {
    name: 'GongchuangDeepSeekFileRetentionSeconds',
    declaration: 'export type GongchuangDeepSeekFileRetentionSeconds = 3600 | 604800 | 2592000;',
  },
  {
    name: 'GongchuangDeepSeekFilesCleanupReceipt',
    declaration: 'export interface GongchuangDeepSeekFilesCleanupReceipt {\n    readonly deletedRemoteFiles: number;\n    readonly clearedAt: string;\n}',
  },
  {
    name: 'GongchuangDeepSeekFilesView',
    declaration: 'export interface GongchuangDeepSeekFilesView {\n    readonly retentionSeconds: number;\n    readonly retentionOptions: readonly GongchuangDeepSeekFileRetentionSeconds[];\n}',
  },
  {
    name: 'GongchuangGraphMemoryConfigureRequest',
    declaration: 'export interface GongchuangGraphMemoryConfigureRequest {\n    readonly enabled: boolean;\n    readonly includeToolResults: boolean;\n}',
  },
  {
    name: 'GongchuangGraphMemorySnapshot',
    declaration: 'export interface GongchuangGraphMemorySnapshot {\n    readonly enabled: boolean;\n    readonly includeToolResults: boolean;\n    readonly personalNodes: number;\n    readonly enterpriseStores: number;\n    readonly totalNodes: number;\n    readonly totalEdges: number;\n    readonly updatedAt: string | null;\n}',
  },
  {
    name: 'GongchuangModelConfigureRequest',
    declaration: 'export type GongchuangModelConfigureRequest = GongchuangOfficialModelConfigureRequest | GongchuangCustomModelConfigureRequest;',
  },
  {
    name: 'GongchuangModelConnectionPhase',
    declaration: 'export type GongchuangModelConnectionPhase = \'missing\' | \'checking\' | \'candidate\' | \'ready\' | \'error\';',
  },
  {
    name: 'GongchuangModelConnectionsSnapshot',
    declaration: 'export interface GongchuangModelConnectionsSnapshot {\n    readonly revision: number;\n    readonly providers: Readonly<Record<GongchuangModelProvider, GongchuangModelConnectionView>>;\n}',
  },
  {
    name: 'GongchuangModelConnectionView',
    declaration: 'export interface GongchuangModelConnectionView {\n    readonly provider: GongchuangModelProvider;\n    readonly route: string;\n    readonly phase: GongchuangModelConnectionPhase;\n    readonly configured: boolean;\n    readonly verifiedAt: string | null;\n    readonly modelCount: number;\n    readonly selectedModel: string | null;\n    readonly message: string;\n    readonly failureKind?: GongchuangModelDirectoryFailureKind;\n    readonly configuration?: {\n        readonly displayName: string;\n        readonly baseURL: string;\n        readonly protocol: GongchuangProviderProtocol;\n        readonly modelId: string;\n    };\n}',
  },
  {
    name: 'GongchuangModelDirectoryFailureKind',
    declaration: 'export type GongchuangModelDirectoryFailureKind = \'auth\' | \'transient\' | \'catalog\';',
  },
  {
    name: 'GongchuangModelProvider',
    declaration: 'export type GongchuangModelProvider = typeof GONGCHUANG_MODEL_PROVIDERS[number][\'id\'];',
  },
  {
    name: 'GongchuangModelRefreshRequest',
    declaration: 'export interface GongchuangModelRefreshRequest {\n    readonly provider?: GongchuangModelProvider;\n}',
  },
  {
    name: 'GongchuangOfficialModelConfigureRequest',
    declaration: 'export interface GongchuangOfficialModelConfigureRequest {\n    readonly provider: Exclude<GongchuangModelProvider, \'custom\'>;\n    readonly apiKey?: string;\n    readonly baseURL?: string;\n    readonly modelId?: string;\n}',
  },
  {
    name: 'GongchuangPersonalizationSaveRequest',
    declaration: 'export interface GongchuangPersonalizationSaveRequest {\n    readonly instructions: string;\n}',
  },
  {
    name: 'GongchuangPersonalizationSnapshot',
    declaration: 'export interface GongchuangPersonalizationSnapshot {\n    readonly instructions: string;\n    readonly updatedAt: string | null;\n    readonly maxCharacters: number;\n    readonly defaultInstructions?: string;\n}',
  },
  {
    name: 'GongchuangProviderProtocol',
    declaration: 'export type GongchuangProviderProtocol = \'openai-completions\' | \'openai-responses\' | \'anthropic-messages\';',
  },
  {
    name: 'GongchuangSkillOperationResult',
    declaration: 'export interface GongchuangSkillOperationResult {\n    readonly operation: string;\n    readonly skill: string;\n    readonly status: \'passed\' | \'rejected\';\n    readonly exitCode: number;\n    readonly stdout: string;\n    readonly stderr: string;\n    readonly sandboxEnforcement: SandboxEnforcement;\n    readonly scriptSha256: string;\n    readonly skillBundleIndexSha256: string;\n    readonly runtimeIntegrity: \'development-external\' | \'signed\';\n    readonly runtimeIndexSha256: string;\n}',
  },
  {
    name: 'GrantRecord',
  ...
  {
    name: 'MarketplaceRepositoryView',
    declaration: 'export interface MarketplaceRepositoryView {\n    readonly id: string;\n    readonly name: string;\n    readonly manifestUrl: string;\n    readonly homepage: string;\n    readonly skillCount: number;\n    readonly addedAt: string;\n}',
  },
  {
    name: 'MarketplaceSkillView',
    declaration: 'export interface MarketplaceSkillView {\n    readonly source: SkillMarketplaceSource;\n    readonly repositoryId?: string;\n    readonly coordinate: string;\n    readonly namespace: string;\n    readonly slug: string;\n    readonly name: string;\n    readonly description: string;\n    readonly category: string;\n    readonly version: string;\n    readonly license: string;\n    readonly downloads: number;\n    readonly iconUrl: string;\n    readonly detailUrl: string;\n    readonly publisherVerified: boolean;\n    readonly platformSigned: boolean;\n    readonly requiresConfiguration: boolean;\n    readonly configurationUrl?: string;\n    readonly featured: boolean;\n    readonly installed: boolean;\n}',
  },
  {
    name: 'McpConnectionPhase',
    declaration: 'export type McpConnectionPhase = \'connecting\' | \'ready\' | \'reconnecting\' | \'failed\' | \'disposed\';',
  },
  {
  ...
  {
    name: 'SkillEnabledSetReceipt',
    declaration: 'export interface SkillEnabledSetReceipt {\n    readonly id: string;\n    readonly name: string;\n    readonly enabled: boolean;\n}',
  },
  {
    name: 'SkillEnabledSetRequest',
    declaration: 'export interface SkillEnabledSetRequest {\n    readonly id: string;\n    readonly enabled: boolean;\n}',
  },
  {
    name: 'SkillEntry',
  ...
  {
    name: 'SkillInstallReceipt',
    declaration: 'export interface SkillInstallReceipt {\n    readonly installed: InstalledSkillView;\n    readonly reused: boolean;\n    readonly checks: readonly string[];\n}',
  },
  {
  ...
  {
    name: 'SkillRemoveReceipt',
    declaration: 'export interface SkillRemoveReceipt {\n    readonly id: string;\n    readonly name: string;\n    readonly source: SkillMarketplaceSource;\n    readonly coordinate: string;\n}',
  },
  {
  ...
  {
    name: 'SkillRepositoryAddReceipt',
    declaration: 'export interface SkillRepositoryAddReceipt {\n    readonly repository: MarketplaceRepositoryView;\n    readonly updated: boolean;\n    readonly checks: readonly string[];\n}',
  },
  {
    name: 'SkillRepositoryAddRequest',
    declaration: 'export interface SkillRepositoryAddRequest {\n    readonly manifestUrl: string;\n}',
  },
  {
    name: 'SkillRepositoryRemoveReceipt',
    declaration: 'export interface SkillRepositoryRemoveReceipt {\n    readonly id: string;\n    readonly name: string;\n    readonly installedSkillCount: number;\n}',
  },
  {
    name: 'SkillRepositoryRemoveRequest',
    declaration: 'export interface SkillRepositoryRemoveRequest {\n    readonly id: string;\n}',
  },
  {
  ...
  {
    name: 'SkillSearchPage',
    declaration: 'export interface SkillSearchPage {\n    readonly source: SkillMarketplaceSource;\n    readonly query: string;\n    readonly page: number;\n    readonly pageSize: number;\n    readonly total: number;\n    readonly skills: readonly MarketplaceSkillView[];\n}',
  },
  {
  ...
  {
    name: 'WorkspaceManagementRequest',
    declaration: 'export interface WorkspaceManagementRequest {\n    readonly workspaceId: WorkspaceId;\n}',
  },
  {
    name: 'WorkspaceManagementValue',
    declaration: 'export interface WorkspaceManagementValue {\n    readonly archivedWorkspaceIds: readonly WorkspaceId[];\n    readonly deletedWorkspaceIds: readonly WorkspaceId[];\n    readonly archivedSessionIds: readonly SessionId[];\n    readonly deletedSessionIds: readonly SessionId[];\n}',
  },
  {
  ...
  {
    name: 'WorkspaceSessionManagementRequest',
    declaration: 'export interface WorkspaceSessionManagementRequest {\n    readonly sessionId: SessionId;\n}',
  },
  {
    name: 'WorkspaceSetSessionPinnedRequest',
    declaration: 'export interface WorkspaceSetSessionPinnedRequest {\n    readonly workspaceId: WorkspaceId;\n    readonly sessionId: SessionId;\n    readonly pinned: boolean;\n}',
  },
  {
    name: 'WorkspaceValue',
