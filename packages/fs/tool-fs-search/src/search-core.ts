 ...
 * fall back to the platform package selected by `@vscode/ripgrep`; Electron
 * uses that package's physical ASAR-unpacked binary. Resolving
...
export function resolveRgPath(): Promise<string> {
  rgPathPromise ??= Promise.resolve().then(async () => {
    ...
    const dependencyPath = (await import('@vscode/ripgrep')).rgPath
    // Electron's fs can read an ASAR member, but the OS cannot spawn that virtual path.
    return process.versions.electron === undefined
      ? dependencyPath
      : dependencyPath.replace(/\.asar([\\/])/u, '.asar.unpacked$1')
  })
