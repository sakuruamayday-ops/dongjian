...
describe('readTextForDiff', () => {
  ...
  it.each(['open', 'stat'] as const)('observes cancellation immediately after %s', async (stage) => {
    ...
    try {
      ...
      const expectedReadBufferBytes = Buffer.byteLength('text') + 1
      const readBufferAllocations = allocate.mock.calls.filter(([size]) => size === expectedReadBufferBytes).length
      controller.abort()
      release.resolve(undefined)
      await expect(pending).rejects.toMatchObject({ code: 'FS_ABORTED' })
      expect(statCalls).toBe(stage === 'open' ? 0 : 1)
      expect(allocate.mock.calls.filter(([size]) => size === expectedReadBufferBytes)).toHaveLength(readBufferAllocations)
    } finally {
      release.resolve(undefined)
