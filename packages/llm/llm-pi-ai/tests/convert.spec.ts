...
describe('mapStopReason / mapUsage', () => {
  ...
  it('keeps catalog cost absent until the exact model price is known', () => {
    const priced = usage(10, 5)
    priced.cost.total = 0.0042
    expect(mapUsage(priced, true)).toEqual({
      inputTokens: 10,
      outputTokens: 5,
      totalTokens: 15,
      estimatedCostUsd: 0.0042,
    })
    expect(mapUsage(priced)).not.toHaveProperty('estimatedCostUsd')
  })
})
