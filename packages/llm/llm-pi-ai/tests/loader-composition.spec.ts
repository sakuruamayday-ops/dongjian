...
describe('llm-pi-ai real dormant composition', () => {
  it('loads OpenCode settings and sends the durable conversation id on successive requests', async () => {
    ...
    for (let turn = 0; turn < 2; turn += 1) {
      const result = await assemble(ctx, {
        provider: 'opencode-go', model: 'qa-model', messages: [], sessionId: SessionId('persisted-conversation'),
      })
      expect(result.message.content).toEqual([{ type: 'text', text: 'hello' }])
    }
    expect(server.headers.map(headers => headers['x-opencode-session']))
      .toEqual(['persisted-conversation', 'persisted-conversation'])
  })
