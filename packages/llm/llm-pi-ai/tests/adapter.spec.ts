...
const SessionId = brandString<NonNullable<GenerateOptions['sessionId']>>
...
describe('PiAiAdapter provider routing', () => {
  it('resolves a catalog model dynamically and uses a private endpoint', async () => {
    ...
    expect(result.usage?.estimatedCostUsd).toBeCloseTo(0.0000007, 12)
  ...
  it('rejects unknown catalog aliases and preserves explicit overrides for valid aliases', async () => {
    expect(() => resolveProfiles({ alias: { catalogProvider: 'missing-catalog' } })).toThrow('unknown catalogProvider')
    const server = await mockServer([{ events: textEvents }])
    const adapter = adapterOf({ alias: {
      catalogProvider: 'opencode', api: 'openai-completions', baseURL: server.url, models: [{ id: 'gpt-5.4' }],
    } })
    for await (const _chunk of adapter.stream({ provider: 'alias', model: 'gpt-5.4', messages: [] })) { /* Drain success. */ }
    expect(server.paths).toEqual(['/chat/completions'])
  })

  it.each(['openai-completions', 'openai-responses', 'anthropic-messages'] as const)(
    'sends OpenCode session routing over the %s wire protocol', async (api) => {
      const server = await mockServer([{ status: 400, body: '{"error":{"message":"wire capture complete"}}' }])
      const adapter = adapterOf({ 'opencode-go': {
        api, baseURL: server.url, models: [{ id: 'qa-model' }],
        headers: { 'X-OpenCode-Session': 'stale-static-id', 'User-Agent': 'wrong' },
      } })
      for await (const _chunk of adapter.stream({
        provider: 'opencode-go', model: 'qa-model', messages: [], sessionId: SessionId('conversation-a'),
      })) { /* Capture the real SDK request, including expected provider rejection. */ }
  ...
  it.each(['opencode-go', 'opencode', 'opencode-zen'])('keeps %s conversation headers stable without leaking between calls', async (provider) => {
    const server = await mockServer(Array.from({ length: 4 }, () => ({ events: textEvents })))
    const adapter = adapterOf({ [provider]: {
      api: 'openai-completions', baseURL: server.url, models: [{ id: 'qa-model' }],
      headers: { 'X-OPENCODE-SESSION': 'stale-static-id' },
    } })
    for (const sessionId of ['conversation-a', 'conversation-a', 'conversation-b', undefined]) {
      const prepared = await adapter.prepareCall(provider, 'qa-model')
      for await (const _chunk of prepared.stream({
        provider, model: 'qa-model', messages: [],
        ...sessionId === undefined ? {} : { sessionId: SessionId(sessionId) },
      })) { /* Drain successful completions on the prepared-call path. */ }
    }
    expect(server.headers.map(headers => headers['x-opencode-session']))
      .toEqual(['conversation-a', 'conversation-a', 'conversation-b', undefined])
  })

  it.each(['', '/v1', '/v1/', '/anthropic', '/anthropic/v1'])(
    'normalizes Anthropic SDK endpoints with suffix %s without changing persisted profiles', async (suffix) => {
      const server = await mockServer([{ status: 400, body: '{"error":{"message":"wire capture complete"}}' }])
      const baseURL = `${server.url}${suffix}`
      const profiles = { custom: { api: 'anthropic-messages' as const, baseURL, models: [{ id: 'qa-model' }] } }
      const adapter = adapterOf(profiles)
      for await (const _chunk of adapter.stream({ provider: 'custom', model: 'qa-model', messages: [] })) { /* Drain expected rejection. */ }
      expect(server.paths).toEqual([`${suffix.startsWith('/anthropic') ? '/anthropic' : ''}/v1/messages`])
      expect(profiles.custom.baseURL).toBe(baseURL)
    },
  )

  it('does not add OpenCode routing headers to unrelated providers', async () => {
    const server = await mockServer([{ events: textEvents }])
    const ctx = await harness(server.url)
    await assemble(ctx, { model: 'deepseek-v4-flash', messages: [], sessionId: SessionId('private-session') })
    expect(server.headers[0]?.['x-opencode-session']).toBeUndefined()
  })

  it.each(['opencode.ai', 'opencode.ai.example.test'])(
    'recognizes only the exact OpenCode endpoint for a custom route at %s', async (hostname) => {
      const server = await mockServer([{ events: textEvents }])
      const originalFetch = globalThis.fetch
      const forwarding = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
        const request = new Request(input, init)
        return originalFetch(new Request(`${server.url}/chat/completions`, request))
      })
      try {
        const adapter = adapterOf({ custom: {
          api: 'openai-completions', baseURL: `https://${hostname}/zen/go/v1`, models: [{ id: 'qa-model' }],
        } })
        for await (const _chunk of adapter.stream({
          provider: 'custom', model: 'qa-model', messages: [], sessionId: SessionId('custom-conversation'),
        })) { /* Redirect only the external transport to the local capture server. */ }
        expect(server.headers).toHaveLength(1)
        expect(server.headers[0]?.['x-opencode-session'])
          .toBe(hostname === 'opencode.ai' ? 'custom-conversation' : undefined)
      } finally {
  ...
  it('dispatches every advertised OpenCode Go effort for DeepSeek V4 Flash Vision Exp', async () => {
    ...
    for (const effort of efforts) {
      ...
      expect(result.usage?.estimatedCostUsd).toBeCloseTo(0.00000132, 12)
    }
    for (const [index, effort] of efforts.entries()) {
      expect(server.requests[index]).toMatchObject({
    ...
    expect(server.requests[3]).toMatchObject({ thinking: { type: 'disabled' } })
