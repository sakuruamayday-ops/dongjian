...
OpenCode and OpenCode Go requests send `GenerateOptions.sessionId` as `x-opencode-session` across completion, Responses, and Anthropic protocols, including custom routes whose endpoint hostname is exactly `opencode.ai`. The runtime value overrides case-insensitive profile collisions; requests without a session omit the header rather than reuse a static id. Other providers keep their existing headers. See the [routing decision](../../../.agents/notes/implemented/architecture/2026-09-08-opencode-session-routing.md).
...
| `api` | catalog protocol | Wire protocol; null clears an inherited override |
| `baseURL` | catalog endpoint | Endpoint of every model on the route; null clears an inherited override |
| `catalogProvider` | route name | Installed provider supplying native per-model protocols, endpoints and capabilities for a differently named route |
...
Installed-catalog metadata remains authoritative for modalities, reasoning levels, and list-price estimates. In the pinned OpenCode Go catalog, `deepseek-v4-flash-vision-exp` accepts text and images and offers exactly `off`, `low`, `high`, and `max`; omitting `reasoningEfforts` inherits those four choices. Its estimate uses the installed catalog's price metadata rather than a model-name rule, while a hand-declared model remains unpriced.

### Run with reasoning and wire compatibility

`reasoningEfforts` declares a model's selectable thinking levels: each key is a level selectors offer, its value the spelling dispatch sends on the wire, so `max: ultra` renames a level for a gateway with its own vocabulary. Omitting the field keeps the installed catalog entry's capability; `false` declares a non-reasoning model. `compat` switches reshape the request for endpoints pi-ai cannot recognize — which role carries the system prompt, which field caps output, how a thinking level travels — configurable per route and per model. For `openai-completions`, `thinkingBudgets` supplies the per-level budget and `compat.thinkingTokenBudgetField` selects `thinking_token_budget`, `thinking_budget`, or `thinking_budget_tokens`; `supportsThinkingTokenBudget: true` remains shorthand for `thinking_token_budget`, while the `thinking.budget` placeholder exposes the resolved budget to chat templates. Catalog-owned vendor routing such as Anthropic's `allowedFallbackModels` stays withheld from profiles. A model neither the entry nor the installed catalog sizes takes the route's `defaultContextWindow` and `defaultMaxTokens` fallbacks.
