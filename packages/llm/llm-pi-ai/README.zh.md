...
OpenCode 和 OpenCode Go 请求将 `GenerateOptions.sessionId` 写入 `x-opencode-session`，覆盖 completion、Responses 和 Anthropic 协议，也覆盖端点主机名精确等于 `opencode.ai` 的自定义路由。运行时值覆盖配置中不区分大小写的同名标头；没有会话的请求省略该标头，不复用静态编号。其他提供方保持原有标头。参见[路由决策](../../../.agents/notes/implemented/architecture/2026-09-08-opencode-session-routing.zh.md)。
...
| `api` | 目录协议 | 协议格式；null 清除继承的覆盖 |
| `baseURL` | 目录端点 | 路由上所有模型的端点；null 清除继承的覆盖 |
| `catalogProvider` | 路由名称 | 为不同名称的路由提供原生逐模型协议、端点及能力的已安装提供方 |
...
已安装目录元数据仍是模态、推理档位和目录价估算的权威来源。在当前锁定的 OpenCode Go 目录中，`deepseek-v4-flash-vision-exp` 接受文本与图片，并且只提供 `off`、`low`、`high`、`max` 四档；省略 `reasoningEfforts` 即继承这四个选项。其估算使用已安装目录的价格元数据，而不是按模型名称推断；手工声明的模型仍保持未计价。

### 带推理与协议兼容运行

`reasoningEfforts` 声明模型可选择的 thinking 等级：每个键都是选择器提供的等级，其值是该等级过线的拼写，因此 `max: ultra` 可以为拥有自有词汇的网关重命名等级。省略该字段时保留已安装目录条目的能力；`false` 声明非推理模型。对于 pi-ai 无法识别的端点，`compat` 开关重塑请求——哪个角色携带系统提示词、哪个字段限制输出、thinking 等级如何传递——可逐路由、逐模型配置。对 `openai-completions` 而言，`thinkingBudgets` 提供逐档预算，`compat.thinkingTokenBudgetField` 可选择 `thinking_token_budget`、`thinking_budget` 或 `thinking_budget_tokens`；`supportsThinkingTokenBudget: true` 仍是 `thinking_token_budget` 的简写，而 `thinking.budget` 占位符会把已解析预算交给聊天模板。Anthropic 的 `allowedFallbackModels` 等目录自有厂商路由信息不向 profile 开放。条目与已安装目录都没有尺寸的模型，会采用路由的 `defaultContextWindow` 与 `defaultMaxTokens` 回退值。
...
插件会回答"该提供方可以提供哪些模型？"，供配置界面正在编辑或起草的路由使用。已安装目录提供的路由直接由目录回答，不发网络请求；只有目录未描述的路由才会经网络询问。`openai-completions` 与 `openai-responses` 使用带 bearer 鉴权的 `GET {baseURL}/models`，`anthropic-messages` 则以 `x-api-key` 和 `anthropic-version` 使用原生 `GET /v1/models?limit=1000` 语义；其列表 URL 接受带或不带末尾 `/v1` 的 API 根地址，因为网关文档两种写法都会发布，推理时也会在调用 Anthropic SDK 前去除末尾 `/v1`，由 SDK 自行追加 `/v1/messages`。已配置且具名的路由会在 Host 内部提供已存凭据与 profile `headers`，因此通过 `settings.yaml` 或 Cordis 配置设置的部署标头可以到达模型发现请求，但不会成为发现请求或 Models 页面的字段；表单中新键入的密钥仍优先于已存凭据。解析器接受标准 `data` 数组或富信息 `models` 对象，并归一化每个候选的 id、显示名、上下文窗口与最大输出 token 数；Anthropic 的 `max_input_tokens` 与 `max_tokens` 会进入相同容量字段，即使对象条目点名了另一个规范 id，对象键仍是请求 id，原始类型的对象属性会被忽略，缺失的显示名则回退到该请求 id。回答是界面可以提供给用户采纳的候选元数据——不存储任何内容，`settings.yaml` 仍然是决定路由服务内容的唯一事实。
