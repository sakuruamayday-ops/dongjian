...
export interface ResolvedPiAiProviderProfile
  ...
  /** Exact model ids backed by the installed catalog's list-price metadata. */
...
export function resolveProfiles(
  ...
  for (const [provider, raw] of entries) {
    // Settings merge user profiles over composition defaults. Null must mask an old
    // route-wide override, otherwise refresh/restart revives the wrong wire protocol.
    const { api, baseURL, ...fields } = raw
    const source = { ...fields, ...api == null ? {} : { api }, ...baseURL == null ? {} : { baseURL } }
    if (source.catalogProvider !== undefined && !catalogProviderIds().includes(source.catalogProvider)) {
      throw new Error(`llm-pi-ai: unknown catalogProvider "${source.catalogProvider}" for route "${provider}"`)
    }
    ...
    resolved.set(provider, {
      ...
      pricedModels: catalog.pricedModels,
