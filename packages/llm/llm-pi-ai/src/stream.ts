 ...
 * @param pricingKnown - whether the exact model comes from pi-ai's priced installed catalog.
 * @returns harness counts with pi-ai's exact total; cache fields appear only
 *   when non-zero and estimated cost only when exact catalog pricing is known.
 */
export function mapUsage(usage: PiUsage, pricingKnown = false): TokenUsage {
  return {
    ...
    ...pricingKnown ? { estimatedCostUsd: usage.cost.total } : {},
  }
}
 ...
 * @param pricingKnown - whether the exact model has installed-catalog list-price metadata.
