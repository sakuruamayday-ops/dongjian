...
/** Merge deployment headers with runtime-owned attribution and OpenCode conversation routing. */
function requestHeaders(
  ...
  const openCode = model.provider === 'opencode' || model.provider === 'opencode-go' || model.provider === 'opencode-zen'
    || new URL(model.baseUrl).hostname === 'opencode.ai'
  // pi-ai does not send its sessionId option on every protocol. Go requires
  // a conversation-stable wire header, including title and compaction calls.
  if (openCode) reserved.add('x-opencode-session')
  return {
    ...Object.fromEntries(Object.entries(headers ?? {}).filter(([name]) => !reserved.has(name.toLowerCase()))),
    ...attribution,
    ...openCode && sessionId !== undefined ? { 'x-opencode-session': String(sessionId) } : {},
  }
}
...
export class PiAiAdapter extends LlmAdapter {
  ...
  private async * streamWithSnapshot(
    ...
    const resolvedModel = this.modelOf(snapshot, options.provider, options.model)
    // Anthropic SDK appends /v1/messages itself, including for persisted legacy profiles.
    const model = resolvedModel.api === 'anthropic-messages'
      ? { ...resolvedModel, baseUrl: resolvedModel.baseUrl.replace(/\/v1\/?$/, '') }
      : resolvedModel
    ...
    try {
      ...
      const iterator = toStreamChunks(
        ...
        model.contextWindow,
        options.signal,
        profile.pricedModels.has(model.id),
