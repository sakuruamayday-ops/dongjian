...
/** Pricing placeholder for a model the installed catalog does not describe. */
...
/** Drift gate over the upstream token-budget field spellings. */
...
/** The token-budget field spellings a profile may name. */
...
export interface RouteCatalogRequest {
  /** Installed provider supplying defaults when the route has a distinct durable name. */
...
export interface RouteCatalog {
  ...
  /** Model ids whose list price came from pi-ai's installed catalog. */
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const defaults = catalogModels(request.catalogProvider ?? provider)
  const providerBaseUrl = catalogProvider(request.catalogProvider ?? provider)?.baseUrl
  ...
  const models = entries.map((entry) => {
    ...
    if (base !== undefined) pricedModels.add(entry.id)
  ...
  return { models, configuredMaxTokens, pricedModels }
}
