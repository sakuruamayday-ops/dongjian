...
/** One local-calendar day's list-price estimate for a session. */
export interface ModelCostDayProjection {
  /** Sum of priced request estimates in USD. */
  estimatedCostUsd: number
  /** Requests carrying a trustworthy model-exact price, including known-free requests. */
  pricedRequests: number
  /** Requests that reported usage but whose exact route/model had no price. */
  unpricedRequests: number
}

/** Durable list-price estimates for a complete session log. */
export interface ModelCostProjection extends ModelCostDayProjection {
  /** Local-calendar buckets keyed as `YYYY-MM-DD` in the host's current time zone. */
  byDay: Readonly<Record<string, ModelCostDayProjection>>
}

/**
 * Convert an epoch timestamp into the local-calendar key used by
 * {@link ModelCostProjection.byDay}. The Electron Host and renderer share the
 * OS time zone, so a client can select today's bucket without a second clock
 * or server-side billing service.
 * @param time - Epoch milliseconds.
 * @returns Local calendar date as `YYYY-MM-DD`.
 */
export function localCostDayKey(time: number): string {
...
declare module '@deepseek-ai/dsh-session-projection/types' {
  interface SessionProjectionMap {
    ...
    /** Model-exact API list-price estimates accumulated from durable usage receipts. */
