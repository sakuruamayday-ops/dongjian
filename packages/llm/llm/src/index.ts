...
export class LlmRuntime extends TypertRemoteService {
  ...
  /** Restore tool-result adjacency and remove replay state owned by another adapter. */
  private forAdapter(options: GenerateOptions, adapter: LlmAdapter): GenerateOptions {
    const messages: Message[] = deferInterleavedNotices(options.messages).map((message) => {
...
/** Keep complete tool batches contiguous without rewriting their durable history. */
function deferInterleavedNotices(messages: readonly Message[]): Message[] {
  ...
  for (let index = 0; index < messages.length; index++) {
    ...
    for (let cursor = index + 1; cursor < messages.length; cursor++) {
      ...
      // Older plugins could append status while a tool was still executing.
      // Only defer text notices; never move user input or invent missing results.
      if (candidate.role === 'user' && candidate.source.kind === 'plugin'
        && candidate.source.form === 'notice'
        && candidate.content.every(block => block.type === 'text')) {
        notices.push(candidate)
        continue
      }
      if (candidate.role !== 'user' || candidate.content.length === 0
        || !candidate.content.every(block => block.type === 'tool-result' && pending.has(block.toolCallId))) break
    ...
    if (pending.size === 0 && notices.length > 0) {
      ordered.push(...results, ...notices)
      index = end
    }
  }
  return ordered
