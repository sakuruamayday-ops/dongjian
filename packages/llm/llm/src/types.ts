...
/**
 * Text-only fallback bound to one exact durable image.
 *
 * The original image remains in the conversation for UI display and future
 * image-capable routes. Text-only adapters may omit that image only when this
 * sibling proves a preprocessor handled the same content-addressed object.
 */
export interface ImageDescriptionBlock {
  type: 'image-description'
  /** Exact image object described by this fallback. */
  attachmentId: ImageAttachmentRef['attachmentId']
  /** Extracted or generated model-facing description. */
  text: string
  /** The preprocessing path that produced the description. */
  method: 'ocr' | 'vision-proxy'
  /** A model-facing boundary that prevents overclaiming from a lossy fallback. */
  limitations?: string
}
