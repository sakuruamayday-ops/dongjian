...
describe('LlmRuntime', () => {
  ...
  it.each(['between-call-and-result', 'between-results'])('defers an injected notice %s without mutating original messages', async (placement) => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const adapter = new RecordingAdapter(SCRIPT)
    ctx.llm.registerAdapter(['target'], adapter)
    const first = ToolCallId('first'), second = ToolCallId('second')
    const call = createMessage({
      role: 'assistant', source: { kind: 'model', provider: 'target', model: 'm' },
      content: [first, second].map(id => ({ type: 'tool-call' as const, id, name: 'read', arguments: '{}' })),
    })
    const notice = createUserMessage({
      source: { kind: 'plugin', plugin: 'delivery', form: 'notice', summary: 'File checked' },
      content: [{ type: 'text', text: 'File checked' }],
    })
    const result1 = createToolResultMessage({ callId: first, isError: false, content: [{ type: 'text', text: 'first bytes' }] })
    const result2 = createToolResultMessage({ callId: second, isError: false, content: [{ type: 'text', text: 'second bytes' }] })
    const messages = placement === 'between-results'
      ? [call, result2, notice, result1]
      : [call, notice, result2, result1]
    const original = JSON.stringify(messages)
    const options = Object.freeze({ provider: 'target', model: 'm', messages })
    for await (const _chunk of ctx.llm.stream(options)) { /* drain */ }
    expect(adapter.lastOptions?.messages).toEqual([call, result2, result1, notice])
    expect(adapter.lastOptions?.messages[3]).toBe(notice)
    expect(Object.isFrozen(adapter.lastOptions)).toBe(true)
    expect(JSON.stringify(messages)).toBe(original)
  })

  it.each(['user', 'incomplete', 'unrelated-result'])('does not repair %s by dropping input or fabricating tool results', async (scenario) => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const adapter = new RecordingAdapter(SCRIPT)
    ctx.llm.registerAdapter(['target'], adapter)
    const id = ToolCallId('expected')
    const call = createMessage({
      role: 'assistant', source: { kind: 'model', provider: 'target', model: 'm' },
      content: [{ type: 'tool-call', id, name: 'read', arguments: '{}' }],
    })
    const notice = createUserMessage({
      source: scenario === 'user' ? { kind: 'user' } : { kind: 'plugin', plugin: 'delivery', form: 'notice', summary: 'Status' },
      content: [{ type: 'text', text: 'Status' }],
    })
    const result = createToolResultMessage({
      callId: scenario === 'unrelated-result' ? ToolCallId('other') : id,
      isError: false, content: [{ type: 'text', text: 'bytes' }],
    })
    const messages = scenario === 'incomplete' ? [call, notice] : [call, notice, result]
    for await (const _chunk of ctx.llm.stream({ provider: 'target', model: 'm', messages })) { /* drain */ }
