...
At dispatch, a complete historical tool exchange interrupted only by plugin text notices is projected as its tool calls, matching results, then notices. This repairs stored notice interleaving without mutating the session log or dropping content. Real user messages, incomplete exchanges, and unrelated results are not reordered.
