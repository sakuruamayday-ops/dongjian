...
async function usageOf(stream: AsyncIterable<StreamChunk>): Promise<TokenUsage> {
  for await (const chunk of stream) {
    if (chunk.type === 'usage') return chunk.usage
  }
  throw new Error('stream completed without usage')
}
...
describe('DeepSeekAdapter against a mock server', () => {
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(result.usage?.estimatedCostUsd).toBeCloseTo(0.00000396, 12)
  ...
  it('prices delayed usage at the request instant when the stream crosses a peak boundary', async () => {
    vi.useFakeTimers({ toFake: ['Date'] })
    const beforePeak = Date.parse('2026-08-31T00:59:59.999Z')
    vi.setSystemTime(beforePeak)
    const requestSent = Promise.withResolvers<true>()
    ...
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(() => {
      expect(Date.now()).toBe(beforePeak)
      requestSent.resolve(true)
    ...
    try {
      const usage = usageOf(adapter.stream({
        provider: 'deepseek-official',
      ...
      responseController?.enqueue(encoder.encode(textEvents.map(event => `data: ${event}\n\n`).join('')))
      responseController?.close()

      expect((await usage).estimatedCostUsd).toBeCloseTo(0.00000132, 12)
    } finally {
  ...
  it('freezes a new pricing instant for the successful chat after a stale-file retry', async () => {
    vi.useFakeTimers({ toFake: ['Date'] })
    const beforePeak = Date.parse('2026-08-31T00:59:59.999Z')
    const peakStart = Date.parse('2026-08-31T01:00:00.000Z')
    vi.setSystemTime(beforePeak)
    const fetchInstants: number[] = []
    const stale = JSON.stringify({ error: { message: 'file_id file-api-1 expired' } })
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(() => {
      fetchInstants.push(Date.now())
      if (fetchInstants.length === 1) {
        vi.setSystemTime(peakStart)
        return Promise.resolve(new Response(stale, {
          status: 400,
          headers: { 'content-type': 'application/json' },
        }))
      }
      return Promise.resolve(successfulSseResponse())
    })
    const attachments = attachmentStoreOf(ref => Promise.resolve(requestImage(ref))).store
    let nextFile = 1
    const files = fileStoreOf(() => Promise.resolve(fileReference(`file-api-${nextFile++}`)))
    const adapter = adapterOf({
      baseURL: 'https://example.invalid',
      models: [{ id: 'deepseek-v4-flash-vision-exp', inputModalities: ['text', 'image'] }],
    }, attachments, files.store)
    try {
      const usage = await usageOf(adapter.stream({
        provider: 'deepseek-official',
        model: 'deepseek-v4-flash-vision-exp',
        messages: [createUserMessage({
          content: [{ type: 'image', attachment: imageRef }],
      ...
      expect(fetchInstants).toEqual([beforePeak, peakStart])
      expect(files.invalidate).toHaveBeenCalledOnce()
      expect(files.ensureUploaded).toHaveBeenCalledTimes(2)
      expect(usage.estimatedCostUsd).toBeCloseTo(0.00000264, 12)
    } finally {
      fetchSpy.mockRestore()
    }
  })
