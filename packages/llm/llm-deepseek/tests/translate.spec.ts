...
describe('mapUsage', () => {
  ...
  it.each([
    ['Flash off-peak', 'deepseek-v4-flash', '2026-08-31T00:30:00.000Z', 0.000001816],
    ['Flash peak', 'deepseek-v4-flash', '2026-08-31T01:30:00.000Z', 0.000003632],
    ['Pro off-peak', 'deepseek-v4-pro', '2026-08-31T00:30:00.000Z', 0.000005456],
    ['Pro peak', 'deepseek-v4-pro', '2026-08-31T06:30:00.000Z', 0.000010912],
    ['Vision off-peak', 'deepseek-v4-flash-vision-exp', '2026-08-31T00:30:00.000Z', 0.000001816],
    ['Vision peak', 'deepseek-v4-flash-vision-exp', '2026-08-31T06:30:00.000Z', 0.000003632],
  ])('estimates %s cache hit, cache miss, and output usage', (_name, model, instant, expected) => {
    const priced = mapUsage({
      prompt_tokens: 10,
      completion_tokens: 2,
      prompt_cache_hit_tokens: 8,
    }, model, Date.parse(instant))
    expect(priced).toMatchObject({ inputTokens: 2, cacheReadTokens: 8, outputTokens: 2 })
    expect(priced.estimatedCostUsd).toBeCloseTo(expected, 12)
  })

  it.each([
    ['before first peak', '2026-08-31T00:59:59.999Z', 0.00000022],
    ['first peak start', '2026-08-31T01:00:00.000Z', 0.00000044],
    ['first peak end', '2026-08-31T04:00:00.000Z', 0.00000022],
    ['before second peak', '2026-08-31T05:59:59.999Z', 0.00000022],
    ['second peak start', '2026-08-31T06:00:00.000Z', 0.00000044],
    ['second peak end', '2026-08-31T10:00:00.000Z', 0.00000022],
    ['weekend', '2026-08-30T07:00:00.000Z', 0.00000022],
  ])('uses the official UTC pricing boundary at %s', (_name, instant, expected) => {
    const priced = mapUsage(
  ...
  it.each([
    ['cache count above prompt', { prompt_tokens: 10, completion_tokens: 2, prompt_cache_hit_tokens: 11 }, true],
    ['negative cache count', { prompt_tokens: 10, completion_tokens: 2, prompt_cache_hit_tokens: -1 }, true],
    ['fractional cache count', { prompt_tokens: 10, completion_tokens: 2, prompt_cache_hit_tokens: 0.5 }, true],
    ['unsafe cache count', { prompt_tokens: 10, completion_tokens: 2, prompt_cache_hit_tokens: Number.MAX_SAFE_INTEGER + 1 }, true],
    ['negative prompt count', { prompt_tokens: -1, completion_tokens: 2 }, false],
    ['fractional prompt count', { prompt_tokens: 1.5, completion_tokens: 2 }, false],
    ['unsafe prompt count', { prompt_tokens: Number.MAX_SAFE_INTEGER + 1, completion_tokens: 0 }, false],
    ['negative completion count', { prompt_tokens: 2, completion_tokens: -1 }, false],
    ['fractional completion count', { prompt_tokens: 2, completion_tokens: 1.5 }, false],
    ['unsafe completion count', { prompt_tokens: 0, completion_tokens: Number.MAX_SAFE_INTEGER + 1 }, false],
  ])('omits a known-model estimate for %s without changing token fields', (_name, wire, hasTotal) => {
    const cacheRead = 'prompt_cache_hit_tokens' in wire ? wire.prompt_cache_hit_tokens : undefined
    const mapped = mapUsage(wire, 'deepseek-v4-flash', Date.parse('2026-08-31T00:30:00.000Z'))
    expect(mapped).toEqual({
      inputTokens: wire.prompt_tokens - (cacheRead ?? 0),
      outputTokens: wire.completion_tokens,
      ...hasTotal ? { totalTokens: wire.prompt_tokens + wire.completion_tokens } : {},
      ...cacheRead === undefined ? {} : { cacheReadTokens: cacheRead },
    })
    expect(mapped).not.toHaveProperty('estimatedCostUsd')
  })

  it('leaves an unknown exact model unpriced', () => {
    expect(mapUsage({ prompt_tokens: 10, completion_tokens: 2 }, 'private-pass-through'))
