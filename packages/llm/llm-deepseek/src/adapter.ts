...
export class DeepSeekAdapter extends LlmAdapter {
   ...
   */
  async releaseAllFiles(signal?: AbortSignal): Promise<number> {
    const connection = this.config.options()
    const apiKey = await this.config.resolveApiKey(connection)
    return this.files.releaseAll({ baseURL: connection.baseURL, apiKey }, signal)
  }
  ...
  private async * request(
    ...
    while (true) {
      ...
      // Freeze the billing bucket at send time so a stream crossing a peak-window
      // boundary never mixes rates within one provider request.
      const pricedAtEpochMs = Date.now()
      ...
      yield* translate(parseSse(response.body, onActivity), options.model, pricedAtEpochMs)
      return
    }
  }
}
