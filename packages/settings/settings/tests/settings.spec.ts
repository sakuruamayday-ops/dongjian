...
describe('registration', () => {
  ...
  it('keeps an explicitly owned registration when the short-lived caller fiber disposes', async () => {
    const { ctx } = await boot()
    let scope: SettingsScope<ThemeConfig> | undefined
    const caller = ctx.plugin({
      inject: ['settings'],
      apply: (child: Context) => {
        scope = child.settings.register('ui-theme', ThemeSchema, { owner: ctx })
      },
    })
    await caller
    await caller.dispose()

    expect(ctx.settings.describe().map(entry => entry.ns)).toContain('ui-theme')
    await scope!.update({ theme: 'light' })
    expect(scope!.get()).toMatchObject({ theme: 'light' })
