...
async function harness(
  ...
  bases: { baseUrl?: string; hostModuleBaseUrl?: string } = {},
): Promise<Context> {
  const ctx = new Context()
  ctx.baseUrl = bases.baseUrl ?? pathToFileURL(FIXTURES).href + '/'
  if (bases.hostModuleBaseUrl !== undefined) ctx.provide('hostModuleBaseUrl', bases.hostModuleBaseUrl)
...
describe('composing an agent from a preset', () => {
  it('uses the packaged-host base for both health and mount when the profile is external', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsh-preset-packaged-host-'))
    const profile = join(root, 'profile')
    const host = join(root, 'host')
    const presetRoot = join(root, 'presets')
    const presetDir = join(presetRoot, 'packaged')
    const packageDir = join(host, 'node_modules', '@fixture', 'host-only')
    await mkdir(profile, { recursive: true })
    await mkdir(presetDir, { recursive: true })
    await mkdir(packageDir, { recursive: true })
    await writeFile(join(packageDir, 'package.json'), JSON.stringify({
      name: '@fixture/host-only', type: 'module', exports: './index.mjs',
    }))
    await writeFile(join(packageDir, 'index.mjs'), 'export function apply() {}\n')
    await writeFile(
      join(presetDir, COMPOSITION_FILE),
      "- id: host-only\n  name: '@fixture/host-only'\n",
    )
    // The packaged desktop passes the host manifest as an absolute filename.
    // Both health discovery and the nested preset Loader require one file URL.
    const hostBase = join(host, 'package.json')
    const normalizedHostBase = pathToFileURL(hostBase).href
    const scoped = await harness(
      {
        default: 'packaged', roots: [{ path: presetRoot, trust: 'system' }],
        includeShippedRoot: false, includeUserRoot: false,
      },
      { baseUrl: pathToFileURL(profile).href + '/', hostModuleBaseUrl: hostBase },
    )
    const imported = vi.spyOn(scoped.loader.internal!, 'import')

    expect((await scoped.agentPresets.list())[0]?.broken).toBeUndefined()
    await expect(agentOn(scoped, 'sess-packaged-host')).resolves.toBeDefined()
    expect(imported).toHaveBeenCalledWith('@fixture/host-only', normalizedHostBase, {})
  })
...
describe('a roster with no base to resolve from', () => {
  it('refuses at load rather than calling every preset broken', async () => {
    ...
    })).rejects.toThrow(/needs `ctx\.hostModuleBaseUrl` or `ctx\.baseUrl`/)
  })
})
