...
describe('the harness-home preset root', () => {
  ...
  it('rejects an empty, duplicate, invalid, or default-excluding allowlist', async () => {
    await expect(roster({ allowedIds: [] })).rejects.toThrow(/at least the configured default/)
    await expect(roster({ allowedIds: ['standard', 'standard'] })).rejects.toThrow(/unique usable/)
    await expect(roster({ allowedIds: ['../standard'] })).rejects.toThrow(/unique usable/)
    await expect(roster({ allowedIds: ['minimal'] })).rejects.toThrow(/include the configured default/)
  })

  it('limits discovery and resolution to the deployment allowlist', async () => {
    await seedHomePreset('mine')
    const ctx = await roster({ allowedIds: ['standard'] })

    expect((await ctx.agentPresets.list()).map(preset => preset.id)).toEqual(['standard'])
    await expect(ctx.agentPresets.resolve('minimal')).rejects.toThrow(/not found/)
