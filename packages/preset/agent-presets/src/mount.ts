...
export async function mountPreset(agentCtx: Context, preset: AgentPreset): Promise<void> {
  ...
  // Captured before the subtree exists. In packaged apps the mutable profile
  // base lives outside the archive; the boot-published host base is the only
  // anchor that can resolve the plugins bundled with the application.
  const moduleBase = presetModuleBase(agentCtx)
  /* v8 ignore next -- roster resolution already rejects a deployment with no module base */
  if (moduleBase !== undefined) harnessBase.set(config, moduleBase)
