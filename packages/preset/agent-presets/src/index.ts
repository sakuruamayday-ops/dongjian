...
import { PRESET_ID, type AgentPreset, type Config, type PresetRoot } from './preset.ts'
import { agentPresetProjectionDefinition } from './session.ts'
import { presetModuleBase } from './module-base.ts'
...
export class AgentPresets extends TypertRemoteService {
  ...
  /** Deployment-owned exact roster, absent for the ordinary extensible DSH app. */
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    if (config.allowedIds !== undefined) {
      if (config.allowedIds.length === 0) {
        throw new Error('agent-presets: allowedIds must contain at least the configured default')
      }
      const allowed = new Set(config.allowedIds)
      if (allowed.size !== config.allowedIds.length
        || config.allowedIds.some(id => !PRESET_ID.test(id))) {
        throw new Error('agent-presets: allowedIds must contain unique usable preset ids')
      }
      if (!allowed.has(config.default)) {
        throw new Error('agent-presets: allowedIds must include the configured default')
      }
      this.allowedIds = allowed
    }
    const moduleBase = presetModuleBase(ctx)
    if (moduleBase === undefined) {
      ...
      throw new Error(
        'agent-presets: the roster needs `ctx.hostModuleBaseUrl` or `ctx.baseUrl` to resolve the plugins '
        + 'a composition names; compose it under a Loader, or set a module base on the context this plugin is applied to',
      )
    }
    this.harnessBase = moduleBase
    this.resolvedRoots = [
  ...
  get defaultId(): string {
    ...
    if (selected === undefined || (this.allowedIds !== undefined && !this.allowedIds.has(selected))) {
  ...
  async list(): Promise<AgentPreset[]> {
    ...
    if (this.allowedIds === undefined) return presets
    return presets.filter(preset => this.allowedIds?.has(preset.id))
  }
