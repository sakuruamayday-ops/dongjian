...
import { createTransport, resolveCredentialConfig } from '@deepseek-ai/dsh-mcp-client/src/transport.ts'
...
describe('syncTools', () => {
  ...
  it('maps keyword to a string searchKey even when the MCP schema omits required metadata', async () => {
    const client = createMockClient([{
    ...
    await syncTools(client as never, ctx, { ...defaultOpts, serverName: 'qcc' }, new Map())

    await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('qcc-alias'),
      name: 'mcp__qcc__enterprise_search',
      arguments: { keyword: '共创测试企业' },
    })

    expect(client.callTool).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'enterprise_search',
        arguments: { searchKey: '共创测试企业' },
      }),
      undefined,
      expect.anything(),
    )
  ...
  it('does not override an explicit searchKey or rewrite unrelated schemas', async () => {
    const client = createMockClient([
      {
        name: 'enterprise_search',
        inputSchema: {
    ...
    await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('explicit-search-key'),
      name: 'mcp__srv__enterprise_search',
      arguments: { searchKey: '明确值', keyword: '不得覆盖' },
    })
    await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('generic-keyword'),
      name: 'mcp__srv__generic_search',
      arguments: { keyword: '原样保留' },
    })

    expect(client.callTool).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ arguments: { searchKey: '明确值', keyword: '不得覆盖' } }),
      undefined,
      expect.anything(),
    )
    expect(client.callTool).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ arguments: { keyword: '原样保留' } }),
      undefined,
      expect.anything(),
    )
  })

  it('maps a singular region to the string-array regions field advertised by project directory tools', async () => {
    const client = createMockClient([{
      name: 'project_directory_match',
      inputSchema: {
        type: 'object',
        properties: {
          query: { type: 'string' },
    ...
    await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('region-alias'),
      name: 'mcp__srv__project_directory_match',
      arguments: { query: '专精特新', region: '杭州' },
    })

    expect(client.callTool).toHaveBeenCalledWith(
      {
        name: 'project_directory_match',
        arguments: { query: '专精特新', regions: ['杭州'] },
      },
      undefined,
      expect.anything(),
    )
  ...
  it('keeps explicit regions and does not rewrite a singular region for unrelated schemas', async () => {
    const client = createMockClient([{
      name: 'project_directory_match',
      inputSchema: {
        type: 'object',
    ...
    await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('explicit-regions'),
      name: 'mcp__srv__project_directory_match',
      arguments: { regions: ['杭州'], region: '宁波' },
    })
    await ctx.tools.execute({
      signal: testToolSignal,
      callId: ToolCallId('generic-region'),
      name: 'mcp__srv__generic_region',
      arguments: { region: '绍兴' },
    })

    expect(client.callTool).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ arguments: { regions: ['杭州'], region: '宁波' } }),
      undefined,
      expect.anything(),
    )
    expect(client.callTool).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ arguments: { region: '绍兴' } }),
      undefined,
      expect.anything(),
    )
  })
...
describe('resolveCredentialConfig', () => {
  it('resolves HTTP and stdio credential references without mutating the input config', async () => {
    const ctx = new Context()
    ctx.provide('credentials', {
      resolve: vi.fn(async (ref: string) => ({ value: `secret-${ref}`, source: 'test' })),
    } as never)
    const http: Config = {
      transport: 'streamable-http', serverName: 'qcc', url: 'https://example.test/mcp',
      headers: {}, headerCredentials: { Authorization: { ref: 'QCC_API_KEY', prefix: 'Bearer ' } },
      toolCallTimeoutMs: 1_000, failOnStartupError: true,
    }
    const stdio: Config = {
      transport: 'stdio', serverName: 'paddle', command: 'paddleocr_mcp', args: [], env: {},
      envCredentials: { PADDLE_TOKEN: 'PADDLE_API_KEY' }, cwd: '',
      toolCallTimeoutMs: 1_000, failOnStartupError: true,
    }
    await expect(resolveCredentialConfig(ctx, http)).resolves.toMatchObject({
      headers: { Authorization: 'Bearer secret-QCC_API_KEY' }, headerCredentials: {},
    })
    await expect(resolveCredentialConfig(ctx, stdio)).resolves.toMatchObject({
      env: { PADDLE_TOKEN: 'secret-PADDLE_API_KEY' }, envCredentials: {},
    })
    expect(http.headers).toEqual({})
    expect(stdio.env).toEqual({})
  })

  it('fails closed for absent providers, missing credentials, and literal-binding collisions', async () => {
    const bare = new Context()
    const configured: Config = {
      transport: 'streamable-http', serverName: 'qcc', url: 'https://example.test/mcp', headers: {},
      headerCredentials: { Authorization: { ref: 'QCC_API_KEY' } },
      toolCallTimeoutMs: 1_000, failOnStartupError: true,
    }
    await expect(resolveCredentialConfig(bare, configured)).rejects.toThrow(/provider is absent/)

    const missing = new Context()
    missing.provide('credentials', { resolve: vi.fn(async () => undefined) } as never)
    await expect(resolveCredentialConfig(missing, configured)).rejects.toThrow(/QCC_API_KEY is not configured/)

    const available = new Context()
    available.provide('credentials', { resolve: vi.fn(async () => ({ value: 'x', source: 'test' })) } as never)
    await expect(resolveCredentialConfig(available, {
      ...configured,
      headers: { Authorization: 'literal' },
    })).rejects.toThrow(/both a literal and a credential binding/)
  })
})
