...
describe('reconnect supervisor', () => {
  ...
  it('reconnects after a transport close, re-syncs tools through the new generation, and serves calls', async () => {
    ...
    ctx.on('mcp/connection-state', (state) => { states.push(state) })
    ...
    expect(states.map(state => state.phase)).toEqual(['connecting', 'ready'])
    ...
    expect(states.at(-1)?.message).toContain('自动重连')

    await vi.waitFor(() => { expect(ctx.tools.get('mcp__srv__revived')).toBeDefined() })
    expect(ctx.tools.get('mcp__srv__remote')).toBeUndefined()
    expect(instances).toHaveLength(2)
    expect(mockConnect).toHaveBeenCalledTimes(2)
    expect(states.at(-1)).toMatchObject({ phase: 'ready', message: '连接已恢复并重新发现工具' })
  ...
  it('stops at the failure cap, unregisters the tools, and reports final failure', async () => {
    ...
    expect(states.at(-1)?.message).toContain('请检查网络或凭据后手动刷新')
    await sleep(30)
    expect(mockConnect).toHaveBeenCalledTimes(3)
  })
