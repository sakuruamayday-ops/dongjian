...
describe('mcp-client plugin module exports', () => {
  ...
  it('Config schema accepts a bounded readiness probe', () => {
    const resolved = ConfigSchema({
      transport: 'stdio',
      serverName: 'srv',
      command: 'echo',
      readinessProbe: { toolName: 'health', arguments: { input: 'fixture' } },
    } as never)
    expect(resolved.readinessProbe).toEqual({
      toolName: 'health', arguments: { input: 'fixture' }, timeoutMs: 60_000,
    })
  })
...
describe('apply (plugin lifecycle)', () => {
  ...
  it('requires a configured raw-tool probe before discovering and registering tools', async () => {
    const order: string[] = []
    ...
    await apply(ctx, {
      ...stdioConfig,
      readinessProbe: { toolName: 'health', arguments: { fixture: true }, timeoutMs: 1_000 },
    })

    expect(order).toEqual(['probe', 'list'])
    expect(mockCallTool).toHaveBeenCalledWith({ name: 'health', arguments: { fixture: true } }, undefined, expect.any(Object))
    expect(ctx.tools.get('mcp__srv__remote')).toBeDefined()
  })

  it('rejects a failed readiness probe before any tool becomes visible', async () => {
    ...
    await expect(apply(ctx, {
      ...
      readinessProbe: { toolName: 'health', arguments: {}, timeoutMs: 1_000 },
