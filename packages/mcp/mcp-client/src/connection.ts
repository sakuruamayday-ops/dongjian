...
import { createTransport, resolveCredentialConfig } from './transport.ts'
import { runReadinessProbe, syncTools } from './tools.ts'
...
/** User-visible lifecycle for one supervised MCP transport generation. */
export type McpConnectionPhase = 'connecting' | 'ready' | 'reconnecting' | 'failed' | 'disposed'

/** Credential-free connection health emitted to product-level aggregators. */
...
export function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy): ConnectionHandle {
  ...
  const publishState = (
    ...
    ctx.emit('mcp/connection-state', Object.freeze({
      serverName: config.serverName,
      phase,
      message,
      attempt,
  ...
  function scheduleReconnect(): void {
    ...
    if (failedAttempts > policy.maxAttempts) {
      ...
      publishState('failed', `连接已中断，连续 ${String(policy.maxAttempts)} 次自动重连失败；请检查网络或凭据后手动刷新`, policy.maxAttempts)
      return
    }
    ...
    publishState('reconnecting', `连接已中断，${String(delayMs)} 毫秒后自动重连（第 ${String(failedAttempts)}/${String(policy.maxAttempts)} 次）`, failedAttempts)
  ...
  async function connectGeneration(startup: boolean): Promise<void> {
    if (startup) publishState('connecting', '正在连接并发现工具')
    ...
    try {
      await generation.connect(createTransport(await resolveCredentialConfig(ctx, config)))
      if (hasClosed()) {
        attemptSettled = true
        generationDown(generation)
        return
      }
      if (config.readinessProbe !== undefined) {
        await runReadinessProbe(
          generation,
          config.readinessProbe.toolName,
          config.readinessProbe.arguments,
          config.readinessProbe.timeoutMs,
        )
      }
      ...
      if (!quiesced) {
        ...
        publishState('failed', '连接异常且旧连接未能安全关闭；为避免重复进程已停止重连，请手动刷新')
        return
      }
      generationDown(generation)
      return
    ...
    publishState('ready', failedAttempts > 0 ? '连接已恢复并重新发现工具' : '连接已就绪')
  }
