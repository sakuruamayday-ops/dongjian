...
/**
 * Execute a fixed product-owned probe before exposing a connection as ready.
 * The raw name and arguments come only from trusted host configuration; no
 * user or model input participates in this call.
 * @param client - The client value.
 * @param rawName - The raw name value.
...
export async function runReadinessProbe(
  ...
  const result = await callToolUncached(client, rawName, args, AbortSignal.timeout(timeoutMs), {
    registrationFailure: 'throw',
    serverName: 'readiness-probe',
    toolCallTimeoutMs: timeoutMs,
  })
  if (result.isError !== true) return
  const content = Array.isArray(result.content) ? result.content as unknown as JsonValue[] : []
  throw new Error(extractText(content, rawName) || `MCP readiness probe failed: ${rawName}`)
}
...
function createExecutor(
  ...
  return async (args: unknown, exec: ToolExecution) => {
    ...
    const argsObj = normalizeCompatibleArguments(
      (typeof args === 'object' && args !== null ? args : {}) as Record<string, unknown>,
      inputSchema,
    )
    const result = await callToolUncached(client, rawName, argsObj, exec.signal, opts)
...
/**
 * Normalize a small set of compatibility aliases only when the remote tool's
 * own JSON schema proves the expected field and shape. Several Chinese data
 * services expose `searchKey` while models commonly emit `keyword`; project
 * directory services expose `regions: string[]` while models often emit a
 * singular `region`. Keeping each decision schema-driven avoids rewriting
 * arguments for unrelated third-party tools and never overrides an explicit
 * canonical field supplied by the model.
 */
function normalizeCompatibleArguments(
  args: Record<string, unknown>,
  inputSchema: unknown,
): Record<string, unknown> {
  if (typeof inputSchema !== 'object' || inputSchema === null) return args
  const schema = inputSchema as {
    properties?: unknown
    required?: unknown
  }
  if (typeof schema.properties !== 'object' || schema.properties === null) return args
  const properties = schema.properties as Record<string, unknown>
  let normalized: Record<string, unknown> = args

  const searchKeySchema = properties.searchKey
  const expectsStringSearchKey = typeof searchKeySchema === 'object'
    && searchKeySchema !== null
    && (searchKeySchema as { type?: unknown }).type === 'string'
  if (expectsStringSearchKey && typeof args.searchKey !== 'string'
    && typeof args.keyword === 'string' && args.keyword.trim().length > 0) {
    normalized = { ...normalized, searchKey: args.keyword }
    if (!Object.hasOwn(properties, 'keyword')) delete normalized.keyword
  }

  const regionsSchema = properties.regions
  const expectsStringRegions = typeof regionsSchema === 'object'
    && regionsSchema !== null
    && (regionsSchema as { type?: unknown }).type === 'array'
    && typeof (regionsSchema as { items?: unknown }).items === 'object'
    && (regionsSchema as { items?: { type?: unknown } }).items?.type === 'string'
  if (expectsStringRegions && !Array.isArray(args.regions)
    && typeof args.region === 'string' && args.region.trim().length > 0) {
    normalized = normalized === args ? { ...args } : normalized
    normalized.regions = [args.region]
    if (!Object.hasOwn(properties, 'region')) delete normalized.region
  }
