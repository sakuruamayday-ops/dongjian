...
const ReadinessProbe: z<ReadinessProbeConfig> = z.object({
  toolName: z.string().required(),
  arguments: z.dict(z.any()).default({}),
  timeoutMs: z.number().min(1).max(MAX_TIMER_DELAY_MS).default(DEFAULT_TOOL_CALL_TIMEOUT_MS),
}).default(undefined as unknown as ReadinessProbeConfig)
...
export const Config = z.union([
  z.object({
    ...
    envCredentials: z.dict(z.string().pattern(/^[A-Za-z_][A-Za-z0-9_]*$/)).default({}),
  ...
  z.object({
    ...
    headerCredentials: z.dict(z.object({
      ref: z.string().pattern(/^[A-Za-z_][A-Za-z0-9_]*$/).required(),
