...
export type IndexInjection =
  ...
  /** External classic script executed after every ordinary injection row has taken effect. */
  | { kind: 'boot-ready'; src: string }
...
function renderRow(row: IndexInjection): { placement: IndexInjectionPlacement; markup: string } {
  switch (row.kind) {
    ...
    case 'boot-ready':
      return { placement: 'body', markup: `<script src="${escapeHtmlAttribute(row.src)}"></script>` }
    case 'style':
 ...
 * group in table order. Boot-ready rows are external scripts appended after
 * every ordinary body row, regardless of subscriber registration order.
...
export function renderIndexInjections(html: string, rows: readonly IndexInjection[]): string {
  ...
  for (const row of rows) {
    ...
    if (row.kind === 'boot-ready') ready += rendered.markup
    else if (rendered.placement === 'head') head += rendered.markup
