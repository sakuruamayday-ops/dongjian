  ...
  it('collects injection rows fresh per render and layers taps over the rendered rows', { timeout: 60_000 }, async () => {
    ...
    loaded.on('webserver/index-inject', (table) => {
      table.push(
        ...
        { kind: 'boot-ready', src: '/plugins/bootstrap-ready.js?rev="3"&x=<ready>' },
      )
    })
    ...
    const order = [
      ...
      '<script src="/plugins/bootstrap-ready.js?rev=&quot;3&quot;&amp;x=&lt;ready&gt;"></script>',
      'shell',
    ...
    // Tag-less fragments: head rows prepend, body rows append, and readiness
    // stays last even when its row was collected before another body row.
    expect(renderIndexInjections('<main>x</main>', [
      { kind: 'script', placement: 'head', text: 'H' },
      { kind: 'boot-ready', src: '/ready.js' },
      { kind: 'script', placement: 'body', text: 'B' },
    ])).toBe('<script>H</script><main>x</main><script>B</script><script src="/ready.js"></script>')
  })
