...
let posixFlock: Promise<Flock> | undefined

/** Load the POSIX-only addon after the Win32 semaphore branch has returned. */
function loadPosixFlock(): Promise<Flock> {
  // Keep fs-ext out of the Windows package. A top-level import makes the
  // Windows build compile and load an unused POSIX addon before this module
  // can select its native Win32 semaphore implementation.
  return posixFlock ??= import('fs-ext').then(module => module.flock)
}
...
async function flockAsync(fd: number, flags: 'exnb' | 'un'): Promise<void> {
  const flock = await loadPosixFlock()
