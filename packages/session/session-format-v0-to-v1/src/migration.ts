...
function normalizeReleasedV0Events(
  ...
  for (const event of events) {
    ...
    const message = normalizeLegacyReplayEvent(normalizeLegacyMessage(steering, sessionId, messageIds))
...
function normalizeLegacyReplay(value: SessionFormatJsonValue): SessionFormatJsonValue {
  const replay = releasedV0Record(value, 'legacy replayState')
  if (replay['kind'] !== 'pi-ai') return value
  assertReleasedV0Keys(replay, ['kind', 'version', 'api', 'provider', 'model', 'stopReason', 'blocks'],
    ['responseId', 'responseModel'], 'legacy pi-ai replayState')
  if (replay['version'] !== 1 || !Array.isArray(replay['blocks'])) {
    throw new SessionFormatError('unsupported legacy pi-ai replayState version or blocks')
  }
  for (const key of ['api', 'provider', 'model', 'stopReason', 'responseId', 'responseModel']) {
    if (replay[key] !== undefined && typeof replay[key] !== 'string') {
      throw new SessionFormatError(`legacy pi-ai replayState ${key} must be a string`)
    }
  }
  // Keep provider-native blocks and response identity for subsequent model requests.
  const { blocks, ...response } = replay
  return { response: { ...response, version: 2 }, blocks: blocks as SessionFormatJsonValue }
}

function normalizeLegacyReplayEvent(event: SessionFormatEvent): SessionFormatEvent {
  const data = releasedV0Record(event.data, `${event.type} ${event.seq} data`)
  if (event.type === 'assistant/chunk') {
    const chunk = releasedV0Record(data['chunk'], 'assistant chunk')
    if (chunk['replayState'] === undefined) return event
    return { ...event, data: { ...data, chunk: { ...chunk, replayState: normalizeLegacyReplay(chunk['replayState']) } } }
  }
  if (event.type === 'assistant/message') {
    const message = releasedV0Record(data['message'], 'assistant message')
    const source = releasedV0Record(message['source'], 'assistant message source')
    if (source['replayState'] === undefined) return event
    return { ...event, data: { ...data, message: { ...message,
      source: { ...source, replayState: normalizeLegacyReplay(source['replayState']) },
    } } }
  }
  return event
}
