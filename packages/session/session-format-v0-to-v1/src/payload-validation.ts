...
export function assertReleasedPayloadSemantics(event: SessionFormatEvent, version: number): void {
  ...
  switch (event.type) {
    ...
    case 'tool/code-dispatch-start':
      if (data['turn'] !== undefined) countValue(data['turn'], `${label} turn`)
      if (data['step'] !== undefined) countValue(data['step'], `${label} step`)
...
function tokenUsageValue(value: SessionFormatJsonValue | undefined, label: string): void {
  const usage = exactRecord(
    ...
    ['totalTokens', 'cacheReadTokens', 'cacheWriteTokens', 'reasoningTokens', 'estimatedCostUsd'],
  )
  for (const key of Object.keys(usage)) {
    // Currency is fractional; token counts remain non-negative integers.
    if (key === 'estimatedCostUsd') {
      const cost = usage[key]
      if (typeof cost !== 'number' || !Number.isFinite(cost) || cost < 0) {
        throw new SessionFormatError(`${label} estimatedCostUsd must be a finite non-negative number`)
      }
    } else countValue(usage[key], `${label} ${key}`)
  }
}
...
function messageSourceValue(
  ...
  switch (source['kind']) {
    case 'user':
      assertReleasedV0Keys(source, ['kind'], ['rpcId', 'clientTimeZone', 'displayText'], label)
      ...
      if (source['displayText'] !== undefined) stringValue(source['displayText'], `${label} displayText`)
      return
    case 'plugin':
...
function pluginSourceValue(source: JsonRecord, label: string): void {
  ...
  if (source['plugin'] === 'gongchuang-policy-gate') optional.push('delivery')
  ...
  if (source['delivery'] !== undefined) {
    const delivery = exactRecord(source['delivery'], `${label} delivery`, ['turn', 'phase', 'files', 'issues'], ['draftText'])
    countValue(delivery['turn'], `${label} delivery turn`)
    literalValue(delivery['phase'], ['running', 'paused', 'waiting-user', 'draft', 'failed', 'formal'], `${label} delivery phase`)
    arrayValue(delivery['files'], `${label} delivery files`, (value, fileLabel) => {
      const file = exactRecord(value, fileLabel, ['originalPath', 'path'])
      nonEmptyString(file['originalPath'], `${fileLabel} originalPath`)
      nonEmptyString(file['path'], `${fileLabel} path`)
    })
    arrayValue(delivery['issues'], `${label} delivery issues`, stringValue)
    if (delivery['draftText'] !== undefined) stringValue(delivery['draftText'], `${label} delivery draftText`)
  }
