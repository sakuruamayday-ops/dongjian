...
The product extension admits typed user `displayText`, non-negative fractional `estimatedCostUsd`, dispatch turn/step positions, and typed delivery notices owned by `gongchuang-policy-gate`. Legacy pi-ai replay records are normalized into response metadata plus blocks before strict validation. Unknown members still fail; original artifacts are never overwritten.
