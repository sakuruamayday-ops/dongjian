...
旧版 `tool/code-dispatch-start` 和 `tool/code-dispatch` 记录如果同时缺少轮次与步骤，可以从当前步骤中唯一的前置根 `tool/call` 恢复这些坐标。根 id 缺失或重复时不会猜测，显式坐标不会被替换，原始存储字节保持不变。[工作区文件链接说明](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.zh.md)记录了受影响的交付消费者与冷浏览器验收。
