...
Legacy `tool/code-dispatch-start` and `tool/code-dispatch` records that omitted both turn and step can recover those coordinates from a unique preceding root `tool/call` in the current step. Missing or duplicated root ids are not guessed, explicit coordinates are not replaced, and the original stored bytes stay unchanged. The [workspace file links note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md) records the affected delivery consumer and cold-browser acceptance.
