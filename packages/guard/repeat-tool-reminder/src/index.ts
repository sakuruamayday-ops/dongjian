/**
 * Per-agent exact-repeat detector. It can enrich post-execute decisions with
 * logged model context and, when configured, refuse an identical attempt
 * before dispatch. Configuration and chain semantics live in the package
...
export const Config: z<Config> = z.object({
  ...
  blockAt: z.number().default(undefined as unknown as number),
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (blockAt !== undefined && (!Number.isInteger(blockAt) || blockAt < 2)) {
    throw new Error(`repeat-tool-reminder: invalid blockAt ${blockAt} — must be an integer >= 2`)
  }
  ...
  function executionKey(exec: ToolExecution): string {
    return JSON.stringify([exec.name, canonicalize(exec.arguments)])
  }

  function shouldBlock(exec: ToolExecution): boolean {
    if (blockAt === undefined || !exec.agent || !tracked(exec.name)) return false
    const chain = chains.get(exec.agent)
    return chain !== undefined && chain.key === executionKey(exec) && chain.count + 1 >= blockAt
  }
  ...
  // Match OpenCode's doom-loop boundary: the configured repeated attempt is
  // refused before the tool runs. Downstream policy decisions retain priority;
  // changing the tool or its arguments remains available as the recovery path.
  ctx.on('tools/pre-execute', async (exec, next): Promise<PreToolDecision> => {
    const downstream = await next()
    if (downstream.kind !== 'allow' || !shouldBlock(exec)) return downstream
    return {
      kind: 'deny',
      reason: `检测到工具 ${exec.name} 以完全相同参数连续调用，本次已阻止。请复用上一次结果、更换参数或结束当前步骤。`,
    }
  })

  // Count first (state advances regardless of the downstream outcome), then
  // delegate and fold the reminder onto the downstream decision.
