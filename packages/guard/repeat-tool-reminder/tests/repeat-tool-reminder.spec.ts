...
async function harness(config: Config = {}, onProbe?: () => void): Promise<Context> {
  ...
  ctx.tools.register(defineContentToolFixture({
    ...
    async execute() { onProbe?.(); return [{ type: 'text', text: 'ok' }] },
  }))
...
describe('threshold escalation', () => {
  it('blocks the configured identical attempt before dispatch while allowing the first two', async () => {
    let executions = 0
    const ctx = await harness({ thresholds: [2, 3], blockAt: 3 }, () => { executions += 1 })
    ...
    const results = agent.session.snapshotEvents()
      .filter((e): e is SessionEvent<'tool/result'> => e.type === 'tool/result')
    expect(results).toHaveLength(4)
    expect(results.slice(0, 2).every(result => result.data.message.content[0].isError === false)).toBe(true)
    expect(results.slice(2).every(result => result.data.message.content[0].isError === true)).toBe(true)
    const blockedContent = results[2]!.data.message.content[0].content
    expect(blockedContent.some(block => block.type === 'text' && block.text.includes('本次已阻止'))).toBe(true)
  })
...
describe('config validation fails loud', () => {
  ...
  it('rejects a blockAt threshold below two or with a fractional value', async () => {
    const ctx = await spine()
    await expect(ctx.plugin(RepeatToolGuard, { blockAt: 1 })).rejects.toThrow(/blockAt/)
    const ctx2 = await spine()
    await expect(ctx2.plugin(RepeatToolGuard, { blockAt: 2.5 })).rejects.toThrow(/blockAt/)
  })
})
