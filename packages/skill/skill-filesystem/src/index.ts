...
export const Config: Schema<Config> = z.object({
  ...
  disabledSkillRegistryPath: z.string(),
})
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private readonly disabledSkillRegistryPath: string | undefined
  ...
  constructor(
    ...
    this.disabledSkillRegistryPath = config.disabledSkillRegistryPath === undefined
      ? undefined
      : resolve(config.disabledSkillRegistryPath)
  }
  ...
  async list(options: SkillLookupOptions): Promise<SkillCandidate[] | SkillProviderObservation> {
    ...
    const disabledSkillNames = await this.readDisabledSkillNames()
    let complete = true
    try {
    ...
    for (const root of roots) {
      for (const skill of await discoverRoot(root, this.ctx, this.name)) {
        if (root.source === 'bundled' && disabledSkillNames.has(skill.name)) continue
  ...
  async get(candidate: SkillCandidate, options: SkillLookupOptions): Promise<SkillDefinition | undefined> {
    if (candidate.source === 'bundled' && (await this.readDisabledSkillNames()).has(candidate.name)) return undefined
  ...
  private async readDisabledSkillNames(): Promise<ReadonlySet<string>> {
    if (this.disabledSkillRegistryPath === undefined) return new Set()
    try {
      const value = JSON.parse(await readFile(this.disabledSkillRegistryPath, 'utf8')) as { disabledSkillNames?: unknown }
      if (!Array.isArray(value.disabledSkillNames)) return new Set()
      return new Set(value.disabledSkillNames.filter((name): name is string => typeof name === 'string'))
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') return new Set()
      this.ctx.logger.warn(`skill-filesystem: failed to read disabled skill registry: ${errorMessage(error)}`)
      return new Set()
    }
  }
