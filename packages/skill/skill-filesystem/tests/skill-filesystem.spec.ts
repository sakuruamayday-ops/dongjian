...
import SkillRegistry, { type SkillProviderControl } from '@deepseek-ai/dsh-skill'
...
describe('FileSystemSkillProvider', () => {
  ...
  it('hides disabled bundled skills from listing and loading without changing their files', async () => {
    const home = await tempDir('skill-disabled-bundled-home')
    const bundled = await tempDir('skill-disabled-bundled-root')
    const registry = join(home, 'marketplace', 'registry.json')
    await writeSkill(bundled, 'bundled-enabled', 'Enabled bundled skill')
    await writeSkill(bundled, 'bundled-disabled', 'Disabled bundled skill')
    ...
    const ctx = await setupLocal(home, {
      ...
      disabledSkillRegistryPath: registry,
    })
    let invalidateCatalog = (): void => {}
    ctx.skills.registerProvider((control: SkillProviderControl) => {
      invalidateCatalog = control.invalidate
      return {
        name: 'disabled-registry-test-control',
        list: async () => [],
        get: async () => undefined,
      }
    })
    const cwd = '/bundled-toggle-check'
    expect((await ctx.skills.list({ cwd })).map(skill => skill.name)).toEqual([
      'bundled-disabled', 'bundled-enabled',
    ])
    ...
    await writeFile(registry, JSON.stringify({
      ...
      installed: [],
      repositories: [],
      disabledSkillNames: ['bundled-disabled'],
    }))
    invalidateCatalog()
    expect((await ctx.skills.list({ cwd })).map(skill => skill.name)).toEqual(['bundled-enabled'])
    await expect(ctx.skills.get('bundled-disabled', { cwd })).resolves.toBeUndefined()
    await expect(readFile(join(bundled, 'bundled-disabled', 'SKILL.md'), 'utf8'))
      .resolves.toContain('Disabled bundled skill')
    ...
    await expect(ctx.skills.get('bundled-disabled', { cwd }))
      .resolves.toMatchObject({ content: 'Use the skill.' })
  })
