...
\`run_code\` requires \`code\` — the body of an async TypeScript function (erasable syntax only — no \`enum\` or namespaces; type annotations are advisory, the code runs type-stripped). It is not a Node.js module: \`require\`, \`process\`, \`fs\`, and module imports are unavailable. Use the declared \`tools\` bindings for files, commands, skills, and other side effects. Include an optional \`description\` summarizing the program; when absent, its display label is "Run code". The declarations below are SDK bindings for this program. A declaration does not make its name a directly callable tool; only names supplied as separate tool schemas may be called directly.`

const SDK_PROGRAM_INSTRUCTIONS = `Inside the program:

- Call tools as \`await tools.name(args)\` — quoted access for exotic names: \`tools["my-tool"](args)\`. Every call resolves to the tool's typed canonical JSON value. Tool arguments must be lossless JSON.
- When an exact file path is already supplied, call the narrow file tool with that path. Do not rediscover it by listing or globbing its parent directory.
