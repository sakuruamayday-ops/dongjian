...
interface OpenExecutionLocation {
  readonly turn: number
  readonly step: number
}

/**
 * Capture the outer call's open Session boundary once before sub-dispatches
 * start. Session exposes immutable events rather than its invariant trace, so
 * folding that snapshot is the public, replay-stable source of these facts.
 */
function openExecutionLocation(agent: ToolRunContext['agent']): OpenExecutionLocation | undefined {
  // Use the public Session API so an upstream API removal cannot silently
  // erase the location of successful nested tool calls.
  ...
  for (const event of events) {
    ...
    if (event.type === 'step/end' && event.data.turn === turn && event.data.step === step) {
      step = undefined
      continue
    }
...
const TYPESCRIPT_FLAVOR: RunCodeFlavor = {
  description:
    'Execute a TypeScript program against the available tools. Requires `code`, '
    + 'the BODY of an async function (erasable syntax only; top-level '
    + '`await` and `return` work), with optional `description`, a short summary of what the program '
...
const PYTHON_FLAVOR: RunCodeFlavor = {
  description:
    'Execute a Python program against the available tools. Requires `code`, '
    + 'the BODY of an async function (top-level `await` and `return` '
    + 'work), with optional `description`, a short summary of what the program does. Call tools as '
 ...
 * display-only `description`, executed through the dispatch bridge described
 * above. The
...
export function createRunCodeTool(registry: ToolRuntime, options: RunCodeBridgeOptions): ToolDefinition {
  ...
  const definition = defineTool({
    ...
    async execute(args, exec): Promise<RunCodeOutput> {
      // A missing display label must not turn valid code into a retry loop.
      // Code and every nested tool argument still pass their normal validation.
      const runtime = requireRuntime()
      // Nested durable events need the outer location so turn-scoped consumers
      // can use structured results without parsing assistant prose.
      const executionLocation = openExecutionLocation(exec.agent)
