...
export interface PtcDispatchStartEventData {
  ...
  /**
   * Owning conversation location captured from the open Session boundary.
   * Optional only for replay compatibility and agent-less embeddings; every
   * newly emitted dispatch from a Session-backed Agent carries both fields.
   */
  turn?: number
  /** See {@link turn}. */
  step?: number
