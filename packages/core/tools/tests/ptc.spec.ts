...
/** Real Session storage with an append observer for the owning Agent fixture. */
function fakeAgent(): { agent: Agent; events: { type: string; data: unknown }[] } {
  const events: { type: string; data: unknown }[] = []
  const session = Session.create(SessionId('ptc-agent'))
...
describe('the sub-dispatch scheduler (native concurrency contract)', () => {
  ...
  it('records the real open turn and step on nested dispatch events', async () => {
    const { ctx, runtime } = await setup({ mode: 'ptc' })
    registerEcho(ctx)
    const session = Session.create(SessionId('located-ptc-dispatch'))
    session.append('turn/start', { turn: 7 })
    session.append('step/start', { turn: 7, step: 3 })
    const agent = { session } as Agent
    runtime.behavior = async request => ({
      logs: [],
      value: await request.bindings[0]!.functions.echo!({ value: 'located' }),
    })

    const result = await runCode(ctx, 'return tools.echo(...)', { agent })

    expect(result.isError).toBe(false)
    const events = session.snapshotEvents().filter(event =>
      event.type === 'tool/code-dispatch-start' || event.type === 'tool/code-dispatch')
    expect(events).toHaveLength(2)
    expect(events.map(event => ({ turn: event.data.turn, step: event.data.step }))).toEqual([
      { turn: 7, step: 3 },
      { turn: 7, step: 3 },
    ])
  })
...
describe('the run_code dispatch bridge', () => {
  ...
  it.each([undefined, '', '   '])('executes valid code with the default label when description is %j', async (description) => {
    const { ctx, runtime, systemPrompt } = await setup({ mode: 'ptc' })
    const args = { code: 'return 1', ...description === undefined ? {} : { description } }
    const result = await ctx.tools.execute({
      signal: testToolSignal, callId: ToolCallId('missing-label'), name: RUN_CODE_NAME, arguments: args,
    })
    expect(result.isError).toBe(false)
    expect(runtime.lastRequest?.program).toBe('return 1')
    expect(ctx.tools.get(RUN_CODE_NAME)?.presentCall?.(args)).toMatchObject({ title: 'Run code', rawInput: 'return 1' })
    const assembly = await systemPrompt.assemble()
    expect(assembly.tools.find(tool => tool.name === RUN_CODE_NAME)?.parameters).toMatchObject({ required: ['code'] })
  })

  it.each([{}, { code: 42 }, { code: 'return 1', description: 42 }])('still rejects invalid executable arguments %j', async (args) => {
    const { ctx, runtime } = await setup({ mode: 'ptc' })
    const result = await ctx.tools.execute({
      signal: testToolSignal, callId: ToolCallId('invalid-code'), name: RUN_CODE_NAME, arguments: args,
    })
    expect(result.isError).toBe(true)
