...
describe('renderToolsSdk', () => {
  ...
  it('declares every tool in lexicographic order with quoted keys for exotic names', () => {
    ...
    expect(text).toContain('It is not a Node.js module')
    expect(text).toContain('`require`, `process`, `fs`, and module imports are unavailable')
    ...
    expect(text).toContain('Do not rediscover it by listing or globbing its parent directory')
  })

  it('distinguishes executable code from its optional display label', () => {
    const text = renderToolsSdk([bash])
    expect(text).toContain('`code`')
    expect(text).toContain('`description`')
    expect(text).toContain('requires `code`')
