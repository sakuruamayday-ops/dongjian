...
/** Preserve scenario-owned timeout diagnostics when Vitest misses its first poll under load. */
async function waitForHarnessCondition(
  assertion: () => void | Promise<void>,
  timeoutMs: number,
  timeoutMessage: string,
): Promise<void> {
  try {
    await vi.waitFor(assertion, { interval: WAIT_POLL_INTERVAL_MS, timeout: timeoutMs })
  }
  catch (error) {
    if (error instanceof Error && (error.message === 'Timed out in waitFor!' || error.message === timeoutMessage)) {
...
async function waitForPersistedTurnStart(
  ...
  const timeoutMessage = `snapshot-harness: session "${sessionId}" did not persist ${detail} within ${timeoutMs}ms`
...
async function waitForPersistedTurnEnd(
  ...
  const timeoutMessage = `snapshot-harness: session "${sessionId}" did not persist turn/end within ${timeoutMs}ms`
  await waitForHarnessCondition(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    if (log === undefined || !latestTurnIsClosed(log.content)) throw new Error(timeoutMessage)
  }, timeoutMs, timeoutMessage)
}
...
async function waitForPersistedChildTurnEnd(
  ...
  const timeoutMessage = `snapshot-harness: subagent child #${child} did not persist closed turn ${minimumTurn} within ${timeoutMs}ms`
  await waitForHarnessCondition(async () => {
...
async function waitForPersistedGoalPhase(
  ...
  const timeoutMessage = `snapshot-harness: session "${sessionId}" did not persist goal phase "${phase}" within ${timeoutMs}ms`
  await waitForHarnessCondition(async () => {
    ...
    if (!matched) throw new Error(timeoutMessage)
  }, timeoutMs, timeoutMessage)
}
...
async function waitForPersistedInboxMessage(
  ...
  const timeoutMessage = `snapshot-harness: session "${sessionId}" did not persist expected inbox message within ${timeoutMs}ms`
  await waitForHarnessCondition(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
...
async function waitForPersistedTitleAfterTurnEnd(
  ...
  const timeoutMessage = `snapshot-harness: session "${sessionId}" did not persist session/title after turn/end within ${timeoutMs}ms`
  await waitForHarnessCondition(async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
    if (log === undefined || !latestTitleFollowsTurnEnd(log.content)) throw new Error(timeoutMessage)
  }, timeoutMs, timeoutMessage)
}
...
async function waitForWorkspaceFile(
  ...
  const timeoutMessage = `snapshot-harness: workspace file "${path}" did not appear within ${timeoutMs}ms`
  await waitForHarnessCondition(() => {
    if (!existsSync(target)) throw new Error(timeoutMessage)
  }, timeoutMs, timeoutMessage)
}
