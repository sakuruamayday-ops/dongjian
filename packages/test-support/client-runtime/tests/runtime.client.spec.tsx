...
describe('sessions', () => {
  ...
  it('creates an addressable blank session without changing the current selection', async () => {
    const runtime = await runtimeWithFrame()
    await runtime.sessions.add({ id: 's1' })
    runtime.sessions.stubCreate(async options => runtime.sessions.add({
      id: 'created',
      summary: { ...(options?.cwd === undefined ? {} : { cwd: options.cwd }), blank: true },
    }, { current: false }))
    const created = await runtime.sessions.create({ cwd: '/workspace/demo' })
    expect(runtime.sessions.list.getSnapshot()).toMatchObject({
      current: 's1',
      byId: { [created]: { id: created, cwd: '/workspace/demo', blank: true } },
    })
    expect(runtime.sessions.binding(created)).toBeDefined()
    expect(runtime.sessions.calls).toContainEqual({ method: 'create', args: [{ cwd: '/workspace/demo' }] })
    await runtime.dispose()
  })
