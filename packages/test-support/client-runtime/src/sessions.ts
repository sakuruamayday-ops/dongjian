...
export class TestSessions implements ISessions {
  ...
  readonly calls: {
    method: 'create' | 'open' | 'openSubagent' | 'setSubagentCatalogOpen' | 'refreshSubagents'
      | 'clear' | 'refresh' | 'search' | 'fork' | 'forkToWorkspace'
    args: unknown[]
  }[] = []
  ...
  /** Record a Workspace-targeted fork without materializing a child fixture. */
  forkToWorkspace(opts: Parameters<ISessions['forkToWorkspace']>[0]): Promise<SessionId> {
    this.calls.push({ method: 'forkToWorkspace', args: [opts] })
    return Promise.resolve(opts.sessionId)
  }
