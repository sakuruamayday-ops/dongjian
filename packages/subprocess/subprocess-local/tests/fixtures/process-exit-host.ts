...
// Keep the parent harness from inspecting the process table until the
// descendant has been spawned and the fixture has published both identities.
// Without this barrier, fast machines can race `ps`/`sysctl` and make the
// process-exit test fail even though the cleanup implementation is correct.
