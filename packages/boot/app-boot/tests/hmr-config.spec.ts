...
describe('HMR exact config paths', () => {
  ...
  it('normalizes refresh failures and broadcasts them without escaping the watcher', { timeout: 45_000 }, async () => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-hmr-config-'))
    hmrRoots.push(dir)
    const filename = join(dir, 'plugins.yml')
    const ctx = await bootHmr(dir)
    const firstFailure = Promise.withResolvers<{ filename: string; error: Error }>()
    ...
    try {
      ...
      ctx.on('hmr/config-update-failed', (failedFilename, error) => {
        failureCount += 1
        firstFailure.resolve({ filename: failedFilename, error })
        if (failureCount === 2) secondFailure.resolve(undefined)
      })
      ...
      const observed = await within(
        ...
        30_000,
        'HMR did not broadcast the first config refresh failure',
      )
      ...
      await within(
        ...
        30_000,
        'HMR stopped broadcasting after an observer rejected',
      )
