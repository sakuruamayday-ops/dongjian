...
async function eventually(test: () => boolean, message: string): Promise<void> {
  // Chokidar competes with hundreds of worker processes in the repository
  // suite. This checks eventual transaction delivery, not watcher latency.
...
describe('boot with user patches', () => {
  ...
  it('watches add, failure, recovery, and removal through transactional HMR', { timeout: 20_000 }, async () => {
    ...
    try {
      ...
      // Default compose: the user layer IS the whole patch list, so the
      // initial watcher scan replaces the app-owned layer instead of stacking on it.
      await dispose()
      writeFileSync(filename, '- id: noop\n  config:\n    value: identity\n')
