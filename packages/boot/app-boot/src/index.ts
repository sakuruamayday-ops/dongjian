...
import Loader, {
  type Entry, type EntryOptions, type ModuleFormat, type ModulePhase, type ModuleRequest, type ResolveResult,
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    ...
    /** Trusted packaged-host base used by non-Loader module scanners. */
...
/**
 * Install the public-API module resolver used by packaged Electron hosts.
 *
 * Electron deliberately does not expose Node's internal ESM loader hook that
 * Cordis normally discovers. Resolving only the root Include is insufficient:
 * plugins and agent presets can create nested Loader trees of their own. The
 * fallback therefore lives on the Loader service itself so every tree shares
 * the same closed, host-owned package graph. Relative and absolute specifiers
 * keep their configuration-defined meaning; only bare package names resolve
 * from the explicit host base.
 *
 * An existing native loader always wins. Ordinary CLI boots do not pass a host
 * base and therefore retain the Loader's native behavior.
 *
 * @param ctx - context carrying an initialized Loader service.
 * @param bareModuleBaseUrl - file URL or absolute filename anchoring the
 * installed host's dependency graph.
 * @returns true when this call installed the fallback.
 */
export function installHostModuleResolver(ctx: Context, bareModuleBaseUrl?: string): boolean {
  if (bareModuleBaseUrl === undefined || ctx.loader.internal !== undefined) return false
  const requireFromHost = createRequire(bareModuleBaseUrl)
  const packageFormat = (filename: string): 'commonjs' | 'module' => {
    let directory = dirname(filename)
    while (true) {
      const manifest = resolve(directory, 'package.json')
      try {
        const type = (JSON.parse(readFileSync(manifest, 'utf8')) as { type?: unknown }).type
        return type === 'module' ? 'module' : 'commonjs'
      } catch (error) {
        if ((error as NodeJS.ErrnoException | null)?.code !== 'ENOENT') {
          throw new Error(`packaged Host resolver could not read package type from ${manifest}`, { cause: error })
        }
      }
  ...
  const dataFormat = (url: string): ModuleFormat => {
    const header = new URL(url).pathname.split(',', 1)[0] ?? ''
    const mediaType = header.split(';', 1)[0]?.toLowerCase()
    if (mediaType === 'text/javascript' || mediaType === 'application/javascript') return 'module'
    if (mediaType === 'application/json') return 'json'
    if (mediaType === 'application/wasm') return 'wasm'
    throw new Error(`packaged Host resolver does not support data URL media type ${JSON.stringify(mediaType || 'text/plain')}`)
  }
  const fileFormat = (url: string): ModuleFormat => {
    ...
    switch (extname(filename)) {
      ...
      case '':
      case '.js': return packageFormat(filename)
      default: throw new Error(`packaged Host resolver does not support file extension ${JSON.stringify(extname(filename))}`)
    }
  }
  const resolveUrl = (name: string, parentUrl: string): string => {
    if (isBuiltin(name)) return name.startsWith('node:') ? name : `node:${name}`
    if (name.startsWith('node:')) throw new Error(`packaged Host resolver does not recognize builtin ${JSON.stringify(name)}`)
    if (name.startsWith('data:') || name.startsWith('file:')) return new URL(name).href
    if (name.startsWith('.')) return new URL(name, parentUrl).href
    if (isAbsolute(name)) return pathToFileURL(name).href
    if (/^[A-Za-z][A-Za-z+.-]*:/.test(name)) {
      throw new Error(`packaged Host resolver does not support URL scheme in ${JSON.stringify(name)}`)
    }
    const resolved = requireFromHost.resolve(name)
    if (isBuiltin(resolved)) return resolved.startsWith('node:') ? resolved : `node:${resolved}`
    return pathToFileURL(resolved).href
  }
  const evaluationPhase: ModulePhase = 2
  const assertEvaluationPhase = (phase: ModulePhase | undefined): void => {
    if (phase === undefined || phase === evaluationPhase) return
    throw new Error(`packaged Host resolver only supports evaluation phase (2), received ${String(phase)}`)
  }
  const assertAttributes = (format: ModuleFormat, url: string, attributes: ImportAttributes): void => {
    const entries = Object.entries(attributes)
    const unsupported = entries.find(([name]) => name !== 'type')
    if (unsupported !== undefined) {
      throw new Error(`packaged Host resolver does not support import attribute ${JSON.stringify(unsupported[0])}`)
    }
    if (format === 'json') {
      if (entries.length === 1 && attributes.type === 'json') return
      throw new Error(`packaged Host resolver requires import attribute type "json" for ${url}`)
    }
    if (entries.length !== 0) {
      throw new Error(`packaged Host resolver does not support import attributes for ${format} module ${url}`)
    }
  }
  const formatOf = (url: string): ModuleFormat => {
    if (url.startsWith('node:')) return 'builtin'
    if (url.startsWith('data:')) return dataFormat(url)
    if (url.startsWith('file:')) return fileFormat(url)
    throw new Error(`packaged Host resolver produced unsupported URL ${JSON.stringify(url)}`)
  }
  const resolveRequest = (parentUrl: string, request: ModuleRequest): ResolveResult => {
    assertEvaluationPhase(request.phase)
    const attributes = request.attributes ?? {}
    const url = resolveUrl(request.specifier, parentUrl)
    const format = formatOf(url)
    assertAttributes(format, url, attributes)
    return { format, url }
  }
  ctx.loader.internal = {
    version: 'v2',
    // Package scanners use resolveSync to find manifests inside ASAR. It must
    // share import's host anchor or a packaged profile composes an empty client graph.
    resolveSync(parentUrl: string, request: ModuleRequest) {
      return resolveRequest(parentUrl, request)
    },
    async import(
      name: string, parentUrl: string, attributes: ImportAttributes, phase?: ModulePhase,
    ): Promise<unknown> {
      ...
      const { url } = resolveRequest(parentUrl, request)
      return import(/* @vite-ignore */url, { with: attributes })
    },
  ...
  ctx.loader.builtins.include = bareModuleBaseUrl === undefined
    ...
    : class HostResolvedRootInclude extends Include {
      override import(name: string, getOuterStack?: () => string[]): unknown {
        ...
        // Electron's embedded Node deliberately hides the internal ESM loader.
        // A closed host still has a complete dependency graph anchored at its
        // package.json, so resolve through the public createRequire API and
        // import the resulting file URL. The host opted into this exact base;
        // ordinary profile boots keep Loader's native behavior above.
...
export async function boot(
  ...
  try {
    ...
    installHostModuleResolver(ctx, bareModuleBaseUrl)
    await prepare?.(ctx)
