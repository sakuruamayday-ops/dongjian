...
describe('startup cleanup sweep', () => {
  ...
  it('keeps a file exactly at the boundary (only strictly-older expires)', async () => {
    ...
    // Filesystems persist mtimes at different precisions. Use the value that
    // actually came back from disk so this assertion exercises equality, not a
    // rounding difference between the requested timestamp and stored mtime.
