  ...
  const implementation = {
    ...
    async newSession(params: NewSessionRequest, signal: AbortSignal): Promise<NewSessionResponse> {
      ...
      try {
        ...
        assertOpen()
        // Publish only after activation is complete. Otherwise a concurrent
        // adapter refresh can emit session/update before session/new returns,
        // and the ACP client has no known sessionId to apply that update to.
    ...
    async resumeSession(params: ResumeSessionRequest, signal: AbortSignal): Promise<ResumeSessionResponse> {
      ...
      return (async (): Promise<ResumeSessionResponse> => {
        ...
        try {
          const configOptions = await record.configOptions(signal)
          assertOpen()
          // `activating` already excludes duplicate resumes; keep the record
          // private until its response payload is complete for the same reason
          // as session/new above.
          sessions.set(sessionId, record)
