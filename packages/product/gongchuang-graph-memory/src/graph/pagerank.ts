...
const graphCaches = new WeakMap<object, GraphStructure>()
...
function loadGraph(db: DatabaseSyncInstance): GraphStructure {
  const cached = graphCaches.get(db)
  ...
  const loaded = { nodeIds, adj, N: nodeIds.size, cachedAt: Date.now() }
  graphCaches.set(db, loaded)
  return loaded
}
...
export function invalidateGraphCache(db: DatabaseSyncInstance): void {
  graphCaches.delete(db)
}

// ─── 个性化 PageRank ─────────────────────────────────────────

/**
 * Data contract for pprresult.
 */
...
/**
 * Data contract for global page rank result.
 */
 ...
 * @param db - The db value.
 * @param cfg - The cfg value.
 * @returns The compute global page rank result.
 */
