...
const VALID_NODE_TYPES = new Set<NodeType>(['TASK', 'SKILL', 'EVENT'])
...
const FINALIZE_USER = (nodes: GmNode[], summary: string) =>
  `<Session Nodes>
...
function isNodeType(value: unknown): value is NodeType {
  return typeof value === 'string' && VALID_NODE_TYPES.has(value as NodeType)
}

function isEdgeType(value: unknown): value is EdgeType {
  return typeof value === 'string' && VALID_EDGE_TYPES.has(value as EdgeType)
}

type ExtractedNode = ExtractionResult['nodes'][number]
type ExtractedEdge = ExtractionResult['edges'][number]

function parseNode(value: unknown): ExtractedNode | null {
  if (!isRecord(value)) return null
  if (!isNodeType(value.type) || typeof value.name !== 'string' || value.name === '') return null
...
function parseEdge(value: unknown): ExtractedEdge | null {
  if (!isRecord(value)) return null
  if (typeof value.from !== 'string' || value.from === '') return null
  if (typeof value.to !== 'string' || value.to === '') return null
  if (!isEdgeType(value.type)) return null
  if (typeof value.instruction !== 'string' || value.instruction === '') return null
  return {
    from: normalizeName(value.from),
    to: normalizeName(value.to),
    type: value.type,
...
function correctEdgeType(
  ...
  const fromOk = EDGE_FROM_CONSTRAINT[type].has(fromType)
  const toOk = EDGE_TO_CONSTRAINT[type].has(toType)
  if (!fromOk || !toOk) {
    return null
  }
...
/**
 * Service implementation for extractor.
 */
export class Extractor {
  constructor(_cfg: GmConfig, private llm: CompleteFn) { void _cfg }

  /**
 * Perform the extract operation through the owning service.
 * @param params - The params value.
 * @returns The extract result.
 */
  async extract(params: {
    messages: Array<{ role: string; turn_index: number; content: string }>
    existingNames: string[]
  }): Promise<ExtractionResult> {
    const msgs = params.messages
      .map(m => `[${m.role.toUpperCase()} t=${m.turn_index}]\n${
  ...
  /**
 * Perform the finalize operation through the owning service.
 * @param params - The params value.
 * @returns The finalize result.
 */
  async finalize(params: { sessionNodes: GmNode[]; graphSummary: string }): Promise<FinalizeResult> {
  ...
  private parseExtract(raw: string): ExtractionResult {
    try {
      ...
      if (!isRecord(parsed)) throw new Error('extraction result must be an object')

      const nodes = unknownArray(parsed.nodes)
        .map(parseNode)
        .filter((node): node is ExtractedNode => node !== null)

      const nameToType = new Map<string, NodeType>()
      for (const n of nodes) nameToType.set(n.name, n.type)

      const edges = unknownArray(parsed.edges)
        .map(parseEdge)
        .filter((edge): edge is ExtractedEdge => edge !== null)
        .map(edge => correctEdgeType(edge, nameToType))
        .filter((edge): edge is ExtractedEdge => edge !== null)

      return { nodes, edges }
    } catch (err) {
      throw new Error(
        `[graph-memory] extraction parse failed: ${String(err)}\nraw (first 200): ${raw.slice(0, 200)}`,
      )
    }
  }

  private parseFinalize(raw: string, sessionNodes: GmNode[]): FinalizeResult {
    try {
      const json = extractJson(raw)
      const parsed = JSON.parse(json) as unknown
      if (!isRecord(parsed)) return { promotedSkills: [], newEdges: [], invalidations: [] }

      const nameToType = new Map<string, NodeType>()
      for (const node of sessionNodes) {
        nameToType.set(normalizeName(node.name), node.type)
      }
      const promotedSkills = unknownArray(parsed.promotedSkills)
        .map((value): FinalizeResult['promotedSkills'][number] | null => {
          if (!isRecord(value)) return null
        ...
        .filter((node): node is FinalizeResult['promotedSkills'][number] => node !== null)
      for (const node of promotedSkills) {
        nameToType.set(node.name, 'SKILL')
      }

      const newEdges = unknownArray(parsed.newEdges)
        .map(parseEdge)
        .filter((edge): edge is ExtractedEdge => edge !== null)
        .map(edge => correctEdgeType(edge, nameToType))
        .filter((edge): edge is ExtractedEdge => edge !== null)
        .map(({ from, to, type, instruction }) => ({ from, to, type, instruction }))

      return {
        promotedSkills,
        newEdges,
        invalidations: unknownArray(parsed.invalidations).filter((id): id is string => typeof id === 'string'),
      }
