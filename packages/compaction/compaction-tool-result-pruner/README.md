...
Every tool result whose text exceeds the threshold is replaced by a trimmed version: the configured head, a visible middle marker, and the configured tail. A result paired with a logged `skill` call names that skill and says the model may call `skill` once more if it needs the omitted instructions. A second pruned load in the same direct-user turn says to continue from retained instructions rather than reloading again; a later direct user message resets that one-shot notice. Ordinary tools keep the generic marker even when their output happens to contain `<skill_content>` text. Rich content such as images and structured blocks keeps its order. The replacement keeps the tool call, step, errors, and metadata — only the text content changes. If a replacement cannot be recorded, the run fails and the trims already applied stay in place.
...
Character counts are Unicode code points, so slicing never splits an emoji pair, though a multi-character grapheme can still be cut. The configured head plus the generic marker plus the tail must fit within the threshold. A longer skill-recovery notice uses otherwise unused threshold budget; when a custom threshold cannot fit that notice with both configured slices, the pruner preserves as much tail as possible before reducing the head. If the notice itself cannot fit, the validated generic marker is used. Every emitted result remains within the threshold and converges in one pass. An unknown setting rejects the plugin at construction.
...
- **Deterministic one-pass convergence.** Slicing is by Unicode code point with bounded budgets, so every emitted result is no larger than `thresholdChars` and is strictly smaller than the triggering input. Generic results retain the configured head and tail exactly; a longer skill-recovery notice reduces retained text only when a custom threshold leaves insufficient room.
...
- [Skill-prune recovery Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-29-skill-prune-one-shot-reload.md) — why skill results receive one explicit reload opportunity rather than automatic retries or pagination.
...
Once a compaction trigger qualifies, ordinary tool results show the retained head, `\n\n[... tool result middle pruned ...]\n\n`, and retained tail in place of the removed text. A long `skill` result instead includes `<skill-result-pruned reload="available">`, the exact skill name, and a direction to call `skill` at most once if the omitted instructions are needed. If that reloaded result is pruned before another direct user message, it carries `reload="used"` and explicitly forbids another reload solely because of the marker. Rich blocks keep their order. The model does not see a second copy of the original.
...
- **Skill recovery is one full reload, not pagination** — the notice reuses the existing `skill` tool and does not add offsets or a second instruction store. A second prune in the same human turn stops reload prompting and leaves the retained head and tail as the available instructions.
