...
import {
  ...
  SHARP_LIBVIPS_PACKAGE_NAMES,
  SHARP_LIBVIPS_VERSION,
...
describe('THIRD_PARTY_NOTICES.md', () => {
  ...
  it('matches what the generator produces from the current manifests', { timeout: 20_000 }, () => {
...
describe('macOS sharp/libvips distribution authorization', () => {
  it('authorizes only the two reviewed payload identities', () => {
    expect(SHARP_LIBVIPS_PACKAGE_NAMES.every(isOwnerAuthorizedRuntime)).toBe(true)
    expect(isOwnerAuthorizedRuntime('@img/sharp-libvips-linux-x64')).toBe(false)
    expect(isOwnerAuthorizedRuntime('@img/sharp-libvips-darwin-arm64-extra')).toBe(false)
  })

  it('pins the reviewed version and LGPL declaration', () => {
    const name = SHARP_LIBVIPS_PACKAGE_NAMES[0]
    expect(() => {
      validateSharpLibvipsManifest(name, {
        name,
        version: SHARP_LIBVIPS_VERSION,
    ...
    }).toThrow('review distribution terms')
    expect(() => {
      validateSharpLibvipsManifest('@img/sharp-libvips-linux-x64', undefined)
    }).toThrow('outside the reviewed sharp-libvips payload identities')
  })
})
