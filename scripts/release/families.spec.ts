...
describe('release families', () => {
  ...
  it('keeps product-private workspace packages out of the upstream npm family', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-release-family-'))
    mkdirSync(join(root, 'apps/cli'), { recursive: true })
    mkdirSync(join(root, 'packages/guard/private-gate'), { recursive: true })
    writeFileSync(join(root, 'apps/cli/package.json'), JSON.stringify({
      name: '@deepseek-ai/dsh', version: '0.1.0-rc.5',
    }))
    writeFileSync(join(root, 'packages/guard/private-gate/package.json'), JSON.stringify({
      name: '@gongchuang/client-policy-gate', version: '0.1.0', private: true,
    }))
