...
/** Product-owned packages are bundled into the desktop app, not published to npm. */
const privateProductPackageNamePrefix = '@gongchuang/'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/
const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@deepseek-ai/dsh': ['lib/*.js', 'lib/types/profile-boot.d.ts'],
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    && manifest.name?.startsWith(privateProductPackageNamePrefix) !== true) {
