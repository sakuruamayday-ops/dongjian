...
describe('client TypeScript aggregate', () => {
  ...
  it('keeps model-connection type output in its owning project', () => {
    const configPath = resolve(root, 'packages/api/remotes/tsconfig.client.json')
    const read = ts.readConfigFile(configPath, file => ts.sys.readFile(file))
    if (read.error !== undefined) {
      throw new Error(ts.flattenDiagnosticMessageText(read.error.messageText, '\n'))
    }
    const config = read.config as unknown as { references?: Array<{ path?: unknown }> }
    const references = config.references ?? []
    expect(references.map(reference => reference.path)).toContain(
      '../../product/gongchuang-model-connections',
    )
  })
})
