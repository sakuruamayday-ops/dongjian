...
/**
 * Return whether a source-looking path is a short-lived lint probe rather than
 * workspace source. Contract tests create these files inside package trees so
 * linters exercise the real include rules; concurrent catalog scans must not
 * treat a probe that may disappear between glob and read as product source.
 * @param file - absolute or repo-relative path returned by a source glob.
 */
export function isTransientLintProbe(file: string): boolean {
  const name = basename(file)
  return name.startsWith('oxlint-contract-') || name.startsWith('staged-lint-probe-')
}
...
export function scanSlotFiles(scanRoot: string, patterns: readonly string[]): ScannedFile[] {
  ...
  const rels = [...new Set(globSync(patterns as string[], { cwd: scanRoot })
    ...
    .filter(rel => !isTransientLintProbe(rel))
    .sort()
