...
/** Exact prebuilt macOS libvips distribution reviewed for the packaged client. */
export const SHARP_LIBVIPS_VERSION = '1.3.2'
export const SHARP_LIBVIPS_PACKAGE_NAMES = [
  '@img/sharp-libvips-darwin-arm64',
  '@img/sharp-libvips-darwin-x64',
] as const
const SHARP_LIBVIPS_PACKAGES = new Set<string>(SHARP_LIBVIPS_PACKAGE_NAMES)
const SHARP_LIBVIPS_LICENSE = 'LGPL-3.0-or-later'
 ...
 * @returns true only for the official Claude Agent SDK package or the exact
 *   reviewed macOS libvips payload identities.
 */
export function isOwnerAuthorizedRuntime(name: string): boolean {
  return name === CLAUDE_AGENT_SDK_PACKAGE || SHARP_LIBVIPS_PACKAGES.has(name)
}

/** Validate one installed libvips payload against the reviewed distribution. */
export function validateSharpLibvipsManifest(
  name: string,
  manifest: Manifest | undefined,
): void {
  if (!SHARP_LIBVIPS_PACKAGES.has(name)) {
    throw new Error(`gen-third-party-notices: ${name} is outside the reviewed sharp-libvips payload identities.`)
  }
  if (
    manifest?.name !== name
    || manifest.version !== SHARP_LIBVIPS_VERSION
    || manifest.license !== SHARP_LIBVIPS_LICENSE
  ) {
    throw new Error(
      `gen-third-party-notices: installed ${name} must be ${SHARP_LIBVIPS_VERSION} with declared license ${SHARP_LIBVIPS_LICENSE}; review distribution terms before changing it.`,
    )
  }
}
...
function collectSharpLibvipsDistribution(
  runtimeDeps: readonly ExternalDep[],
  manifests: Map<string, Manifest>,
): readonly string[] | undefined {
  const names = runtimeDeps
    .filter(dep => SHARP_LIBVIPS_PACKAGES.has(dep.name))
    .map(dep => dep.name)
    .sort()
  if (names.length === 0) return undefined
  const expected = [...SHARP_LIBVIPS_PACKAGE_NAMES].sort()
  if (names.join('\n') !== expected.join('\n')) {
    throw new Error(
      `gen-third-party-notices: reviewed sharp-libvips payload set must remain ${expected.join(', ')}.`,
    )
  }
  for (const name of names) validateSharpLibvipsManifest(name, installedManifest(name, manifests))
  return names
}
...
function verifyBuildTimePins(): void {
  for (const tool of BUILD_TIME_TOOLS) {
    const value = readFileSync(resolve(root, tool.pinSource), 'utf8')
...
function renderSharpLibvipsDistribution(names: readonly string[] | undefined): string {
  if (names === undefined) return ''
  const rows = names.map(name =>
    `| [\`${name}\`](https://www.npmjs.com/package/${name}) | ${SHARP_LIBVIPS_VERSION} | ${SHARP_LIBVIPS_LICENSE} |`,
  )
  return `
## macOS sharp/libvips payloads

The packaged macOS client dynamically loads the following unmodified prebuilt shared-library payloads through \`sharp\`. The project owner authorizes distribution only for these exact package identities at version ${SHARP_LIBVIPS_VERSION}; any identity, version, declared-license, binary, or component-set change requires a new distribution review.

| Package | Version | Declared license |
| --- | --- | --- |
${rows.join('\n')}

The GNU GPLv3 and LGPLv3 texts, the exact upstream component notices and versions, and the corresponding-source and relinking information are bundled in [\`apps/desktop/assets/licenses/sharp-libvips-${SHARP_LIBVIPS_VERSION}/\`](apps/desktop/assets/licenses/sharp-libvips-${SHARP_LIBVIPS_VERSION}/). The same directory is copied into every packaged macOS application at \`Contents/Resources/product/brand/licenses/sharp-libvips-${SHARP_LIBVIPS_VERSION}/\`.
`
}
...
export function render(): string {
  ...
  const claudeDistribution = runtimeDeps.some(
    ...
    : undefined
  const sharpLibvipsDistribution = collectSharpLibvipsDistribution(runtimeDeps, manifests)
...
${renderSharpLibvipsDistribution(sharpLibvipsDistribution)}
