...
import { isTransientLintProbe } from './slot-walk.ts'
...
describe('client slot contract validation', () => {
  it('excludes transient lint probes from source catalog discovery', () => {
    expect(isTransientLintProbe('packages/client/ui-primitives/src/oxlint-contract-123.ts')).toBe(true)
    expect(isTransientLintProbe('scripts/staged-lint-probe-123.ts')).toBe(true)
    expect(isTransientLintProbe('packages/client/ui-primitives/src/contract.ts')).toBe(false)
  })
