...
const SINGLE_LOCALE_PRODUCT_ROOTS = ['packages/client/ui-gongchuang-product/']
...
const INTERNAL_SENTINEL = /^::[a-z][a-z0-9-]*$/
...
function containsProductText(text: string): boolean {
  ...
  return normalized !== ''
    ...
    && !INTERNAL_SENTINEL.test(normalized)
...
/** Keep explicitly single-locale product surfaces outside the shared DSH locale contract. */
export function clientUiI18nIncluded(file: string): boolean {
  const normalized = file.replaceAll('\\', '/')
  return !SINGLE_LOCALE_PRODUCT_ROOTS.some(prefix => normalized.startsWith(prefix))
}
